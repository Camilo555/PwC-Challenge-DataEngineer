...
class APICacheManager:
    ...
    def configure_endpoint(
        ...
        logger.info(
            f"Configured caching for {path_pattern}: {strategy.value}, TTL: {ttl or self.default_ttl}"
        )
    ...
    def _should_cache_request(
        self, method: str, path: str, response_status: int, response_size: int
    ) -> bool:
        ...
        try:
            ...
            if success:
                ...
                cache_entry = APICacheEntry(
                    ...
                    headers={
                        k: v for k, v in headers.items() if k.lower() in ["accept", "content-type"]
                    },
    ...
    async def get_cached_response(
        ...
        try:
            ...
            # Create response
            response = Response(
                content=body, status_code=cached_data["status_code"], headers=cached_data["headers"]
            )
    ...
    async def invalidate_by_pattern(
        self, pattern: str, trigger: InvalidationTrigger = InvalidationTrigger.MANUAL
    ) -> int:
        ...
        await publish_cache_invalidation(
            pattern, self.namespace, pattern=True, source="api_cache_manager"
        )
    ...
    async def apply_invalidation_rules(self, trigger: str, context: dict[str, Any] = None):
        ...
        for pattern in patterns:
            ...
            count = await self.invalidate_by_pattern(pattern, InvalidationTrigger(trigger))
            total_invalidated += count

        logger.info(
            f"Applied invalidation rules for trigger {trigger}: {total_invalidated} entries"
        )
    ...
    async def get_cache_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "endpoint_stats": {},
            "cache_health": {"hit_rate": 0.0, "avg_response_size": 0, "compression_savings": 0},
            "top_endpoints": [],
        }
        ...
        try:
            ...
            sorted_endpoints = sorted(
                endpoint_stats.items(), key=lambda x: x[1]["hits"], reverse=True
            )
            stats["top_endpoints"] = [
...
_api_cache_manager: APICacheManager | None = None
...
class APICacheMiddleware:
    ...
    def __init__(self, cache_manager: APICacheManager | None = None):
        self.cache_manager = cache_manager
