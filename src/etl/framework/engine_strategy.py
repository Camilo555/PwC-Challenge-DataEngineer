...
class PolarsEngine(DataFrameOperations):
    ...
    def read_parquet(self, path: str, columns: Optional[List[str]] = None) -> pl.LazyFrame:
    ...
    def write_parquet(self, df: Union[pl.DataFrame, pl.LazyFrame], path: str, 
                     partition_cols: Optional[List[str]] = None, mode: str = 'overwrite') -> None:
    ...
    def filter(self, df: Union[pl.DataFrame, pl.LazyFrame], condition: pl.Expr) -> Union[pl.DataFrame, pl.LazyFrame]:
    ...
    def groupby_agg(self, df: Union[pl.DataFrame, pl.LazyFrame], by: List[str], 
    ...
    def with_columns(self, df: Union[pl.DataFrame, pl.LazyFrame], 
                    expressions: Dict[str, pl.Expr]) -> Union[pl.DataFrame, pl.LazyFrame]:
        """Add or modify columns using Polars expressions."""
        return df.with_columns([expr.alias(name) for name, expr in expressions.items()])
    
    def window_dedup_latest(self, df: Union[pl.DataFrame, pl.LazyFrame], 
    ...
    def create_scd2_records(self, df: Union[pl.DataFrame, pl.LazyFrame], 
                           business_key_cols: List[str], hash_col: str, 
                           effective_date_col: str) -> Union[pl.DataFrame, pl.LazyFrame]:
        """Create SCD Type 2 records with validity periods."""
    ...
    def merge_delta(self, target_path: str, source_df: Union[pl.DataFrame, pl.LazyFrame], 
...
class SparkEngine(DataFrameOperations):
    ...
    def join(self, left: SparkDataFrame, right: SparkDataFrame, 
             on: Union[str, List[str]], how: str = 'inner') -> SparkDataFrame:
...
class DuckDBEngine(DataFrameOperations):
    ...
    def write_parquet(self, df: Any, path: str, partition_cols: Optional[List[str]] = None, 
...
def get_dataframe_engine(engine_type: Optional[str] = None, 
                        ...
                        config: Optional[Dict[str, Any]] = None) -> DataFrameOperations:
