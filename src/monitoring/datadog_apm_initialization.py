...
# DataDog imports
from ddtrace import config, patch, patch_all, tracer
...
class DataDogAPMInitializer:
    ...
    def __init__(self):
        ...
        self.runtime_metrics: RuntimeMetrics | None = None
    ...
    async def initialize_datadog_apm(self, service_name: str | None = None) -> bool:
        """
    ...
    def _configure_basic_tracer(self):
        ...
        config.trace.partial_flush_min_spans = (
            self.configuration.performance.partial_flush_min_spans
        )
    ...
    async def _setup_instrumentation(self):
        ...
        self.logger.info(
            f"Instrumentation setup completed. Patched: {list(self.patched_integrations)}"
        )
    ...
    async def _patch_integration(self, integration_name: str):
        ...
        try:
            self.available_integrations[integration_name]["module"]
    ...
    async def _setup_profiling(self):
        ...
        try:
            ...
            self.profiler = Profiler(
                ...
                tags={**self.configuration.service.global_tags, "component": "profiler"},
            )

            # Configure profiling types
            if not self.configuration.profiling.cpu_enabled:
                self.profiler._profilers = [
                    p for p in self.profiler._profilers if "cpu" not in str(type(p)).lower()
                ]

            if not self.configuration.profiling.memory_enabled:
                self.profiler._profilers = [
                    p for p in self.profiler._profilers if "memory" not in str(type(p)).lower()
                ]
            ...
            self.logger.warning(
                "DataDog profiling not available - ddtrace profiling module not found"
            )
        except Exception as e:
    ...
    def _configure_security_settings(self):
        ...
        if self.configuration.security.obfuscate_http_headers:
            config.http_header_tags = dict.fromkeys(
                self.configuration.security.obfuscate_http_headers, False
            )
...
_datadog_apm_initializer: DataDogAPMInitializer | None = None


async def initialize_datadog_apm(service_name: str | None = None) -> bool:
    """
...
def get_datadog_apm_initializer() -> DataDogAPMInitializer | None:
...
def get_datadog_apm_status() -> dict[str, Any]:
    ...
    if _datadog_apm_initializer:
        ...
        return {"is_initialized": False, "error": "DataDog APM not initialized"}
...
async def datadog_apm_startup_hook(service_name: str | None = None):
