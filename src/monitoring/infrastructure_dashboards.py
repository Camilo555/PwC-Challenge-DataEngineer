...
class DashboardDataProvider(ABC):
    ...
    @abstractmethod
    async def get_data(
        self, widget: DashboardWidget, time_range: dict[str, datetime]
    ) -> dict[str, Any]:
        """Get data for widget"""
        pass
...
class InfrastructureDataProvider(DashboardDataProvider):
    ...
    async def _get_component_performance_data(
    ...
    async def _get_availability_metrics_data(
        self, time_range: dict[str, datetime]
    ) -> dict[str, Any]:
        ...
        for monitor in self.health_manager.monitors.values():
            hours = int(
                (
                    time_range.get("end", datetime.now())
                    - time_range.get("start", datetime.now() - timedelta(hours=24))
                ).total_seconds()
                / 3600
            )
        ...
        return {
            ...
            "average_availability": statistics.mean(
                [c["availability_percent"] for c in availability_data]
            )
    ...
    async def _get_integration_health_data(self) -> dict[str, Any]:
        ...
        return {
            ...
            "healthy_integrations": len(
                [i for i in integration_health.values() if i.get("status") == "healthy"]
            ),
            "timestamp": datetime.now().isoformat(),
        }
    ...
    async def _get_performance_baseline_data(self) -> dict[str, Any]:
        ...
        baseline_data = []
        for _baseline_key, baseline_info in baselines.items():
...
class DashboardManager:
    ...
    async def get_dashboard_data(
        self, dashboard_id: str, time_range: dict[str, datetime] = None
    ) -> dict[str, Any]:
    ...
    async def get_widget_data(
        self, dashboard_id: str, widget_id: str, time_range: dict[str, datetime] = None
    ) -> dict[str, Any]:
    ...
    def get_dashboard_list(self) -> list[dict[str, Any]]:
        ...
        for _dashboard_id, dashboard in self.dashboards.items():
        ...
        return {
            ...
            "dashboard_types": {
                dashboard_type.value: len(
                    [d for d in self.dashboards.values() if d.type == dashboard_type]
                )
