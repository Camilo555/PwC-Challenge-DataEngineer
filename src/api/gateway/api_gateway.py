...
from api.gateway.service_registry import (
...
class ServiceMeshProxy:
    ...
    async def forward_request(
        self, request: Request, route_config: RouteConfig, service_instance: str | None = None
    ) -> Response:
        ...
        for attempt in range(route_config.retry_attempts + 1):
            try:
                ...
                if attempt < route_config.retry_attempts:
                    await self._wait_for_retry(attempt, route_config.retry_strategy)
                    logger.warning(
                        f"Request retry {attempt + 1}/{route_config.retry_attempts}: {e}"
                    )
                    continue
    ...
    async def _process_response(
        self, response: httpx.Response, route_config: RouteConfig
    ) -> Response:
...
class APIGateway(BaseHTTPMiddleware):
    ...
    def __init__(self, app, routes: list[RouteConfig], enable_service_mesh: bool = True):
        ...
        self.route_patterns = sorted(self.routes.keys(), key=lambda x: (-x.count("/"), -len(x), x))

    def find_matching_route(self, path: str) -> RouteConfig | None:
...
def create_api_gateway(app, custom_routes: list[RouteConfig] | None = None) -> APIGateway:
