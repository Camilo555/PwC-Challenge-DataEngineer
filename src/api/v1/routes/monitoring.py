...
async def get_metrics(
    metric_names: str | None = Query(None, description="Comma-separated metric names"),
...
async def create_alert(request: AlertRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid severity: {request.severity}"
        )
...
async def get_alert_details(alert_id: str):
    ...
    instances = [
        instance
        for instance in observability_collector.alert_instances.values()
...
async def get_top_metrics(limit: int = Query(10, description="Number of top metrics to return")):
    ...
    return {
        "top_metrics": [
            {"metric_name": metric, "data_points": count} for metric, count in top_metrics
        ],
