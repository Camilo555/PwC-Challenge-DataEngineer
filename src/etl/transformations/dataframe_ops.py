...
class DataFrameTransformer:
    ...
    def clean_sales_data(
        self, df: Union[pd.DataFrame, "pl.DataFrame", "SparkDataFrame"]
    ) -> Union[pd.DataFrame, "pl.DataFrame", "SparkDataFrame"]:
        """
    ...
    def _clean_sales_pandas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_cleaned = df_filtered.drop_duplicates(
            subset=["invoice_no", "stock_code", "quantity", "unit_price"], keep="first"
        )
    ...
    def _clean_sales_spark(self, df: "SparkDataFrame") -> "SparkDataFrame":
        ...
        df_transformed = df_filtered.withColumns(
            {
                ...
                "customer_id": F.when(F.col("customer_id").isNull(), "UNKNOWN").otherwise(
                    F.col("customer_id")
                ),
                "description": F.when(F.col("description").isNull(), "No Description").otherwise(
                    F.col("description")
                ),
        ...
        ).orderBy(F.col("invoice_date"))
        df_deduplicated = (
            df_transformed.withColumn("row_number", F.row_number().over(window_spec))
    ...
    def aggregate_by_dimensions(
        self, df: Union[pd.DataFrame, "pl.DataFrame", "SparkDataFrame"], group_cols: list[str]
    ) -> Union[pd.DataFrame, "pl.DataFrame", "SparkDataFrame"]:
        """
    ...
    def window_operations(
        self,
        df: Union[pd.DataFrame, "pl.DataFrame", "SparkDataFrame"],
    ...
    def _window_pandas(
        self, df: pd.DataFrame, partition_cols: list[str], order_cols: list[str]
    ) -> pd.DataFrame:
        ...
        # Add ranking
        df_windowed["amount_rank"] = df_windowed.groupby(partition_cols)["total_amount"].rank(
    ...
    def _window_polars(
        self, df: "pl.DataFrame", partition_cols: list[str], order_cols: list[str]
    ) -> "pl.DataFrame":
    ...
    def _window_spark(
        self, df: "SparkDataFrame", partition_cols: list[str], order_cols: list[str]
    ) -> "SparkDataFrame":
        ...
        return df.withColumns(
            {
                ...
                "running_total": F.sum("total_amount").over(
                    window_spec.rowsBetween(Window.unboundedPreceding, Window.currentRow)
                ),
                "amount_rank": F.dense_rank().over(
                    Window.partitionBy(*partition_cols).orderBy(F.col("total_amount").desc())
                ),
            }
        )
    ...
    def join_dataframes(
        self,
        left_df: Union[pd.DataFrame, "pl.DataFrame", "SparkDataFrame"],
    ...
    def pivot_operations(
        ...
        if self.engine == "pandas":
            return df.pivot_table(
                index=index_cols, values=value_col, aggfunc=agg_func, fill_value=0
            )
        elif self.engine == "polars":
            # Polars doesn't have direct pivot, use group_by with conditional aggregation
            return df.group_by(index_cols).agg([pl.col(value_col).sum().alias(f"{value_col}_sum")])
        elif self.engine == "spark":
