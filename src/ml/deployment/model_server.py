...
class ModelMetadata:
    ...
    feature_pipeline_path: str | None = None
    model_path: str = ""
...
class PredictionRequest(BaseModel):
    ...
    ab_test_context: dict[str, Any] | None = None
...
class ModelRegistry:
            ...
            if model_id not in self.models:
                ...
                if self.cache_manager:
                    cached_data = await self.cache_manager.get(
                        f"model_registry:{model_id}", namespace="ml_models"
                    )
                    ...
                    if cached_data:
                        ...
                        metadata = ModelMetadata(
                            ...
                            last_used=datetime.fromisoformat(
                                cached_data.get("last_used", cached_data["created_at"])
                            ),
                            usage_count=cached_data.get("usage_count", 0),
                            avg_latency_ms=cached_data.get("avg_latency_ms", 0.0),
                            enable_prediction_cache=cached_data.get(
                                "enable_prediction_cache", True
                            ),
                            prediction_cache_ttl=cached_data.get("prediction_cache_ttl", 300),
                            enable_deployment_notifications=cached_data.get(
                                "enable_deployment_notifications", True
                            ),
                            enable_prediction_streaming=cached_data.get(
                                "enable_prediction_streaming", True
                            ),
    ...
    async def unload_model(self, model_id: str) -> bool:
        ...
        try:
            ...
            self.metrics_collector.increment_counter(
                "model_registry_models_unloaded_total", tags={"model_id": model_id}
            )
    ...
    def get_feature_pipeline(self, model_id: str) -> FeatureEngineeringPipeline | None:
...
class ModelServer:
    ...
    def __init__(self, registry: ModelRegistry, feature_store: FeatureStore | None = None):
    ...
    async def _stream_prediction_event(
        self, request_id: str, model_id: str, event_data: dict[str, Any]
    ):
    ...
    async def predict(self, request: PredictionRequest) -> PredictionResponse:
        ...
        try:
            ...
            if request.cache_prediction and metadata.enable_prediction_cache and self.cache_manager:
                ...
                cached_prediction = await self.cache_manager.get(
                    cache_key, namespace="ml_predictions"
                )
                ...
                if cached_prediction:
                    ...
                    await self._stream_prediction_event(
                        request_id,
                        request.model_id,
                        {"from_cache": True, "cache_hit": True, "latency_ms": latency},
                    )
            ...
            if self.feature_store_client:
                ...
                try:
                    additional_features = await self.feature_store_client.get_online_features(
                        [{"name": "customer_features", "version": "1.0"}], entity_keys
                    )
            ...
            if metadata.problem_type == "classification":
                ...
                prediction = await loop.run_in_executor(self.executor, model.predict, features_df)
                probabilities = None
            ...
            if request.return_feature_importance:
                if hasattr(model, "feature_importances_"):
                    feature_importance = dict(
                        zip(features_df.columns, model.feature_importances_, strict=False)
                    )
                elif hasattr(model, "coef_"):
                    feature_importance = dict(
                        zip(features_df.columns, np.abs(model.coef_.flatten()), strict=False)
                    )
            ...
            self.metrics_collector.record_histogram(
                "model_server_prediction_latency_ms", latency, tags={"model_id": request.model_id}
            )
            ...
            self.metrics_collector.increment_counter(
                "model_server_prediction_errors_total", tags={"model_id": request.model_id}
            )
    ...
    async def predict_batch(self, request: BatchPredictionRequest) -> list[PredictionResponse]:
        ...
        try:
            ...
            if metadata.problem_type == "classification":
                ...
                predictions = await loop.run_in_executor(self.executor, model.predict, features_df)
                probabilities_array = None
            ...
            self.metrics_collector.increment_counter(
                "model_server_batch_prediction_errors_total", tags={"model_id": request.model_id}
            )
            raise HTTPException(status_code=500, detail=str(e))
...
if __name__ == "__main__":
    uvicorn.run("src.ml.deployment.model_server:app", host="0.0.0.0", port=8001, reload=True)
