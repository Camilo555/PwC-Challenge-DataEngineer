...
class PerformanceAnalyzer:
    ...
    def analyze_connection_patterns(self, connected_clients: int, max_clients: int) -> list[str]:
...
class RedisMonitor:
    ...
    def __init__(
        self, cache_manager: RedisCacheManager | None = None, monitoring_interval: int = 60
    ):  # 1 minute
    ...
    async def collect_metrics(self) -> list[RedisMetric]:
        ...
        try:
            ...
            metrics.extend(
                [
                    RedisMetric(
                        "used_memory", info.get("used_memory", 0), MetricType.MEMORY, timestamp
                    ),
                    RedisMetric(
                        "used_memory_rss",
                    ...
                    RedisMetric(
                        "maxmemory", info.get("maxmemory", 0), MetricType.MEMORY, timestamp
                    ),
                    RedisMetric(
            ...
            metrics.extend(
                [
                    ...
                    RedisMetric(
                        "keyspace_hits", keyspace_hits, MetricType.CACHE_EFFICIENCY, timestamp
                    ),
                    RedisMetric(
                        "keyspace_misses", keyspace_misses, MetricType.CACHE_EFFICIENCY, timestamp
                    ),
                    RedisMetric("hit_rate", hit_rate, MetricType.CACHE_EFFICIENCY, timestamp),
                    RedisMetric(
                        "expired_keys",
                        info.get("expired_keys", 0),
                        MetricType.CACHE_EFFICIENCY,
                        timestamp,
                    ),
                    RedisMetric(
                        "evicted_keys",
    ...
    async def _collect_system_metrics(self, timestamp: datetime) -> list[RedisMetric]:
        ...
        try:
            ...
            metrics.append(
                RedisMetric("system_cpu_percent", cpu_percent, MetricType.PERFORMANCE, timestamp)
            )
            ...
            metrics.extend(
                [
                    RedisMetric("system_memory_total", memory.total, MetricType.MEMORY, timestamp),
                    RedisMetric(
                        "system_memory_available", memory.available, MetricType.MEMORY, timestamp
                    ),
                    RedisMetric(
                        "system_memory_percent", memory.percent, MetricType.MEMORY, timestamp
                    ),
                ]
            )
            ...
            if disk_io:
                metrics.extend(
                    [
                        RedisMetric(
                            "disk_read_bytes", disk_io.read_bytes, MetricType.PERFORMANCE, timestamp
                        ),
                        RedisMetric(
                            "disk_write_bytes",
                            disk_io.write_bytes,
            ...
            if cluster_info:
                metrics.extend(
                    [
                        RedisMetric(
                            "cluster_state",
                            cluster_info.get("cluster_state", "unknown"),
                            MetricType.CLUSTER,
                            timestamp,
                        ),
                        RedisMetric(
                            "cluster_slots_assigned",
                            cluster_info.get("cluster_slots_assigned", 0),
                            MetricType.CLUSTER,
                            timestamp,
                        ),
                        RedisMetric(
                            "cluster_slots_ok",
                            cluster_info.get("cluster_slots_ok", 0),
                            MetricType.CLUSTER,
                            timestamp,
                        ),
                        RedisMetric(
                            "cluster_known_nodes",
                            cluster_info.get("cluster_known_nodes", 0),
                            MetricType.CLUSTER,
                            timestamp,
                        ),
                    ]
    ...
    async def _store_metrics(self, metrics: list[RedisMetric]):
        ...
        try:
            ...
            await cache_manager.lpush("metrics_history", *metrics_data, namespace=self.namespace)

            # Keep only recent metrics (last 1440 = 24 hours worth)
            await cache_manager.async_redis_client.ltrim(
                cache_manager._build_key("metrics_history", self.namespace), 0, 1440
            )
    ...
    async def _store_alert(self, alert: Alert):
        ...
        try:
            ...
            await cache_manager.set(
                ...
                alert_data,
                604800,
                self.namespace,  # 7 days
            )

            # Add to alerts list
            await cache_manager.lpush("alerts_history", alert.alert_id, namespace=self.namespace)

            # Keep only recent alerts (last 1000)
            await cache_manager.async_redis_client.ltrim(
                cache_manager._build_key("alerts_history", self.namespace), 0, 999
            )
        ...
        report = {
            ...
            "overview": {
                ...
                "memory_usage_percent": (
                    ...
                    if used_memory and maxmemory and maxmemory.value > 0
                    else 0
                ),
    ...
    async def suggest_memory_optimizations(self) -> list[str]:
        ...
        try:
            ...
            if expired_keys > 10000:
                suggestions.append("High number of expired keys, consider more aggressive cleanup")
