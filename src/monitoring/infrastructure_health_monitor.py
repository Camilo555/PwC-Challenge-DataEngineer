...
class HealthMonitorBase(ABC):
    ...
    def get_latest_metrics(self) -> HealthMetrics | None:
        """Get latest metrics"""
    ...
    def calculate_availability(self, hours: int = 24) -> float:
        ...
        healthy_count = sum(
            1 for m in recent_metrics if m.status in [HealthStatus.HEALTHY, HealthStatus.WARNING]
        )
        return (healthy_count / len(recent_metrics)) * 100.0

    def generate_alert(
        self, severity: AlertSeverity, title: str, message: str, metadata: dict[str, Any] = None
    ) -> Alert:
        """Generate alert"""
        alert = Alert(
    ...
    def update_circuit_breaker(self, success: bool):
        ...
        if (
            ...
            and (datetime.now() - cb.last_failure_time).seconds > cb.recovery_timeout
        ):
...
class RedisHealthMonitor(HealthMonitorBase):
    ...
    async def collect_metrics(self) -> RedisHealthMetrics:
        ...
        try:
            ...
            metrics = RedisHealthMetrics(
                ...
                cluster_state=info.get("cluster_enabled", "no") == "yes"
                and "cluster_state"
                or "standalone",
                custom_metrics={
    ...
    def _determine_redis_health_status(
        self, ping_time: float, memory_usage: float, hit_ratio: float, info: dict
    ) -> HealthStatus:
        ...
        if ping_time > self.response_time_critical_ms:
            ...
            ping_time > self.response_time_warning_ms
            or memory_usage > self.memory_usage_warning
            or hit_ratio < self.cache_hit_ratio_warning
        ):
...
class RabbitMQHealthMonitor(HealthMonitorBase):
        ...
        try:
            ...
            for queue_name in test_queues:
                try:
                    ...
                    self.logger.warning(
                        f"Could not get stats for queue {queue_name}: {queue_error}"
                    )
            ...
            metrics = RabbitMQHealthMetrics(
                ...
                node_status="running",
                custom_metrics={
                    "average_queue_depth": total_queue_depth / len(queue_metrics)
                    if queue_metrics
                    else 0,
                    "queues_without_consumers": sum(
                        1 for q in queue_metrics.values() if q["consumer_count"] == 0
                    ),
                },
            )
    ...
    def _determine_rabbitmq_health_status(
        self, total_depth: int, total_consumers: int, queue_count: int
    ) -> HealthStatus:
...
class KafkaHealthMonitor(HealthMonitorBase):
        ...
        try:
            ...
            metrics = KafkaHealthMetrics(
                ...
                metadata={
                    ...
                    if hasattr(metadata, "controller")
                    else -1,
                },
            )
    ...
    def _determine_kafka_health_status(
        self, broker_count: int, under_replicated: int, offline_partitions: int, consumer_lag: int
    ) -> HealthStatus:
        """Determine Kafka health status"""
        if (
            broker_count == 0
            or offline_partitions >= self.offline_partitions_threshold
            or consumer_lag >= self.consumer_lag_critical
        ):
            return HealthStatus.CRITICAL
        elif (
            under_replicated >= self.under_replicated_threshold
            or consumer_lag >= self.consumer_lag_warning
        ):
            return HealthStatus.WARNING
    ...
    async def _check_kafka_alerts(self, metrics: KafkaHealthMetrics):
        ...
        if metrics.broker_count == 0:
            self.generate_alert(
                AlertSeverity.EMERGENCY, "Kafka Cluster Offline", "No Kafka brokers are available"
            )
...
class InfrastructureHealthManager:
    ...
    async def _check_integration_health(self):
        ...
        try:
            ...
            redis_monitors = [
                m for m in self.monitors.values() if m.component_type == ComponentType.REDIS
            ]
            rabbitmq_monitors = [
                m for m in self.monitors.values() if m.component_type == ComponentType.RABBITMQ
            ]

            if redis_monitors and rabbitmq_monitors:
                redis_healthy = all(
                    m.get_latest_metrics().status in [HealthStatus.HEALTHY, HealthStatus.WARNING]
                    for m in redis_monitors
                    if m.get_latest_metrics()
                )
                rabbitmq_healthy = all(
                    m.get_latest_metrics().status in [HealthStatus.HEALTHY, HealthStatus.WARNING]
                    for m in rabbitmq_monitors
                    if m.get_latest_metrics()
                )
            ...
            for monitor in self.monitors.values():
                ...
                if baseline_key in self.baselines:
                    ...
                    baseline.baseline_value = (
                        alpha * current_value + (1 - alpha) * baseline.baseline_value
                    )

                    # Calculate deviation
                    baseline.current_value = current_value
                    baseline.deviation_percent = (
            ...
            for monitor in self.monitors.values():
                ...
                if component_status in [HealthStatus.CRITICAL, HealthStatus.OFFLINE]:
                    ...
                    critical_issues.append(
                        f"{monitor.component_id}: {health.get('status', 'unknown')}"
                    )
                elif (
                    component_status in [HealthStatus.WARNING, HealthStatus.DEGRADED]
    ...
    async def get_performance_summary(self) -> dict[str, Any]:
        ...
        try:
            performance_data = {"redis": {}, "rabbitmq": {}, "kafka": {}, "overall": {}}
...
def create_infrastructure_manager() -> InfrastructureHealthManager:
    ...
    try:
        ...
        redis_monitor = create_redis_monitor("primary_redis", host=redis_host, port=redis_port)
        ...
        rabbitmq_monitor = create_rabbitmq_monitor(
            "primary_rabbitmq", host=rabbitmq_host, port=rabbitmq_port
        )
