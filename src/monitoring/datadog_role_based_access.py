...
class DataDogRoleBasedAccess:
    ...
    def _initialize_role_permissions(self):
        ...
        self.role_permissions = {
            Role.SUPER_ADMIN: [
                ...
                Permission.METRICS_CREATE,
                Permission.METRICS_EDIT,
                ...
                Permission.DATA_SENSITIVE_VIEW,
                Permission.ML_MODEL_DEPLOY,
                Permission.ML_EXPERIMENT_CREATE,
                Permission.ML_EXPERIMENT_VIEW,
            ],
            Role.ADMIN: [
                Permission.DASHBOARD_CREATE,
                Permission.DASHBOARD_EDIT,
            ...
            Role.GUEST: [Permission.DASHBOARD_VIEW, Permission.METRICS_VIEW],
        }
                    ...
                    for user_data in users_data:
                        user = User(
                            ...
                            additional_permissions=[
                                Permission(p) for p in user_data.get("additional_permissions", [])
                            ],
                            restricted_permissions=[
                                Permission(p) for p in user_data.get("restricted_permissions", [])
                            ],
                        )
    ...
    async def validate_session(self, token: str) -> SessionInfo | None:
        ...
        try:
            ...
            for access in resource_accesses:
                if (
                    ...
                    and access.resource_type == resource_type
                    and (not access.expires_at or access.expires_at > current_time)
                ):
    ...
    async def grant_resource_access(
        ...
        try:
            ...
            self.resource_access[resource_id] = [
                access
                for access in self.resource_access[resource_id]
    ...
    async def revoke_resource_access(
        self, user_id: str, resource_type: ResourceType, resource_id: str, revoked_by: str
    ) -> bool:
        ...
        try:
            ...
            if success:
                ...
                await self._log_audit_event(
                    ...
                    resource_type=request.resource_type,
                    resource_id=request.resource_id,
                    additional_data={"request_id": request_id, "target_user": request.user_id},
                )
        ...
        while True:
            try:
                ...
                if cache_keys_to_remove:
                    self.logger.info(
                        f"Refreshed permission cache for {len(cache_keys_to_remove)} users"
                    )
        ...
        try:
            ...
            return {
                ...
                "permissions": {
                    ...
                    "cached_permission_checks": sum(
                        len(cache) for cache in self.permission_cache.values()
                    ),
                },
                "resource_access": {
...
_rbac_system: DataDogRoleBasedAccess | None = None


def get_rbac_system(
    service_name: str = "rbac-system", config_path: str | None = None, jwt_secret: str | None = None
) -> DataDogRoleBasedAccess:
...
async def authenticate_user(
    username: str, password: str, ip_address: str | None = None, user_agent: str | None = None
) -> str | None:
