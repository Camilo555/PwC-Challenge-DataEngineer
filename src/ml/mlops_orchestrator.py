...
from src.ml.deployment.model_server import ModelRegistry, ModelServer
from src.ml.feature_engineering.feature_store import FeatureStore, create_sales_feature_groups
from src.ml.insights.narrative_generator import InsightEngine
from src.ml.monitoring.model_monitor import DriftConfig, ModelMonitor
...
class MLOpsOrchestrator:
    ...
    async def _perform_health_check(self):
        ...
        try:
            ...
            # Check feature store
            try:
                await self.feature_store.list_feature_groups()
            ...
            for model_id, _monitor in self.model_monitors.items():
                try:
            ...
            for model_id, monitor in self.model_monitors.items():
                try:
                    ...
                    significant_drift = any(
                        ...
                        and result.drift_score > self.config.critical_alert_threshold
                        for result in drift_results
                    )

                    if significant_drift and self.config.auto_retrain_enabled:
                        logger.warning(
                            f"Significant drift detected for model {model_id}, triggering retraining"
                        )
    ...
    async def _refresh_features(self):
        ...
        try:
            ...
            await self.feature_store.ingest_features(
                "customer_features", "1.0", updated_customer_features
            )
