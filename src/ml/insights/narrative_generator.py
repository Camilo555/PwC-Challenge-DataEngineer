...
class InsightEngine:
    ...
    async def _analyze_trends(
        self, data: pd.DataFrame, date_column: str, metric_columns: list[str]
    ) -> list[Insight]:
            ...
            for metric in metric_columns:
                ...
                insight = Insight(
                    ...
                    recommendations=self._generate_trend_recommendations(
                        trend_type, metric, percentage_change
                    ),
                )
    ...
    async def _detect_anomalies(
        self, data: pd.DataFrame, metric_columns: list[str]
    ) -> list[Insight]:
                ...
                if len(anomalies) > 0:
                    ...
                    insight = Insight(
                        ...
                        recommendations=self._generate_anomaly_recommendations(
                            anomaly_type, metric, len(anomalies)
                        ),
                    )
        ...
        try:
            for metric in metric_columns:
                ...
                insight = Insight(
                    ...
                    summary=narrative,
                    detailed_explanation="Performance analysis based on variability assessment. "
                    "Lower coefficient of variation indicates more consistent performance.",
        ...
        try:
            ...
            if strong_correlations:
                ...
                correlation_strength = (
                    "very strong" if abs(strongest["correlation"]) > 0.9 else "strong"
                )
    ...
    def _generate_trend_recommendations(
        self, trend_type: str, metric: str, percentage_change: float
    ) -> list[str]:
        ...
        if trend_type == "increasing":
            if percentage_change > 20:
                recommendations.append(
                    f"Monitor the rapid growth in {metric} to ensure sustainability"
                )
            ...
            recommendations.append(
                "Maintain current performance levels while exploring improvements"
            )

        return recommendations

    def _generate_anomaly_recommendations(
        self, anomaly_type: str, metric: str, count: int
    ) -> list[str]:
    ...
    def _generate_performance_recommendations(
        self, performance_level: str, metric: str, cv: float
    ) -> list[str]:
    ...
    def _generate_correlation_recommendations(self, correlation: dict[str, Any]) -> list[str]:
        ...
        if correlation["correlation"] > 0:
            recommendations.append(
                f"Leverage the positive relationship between {metric1} and {metric2}"
            )
            recommendations.append(f"Consider using {metric1} as a leading indicator for {metric2}")
        else:
            recommendations.append(
                f"Investigate the inverse relationship between {metric1} and {metric2}"
            )
    ...
    def _generate_executive_summary(self, insights: list[Insight]) -> str:
        ...
        category_text = ", ".join(
            [f"{count} {category.replace('_', ' ')}" for category, count in categories.items()]
        )
    ...
    def _generate_technical_details(self, insights: list[Insight]) -> str:
        ...
        details.append("**Analysis Configuration**:")
