...
class StarSchemaBuilder:
    ...
    def __init__(self, engine: DataFrameOperations, config: dict[str, Any]):
        ...
        self.dimensions_config = {
            ...
            "customer": DimensionConfig(
                "customer",
                ["customer_id", "email"],
                ["customer_name", "segment", "country", "city"],
            ),
            "store": DimensionConfig(
                "store", ["store_id", "store_code"], ["store_name", "address", "manager"], type=1
            ),  # Type 1 - overwrite
        }
    ...
    def build_complete_star_schema(self, silver_data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Build fact table
        schema_tables["fact_sales"] = self.build_fact_sales(silver_data.get("sales"), schema_tables)
    ...
    def build_dim_date(self, start_date: str, end_date: str) -> Any:
        ...
        dates = pl.date_range(
            date.fromisoformat(start_date), date.fromisoformat(end_date), interval="1d", eager=True
        )
    ...
    def build_dim_product(self, df_product_silver: Any) -> Any:
        ...
        df = self._add_business_keys_and_hash(
            df_product_silver,
            {
    ...
    def build_dim_store(self, df_store_silver: Any) -> Any:
        ...
        df = self._add_business_keys_and_hash(
            df_store_silver,
            {"store_bk": ["store_id", "store_code"], "store_key": ["store_id", "store_code"]},
        )
        ...
        if POLARS_AVAILABLE:
            df = df.with_columns(
                [
                    ...
                    pl.concat_str([pl.col("country"), pl.col("region")], separator="_").alias(
                        "country_region"
                    ),
                    pl.concat_str(
                        [
    ...
    def build_fact_sales(self, df_sales_silver: Any, dimensions: dict[str, Any]) -> Any:
        ...
        self.engine.write_parquet(df, output_path, partition_cols=["date_key"], mode="overwrite")
    ...
    def _apply_scd2_logic(
        self, existing_df: Any, new_df: Any, business_key: str, hash_col: str, dimension_name: str
    ) -> Any:
        """
        ...
        if POLARS_AVAILABLE:
            existing_df.filter(pl.col("is_current"))
            ...
            closed_records = existing_df.with_columns(
                [pl.lit(datetime.now().date()).alias("end_date"), pl.lit(False).alias("is_current")]
            )
    ...
    def _add_scd2_columns(self, df: Any, dimension_name: str) -> Any:
        ...
        if POLARS_AVAILABLE and hasattr(df, "with_columns"):
            return df.with_columns(
                [
                    self._generate_surrogate_key_expr([f"{dimension_name}_bk"]).alias(
                        f"{dimension_name}_key"
                    ),
                    pl.lit(datetime.now().date()).alias("effective_date"),
                    pl.lit(date(9999, 12, 31)).alias("end_date"),
                    pl.lit(True).alias("is_current"),
    ...
    def _generate_business_key_expr(self, columns: list[str]) -> Any:
        ...
        if POLARS_AVAILABLE:
            ...
            return concat_expr.map_elements(
                lambda x: hashlib.sha256(str(x).encode()).hexdigest()[:16], return_dtype=pl.Utf8
            )
        else:
            return None
    ...
    def _join_with_product_dimension(self, df: Any, dim_product: Any) -> Any:
        ...
        if POLARS_AVAILABLE:
            current_products = dim_product.filter(pl.col("is_current"))
            return df.join(
                current_products.select(["product_key", "product_id"]), on="product_id", how="left"
    ...
    def _join_with_customer_dimension(self, df: Any, dim_customer: Any) -> Any:
        ...
        if POLARS_AVAILABLE:
            current_customers = dim_customer.filter(pl.col("is_current"))
            return df.join(
    ...
    def _join_with_store_dimension(self, df: Any, dim_store: Any) -> Any:
        ...
        if POLARS_AVAILABLE:
            return df.join(
                dim_store.select(["store_key", "store_id"]), on="store_id", how="left"
            ).with_columns([pl.col("store_key").fill_null(-1)])
        else:
            return df
            ...
            return df.with_columns(
                [
                    ...
                    (
                        pl.col("quantity")
                        * pl.col("unit_price")
                        * pl.col("discount_percentage").fill_null(0)
                        / 100
...
def create_star_schema_builder(
    engine_type: EngineType = EngineType.POLARS, config: dict[str, Any] | None = None
) -> StarSchemaBuilder:
    """
    ...
    """
    engine_config = EngineConfig(engine_type=engine_type, **(config or {}))
