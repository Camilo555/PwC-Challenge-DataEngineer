...
from messaging.rabbitmq_manager import QueueType, RabbitMQManager
...
class RabbitMQMonitor:
    ...
    async def collect_metrics(self) -> RabbitMQMetrics:
        ...
        try:
                ...
                try:
                    ...
                    if "error" not in stats:
                        ...
                        queue_metrics[queue_name] = {
                            ...
                            "health_status": self._assess_queue_health(
                        ...
                        self.metrics.set_gauge(
                            "rabbitmq_queue_depth", message_count, labels={"queue": queue_name}
                        )
                        self.metrics.set_gauge(
                            "rabbitmq_consumer_count", consumer_count, labels={"queue": queue_name}
                        )

                except Exception as e:
                    self.logger.warning(
                        f"Failed to collect metrics for queue {queue_type.value}: {e}"
                    )
            ...
            return RabbitMQMetrics(timestamp=datetime.now(), health_status=HealthStatus.UNKNOWN)
    ...
    def _cleanup_history(self):
        ...
        self.message_count_history = [
            point for point in self.message_count_history if point[0] > cutoff_time
        ]
...
class KafkaMonitor:
    ...
    async def collect_metrics(self) -> KafkaMetrics:
        ...
        try:
                ...
                try:
                    ...
                    if "error" not in topic_info:
                        ...
                        topic_metrics[topic_name] = {
                            ...
                            "health_status": self._assess_topic_health(
                                partition_count, consumer_lag
                            ),
                            "timestamp": current_time.isoformat(),
                        }
                        ...
                        self.metrics.set_gauge(
                            "kafka_partition_count", partition_count, labels={"topic": topic_name}
                        )
                        self.metrics.set_gauge(
                            "kafka_consumer_lag", consumer_lag, labels={"topic": topic_name}
                        )
            ...
            return KafkaMetrics(timestamp=datetime.now(), health_status=HealthStatus.UNKNOWN)
    ...
    def _calculate_message_rate(self) -> float:
        ...
        if self.kafka.producer:
            ...
            return (
                kafka_metrics.get("kafka_metrics", {}).get("messages_produced", 0) / 60.0
...
class MessagingMonitoringManager:
    ...
    async def collect_all_metrics(self):
        ...
        try:
            ...
            self.metrics.set_gauge(
                ...
                1
                if (
                    rabbitmq_metrics.health_status == HealthStatus.HEALTHY
                    and kafka_metrics.health_status == HealthStatus.HEALTHY
                )
                else 0,
            )
    ...
    async def get_health_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                rabbitmq_health["status"] == HealthStatus.UNHEALTHY.value
                or kafka_health["status"] == HealthStatus.UNHEALTHY.value
            ):
                overall_status = HealthStatus.UNHEALTHY
            elif (
                rabbitmq_health["status"] == HealthStatus.DEGRADED.value
                or kafka_health["status"] == HealthStatus.DEGRADED.value
            ):
                overall_status = HealthStatus.DEGRADED

            return {
                "overall_status": overall_status.value,
                "components": {"rabbitmq": rabbitmq_health, "kafka": kafka_health},
                "timestamp": datetime.now().isoformat(),
            }
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        return {
            "rabbitmq": {
                "last_collection": self.rabbitmq_monitor.last_metrics.timestamp.isoformat()
                if self.rabbitmq_monitor.last_metrics
                else None,
                "queue_count": len(self.rabbitmq_monitor.last_metrics.queue_metrics)
                if self.rabbitmq_monitor.last_metrics
                else 0,
                "total_messages": self.rabbitmq_monitor.last_metrics.total_messages
                if self.rabbitmq_monitor.last_metrics
                else 0,
                "health_status": self.rabbitmq_monitor.last_metrics.health_status.value
                if self.rabbitmq_monitor.last_metrics
                else "unknown",
            },
            "kafka": {
                "last_collection": self.kafka_monitor.last_metrics.timestamp.isoformat()
                if self.kafka_monitor.last_metrics
                else None,
                "topic_count": len(self.kafka_monitor.last_metrics.topic_metrics)
                if self.kafka_monitor.last_metrics
                else 0,
                "broker_count": self.kafka_monitor.last_metrics.broker_count
                if self.kafka_monitor.last_metrics
                else 0,
                "health_status": self.kafka_monitor.last_metrics.health_status.value
                if self.kafka_monitor.last_metrics
                else "unknown",
            },
            "monitoring": {
