...
from pyspark.sql.functions import (
    ...
    max as spark_max, min as spark_min, avg, stddev,
    regexp_replace, trim, upper, lower, split,
    current_timestamp, lit, desc, asc, row_number,
...
class EnhancedBronzeProcessor(BaseSparkProcessor):
    ...
    def _read_source_data(self, source_name: str, config: Dict) -> Optional[DataFrame]:
...
class EnhancedSilverProcessor(BaseSparkProcessor):
    ...
    def _read_bronze_data(self) -> Optional[DataFrame]:
...
class EnhancedGoldProcessor(BaseSparkProcessor):
    ...
    def _read_silver_data(self) -> Optional[DataFrame]:
    ...
    def _build_time_series_analytics(self, df: DataFrame) -> Dict[str, Any]:
        ...
        try:
            from pyspark.sql.functions import date_trunc, dayofweek, hour
