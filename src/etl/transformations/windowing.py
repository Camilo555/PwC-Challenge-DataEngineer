...
def _apply_scd2_window_polars(
    df: pl.DataFrame | pl.LazyFrame, partition_key: str, order_key: str
) -> pl.DataFrame | pl.LazyFrame:
    """Apply SCD2 window functions using Polars."""
    return df.with_columns(
        [
            # Previous hash for change detection
            pl.col("attribute_hash")
            .shift(1)
            .over(partition_key, order_by=order_key)
            .alias("prev_hash"),
            # Next valid_from for setting valid_to
            pl.col(order_key)
            .shift(-1)
            .over(partition_key, order_by=order_key)
            .alias("next_valid_from"),
            ...
            # Is current record
            (pl.int_range(pl.len()) == pl.count().over(partition_key) - 1)
            .over(partition_key, order_by=order_key)
            .alias("is_current"),
        ]
    ).with_columns(
        [
            # Set valid_to
            pl.when(pl.col("is_current"))
            .then(None)
            .otherwise(pl.col("next_valid_from"))
            .alias("valid_to")
        ]
    )


def _apply_scd2_window_spark(
    df: SparkDataFrame, partition_key: str, order_key: str
) -> SparkDataFrame:
    ...
    ).withColumn("valid_to", F.when(F.col("is_current"), None).otherwise(F.col("next_valid_from")))


def calculate_running_totals(
    df: Any, partition_cols: list[str], order_col: str, sum_col: str, engine: str = "pandas"
) -> Any:
    """
...
def _calculate_running_totals_pandas(
    df: pd.DataFrame, partition_cols: list[str], order_col: str, sum_col: str
) -> pd.DataFrame:
...
def _calculate_running_totals_polars(
    df: pl.DataFrame | pl.LazyFrame, partition_cols: list[str], order_col: str, sum_col: str
) -> pl.DataFrame | pl.LazyFrame:
    ...
    return df.with_columns(
        [
            pl.col(sum_col)
            .cumsum()
            .over(partition_cols, order_by=order_col)
            .alias(f"{sum_col}_running_total")
        ]
    )


def _calculate_running_totals_spark(
    df: SparkDataFrame, partition_cols: list[str], order_col: str, sum_col: str
) -> SparkDataFrame:
...
def _calculate_lag_lead_pandas(
    df: pd.DataFrame, partition_cols: list[str], order_col: str, value_col: str, lag: int, lead: int
) -> pd.DataFrame:
...
def _calculate_lag_lead_polars(
    ...
    return df.with_columns(
        [
            ...
            .shift(lag)
            .over(partition_cols, order_by=order_col)
            .alias(f"{value_col}_lag_{lag}"),
            pl.col(value_col)
            .shift(-lead)
            .over(partition_cols, order_by=order_col)
            .alias(f"{value_col}_lead_{lead}"),
        ]
    )


def _calculate_lag_lead_spark(
    df: SparkDataFrame,
    partition_cols: list[str],
    order_col: str,
    value_col: str,
...
def calculate_rank_percentile(
    df: Any, partition_cols: list[str], order_col: str, engine: str = "pandas"
) -> Any:
    """
...
def _calculate_rank_percentile_pandas(
    df: pd.DataFrame, partition_cols: list[str], order_col: str
) -> pd.DataFrame:
...
def _calculate_rank_percentile_polars(
    df: pl.DataFrame | pl.LazyFrame, partition_cols: list[str], order_col: str
) -> pl.DataFrame | pl.LazyFrame:
...
def _calculate_rank_percentile_spark(
    df: SparkDataFrame, partition_cols: list[str], order_col: str
) -> SparkDataFrame:
    """Calculate rank and percentile using Spark."""
    window_spec = Window.partitionBy(*partition_cols).orderBy(F.desc(order_col))

    return df.withColumns(
        {"rank": F.dense_rank().over(window_spec), "percentile": F.percent_rank().over(window_spec)}
    )
...
def _detect_data_changes_pandas(
    current_df: pd.DataFrame, incoming_df: pd.DataFrame, key_cols: list[str], track_cols: list[str]
) -> pd.DataFrame:
    ...
    comparison = incoming_df.merge(
        current_df[key_cols + ["current_hash"]], on=key_cols, how="left", suffixes=("", "_current")
    )
    ...
    comparison.loc[
        (comparison["current_hash"].notna()) & comparison["has_changed"], "change_type"
    ] = "update"

    return comparison


def _detect_data_changes_polars(
    ...
    comparison = (
        incoming_with_hash.join(
            current_with_hash.select(key_cols + ["current_hash"]), on=key_cols, how="left"
        )
        .with_columns(
            [
                (
...
def _detect_data_changes_spark(
    current_df: SparkDataFrame,
    incoming_df: SparkDataFrame,
    key_cols: list[str],
    track_cols: list[str],
) -> SparkDataFrame:
    """Detect changes using Spark."""
    # Generate hash for tracked columns
    current_with_hash = current_df.withColumn("current_hash", F.hash(*track_cols))

    incoming_with_hash = incoming_df.withColumn("incoming_hash", F.hash(*track_cols))
