...
class AnomalyDetector:
    ...
    def _detect_seasonality(self, values: list[float]) -> int | None:
    ...
    def detect_anomalies(
        self, metric_name: str, current_value: float, sensitivity: float = 2.5
    ) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            is_iqr_anomaly = (
                current_value < baseline["q25"] - iqr_threshold
                or current_value > baseline["q75"] + iqr_threshold
            )
            detection_methods.append(
                {
                    "method": "iqr",
            ...
            if baseline["sample_count"] > 30:
                ...
                if modified_std > 0:
                    ...
                    is_statistical_anomaly = t_score > stats.norm.ppf(
                        1 - 0.05 / 2
                    )  # 95% confidence
                    detection_methods.append(
                        {
...
class CorrelationEngine:
    ...
    def find_correlated_events(
        self, event: CorrelatedEvent, time_window: timedelta = timedelta(minutes=5)
            ...
            for event in list(self.events):
                ...
                if event.service_name:
                    for other_event in list(self.events):
                        if (
                            other_event.trace_id == event.trace_id
                            and other_event.service_name != event.service_name
                            and abs((other_event.timestamp - event.timestamp).total_seconds()) < 60
                        ):
...
class SLOManager:
    ...
    def record_sli_measurement(
        self, slo_name: str, sli_name: str, value: float, timestamp: datetime | None = None
    ):
    ...
    def calculate_slo_compliance(
        self, slo_name: str, time_window: timedelta | None = None
    ) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            for sli_metric in slo.sli_metrics:
                ...
                recent_measurements = [
                    m for m in self.sli_history[key] if m["timestamp"] >= cutoff_time
                ]
                ...
                compliant_measurements = [
                    m for m in recent_measurements if m["value"] <= sli_metric.target_value
                ]
            ...
            if compliance_results:
                overall_compliance = sum(
                    r["compliance_rate"] for r in compliance_results.values()
                ...
                error_budget_consumed = max(
                    0, (1 - overall_compliance) / (slo.error_budget_percentage / 100)
                )
    ...
    def get_error_budget_status(self, slo_name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "error_budget_consumed_percentage": compliance_result[
...
class BusinessMetricsCollector:
    ...
    def update_kpi_value(self, kpi_name: str, value: float, timestamp: datetime | None = None):
        ...
        with self.lock:
            ...
            self.kpi_history[kpi_name].append(
                {"value": value, "timestamp": timestamp, "trend": kpi.trend}
            )
...
class EnterpriseObservabilityPlatform:
    ...
    def _setup_prometheus_metrics(self):
        ...
        self.prometheus_metrics = {
            ...
            "app_request_duration": Histogram(
                "app_request_duration_seconds", "Application request duration"
            ),
            ...
            # Business metrics
            "business_revenue_daily": Gauge(
                "business_revenue_daily_dollars", "Daily revenue in dollars"
            ),
            "business_active_users": Gauge("business_active_users_total", "Total active users"),
            "business_conversion_rate": Gauge(
                "business_conversion_rate_percent", "Conversion rate percentage"
            ),
            # Data quality metrics
            "data_quality_score": Gauge(
                "data_quality_score_percent", "Data quality score percentage"
            ),
            ...
            "security_compliance_score": Gauge(
                "security_compliance_score_percent", "Security compliance score"
            ),
        }
    ...
    def record_metric(
        ...
        event = CorrelatedEvent(
            ...
            metrics={name: value},
            tags=tags or {},
            attributes={"category": category.value, "metric_name": name, "metric_value": value},
        )
    ...
    def _handle_anomaly_detection(
        self, metric_name: str, value: float, anomaly_result: dict[str, Any], event: CorrelatedEvent
    ):
    ...
    async def trace_operation(
        ...
        try:
            ...
            success_event = CorrelatedEvent(
                ...
                attributes={"operation_phase": "success", "duration_seconds": duration},
            )
...
_platform: EnterpriseObservabilityPlatform | None = None
...
def monitor_business_operation(
    operation_name: str, service_name: str = "", user_id_param: str | None = None
):
