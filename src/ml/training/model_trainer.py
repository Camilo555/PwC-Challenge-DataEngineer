...
class ModelConfig:
    ...
    n_trials: int = 100
    timeout_seconds: int | None = 3600  # 1 hour
    ...
    def get_model(self, **params) -> Any:
        if self.config.problem_type == "classification":
            return cb.CatBoostClassifier(
                random_state=self.config.random_state, verbose=False, **params
            )
        else:
...
class AutoMLTrainer:
    ...
    def __init__(self, config: ModelConfig):
        ...
        if (
            ...
            or self.config.enable_messaging
            or self.config.enable_streaming
        ):
    ...
    async def _cache_training_data(self, X: pd.DataFrame, y: pd.Series) -> str | None:
        ...
        try:
            ...
            data_hash = hashlib.sha256(
                str(X.values.tobytes() + y.values.tobytes()).encode()
            ).hexdigest()
    ...
    async def _get_cached_training_data(
        self, cache_key: str
    ...
    async def _register_model_distributed(self, result: TrainingResult):
        ...
        try:
            ...
            await self.cache_manager.hset(
                "model_registry", result.model_name, registry_entry, namespace="ml_registry"
            )
    ...
    async def _train_models_async(
        ...
        try:
            ...
            await self._publish_training_progress(
                job_id,
                "started",
                {
                    "total_samples": len(X),
            ...
            await self._cache_training_data(X, y) if self.config.cache_training_data else None

            # Feature engineering
            if feature_pipeline:
                logger.info("Applying feature engineering pipeline")
                await self._publish_training_progress(
                    job_id, "feature_engineering", {"status": "started"}
                )
                X = feature_pipeline.fit_transform(X, y)
                await self._publish_training_progress(
                    job_id, "feature_engineering", {"status": "completed"}
                )
            ...
            for i, (model_type, trainer) in enumerate(self.trainers.items()):
                ...
                await self._publish_training_progress(
                    job_id,
                    f"model_training_{model_type}",
                    {"status": "started", "model_index": i + 1, "total_models": len(self.trainers)},
                )
            ...
            if results:
                await self._stream_training_metrics(
                    job_id,
                    "automl",
                    {
            ...
            self.metrics_collector.increment_counter(
                "automl_training_completed_total", tags={"models_trained": str(len(results))}
            )
            self.metrics_collector.record_histogram("automl_training_duration_seconds", duration)
            ...
            self.metrics_collector.increment_counter("automl_training_errors_total")
            raise
    ...
    async def _train_single_model_async(
        ...
        with mlflow.start_run(
            run_name=f"{trainer.model_type}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
        ):
            # Log parameters
            ...
            if self.config.cv_folds > 1:
                ...
                cv_scores = cross_val_score(
                    best_model, X_train, y_train, cv=cv_splitter, scoring=scoring, n_jobs=-1
                )
            ...
            if hasattr(best_model, "feature_importances_"):
                ...
                feature_importance = dict(
                    zip(X_train.columns, np.abs(best_model.coef_.flatten()), strict=False)
                )
