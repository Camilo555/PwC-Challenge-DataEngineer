...
from src.messaging.enterprise_rabbitmq_manager import (
    EnterpriseMessage,
    MessagePriority,
    QueueType,
...
class FeatureDriftDetector(BaseDriftDetector):
    ...
    def _detect_numerical_drift(
        self, ref_data: pd.Series, curr_data: pd.Series, feature_name: str
    ) -> DriftResult:
    ...
    def _detect_categorical_drift(
        ...
        if self.config.statistical_test == "chi2":
            ...
            contingency = []
            for cat in all_categories:
                contingency.append([ref_freq.get(cat, 0), curr_freq.get(cat, 0)])
...
class ModelMonitor:
    ...
    def __init__(self, model_id: str, config: DriftConfig):
        ...
        self.reference_labels: pd.Series | None = None
    ...
    async def _publish_drift_alert(self, drift_result: DriftResult):
        ...
        if (
            not self.rabbitmq_manager
            or not self.config.enable_messaging
            or not self.config.publish_drift_alerts
        ):
            return
        ...
        try:
            ...
            logger.debug(
                f"Drift alert published for model {self.model_id}: {drift_result.drift_type.value}"
            )
    ...
    async def _publish_performance_alert(
        self, metrics: "ModelPerformanceMetrics", alert_type: str, alert_data: dict[str, Any]
    ):
        """Publish performance alert to RabbitMQ."""
        if (
            not self.rabbitmq_manager
            or not self.config.enable_messaging
            or not self.config.publish_performance_alerts
        ):
            return
    ...
    async def _stream_drift_detection(self, drift_result: DriftResult):
        ...
        if (
            not self.kafka_manager
            or not self.config.enable_streaming
            or not self.config.stream_drift_detection
        ):
            return
    ...
    async def _stream_real_time_metrics(self, metrics_data: dict[str, Any]):
        ...
        if (
            not self.kafka_manager
            or not self.config.enable_streaming
            or not self.config.stream_real_time_metrics
        ):
            return
    ...
    async def _cache_model_metrics(self, metrics: "ModelPerformanceMetrics"):
        ...
        if (
            ...
            or not self.config.cache_model_metrics
        ):
            return
        ...
        try:
            ...
            await self.cache_manager.lpush(
                recent_metrics_key, metrics_data, namespace="ml_monitoring"
            )
            await self.cache_manager.ltrim(
                recent_metrics_key, 0, 99, namespace="ml_monitoring"
            )  # Keep last 100
    ...
    async def _cache_drift_data(self, drift_results: list[DriftResult]):
        ...
        if (
            not self.cache_manager
            or not self.config.enable_caching
            or not self.config.cache_drift_data
        ):
            return
        ...
        try:
            ...
            await self.cache_manager.set(
                cache_key, drift_data, ttl=self.config.cache_ttl_seconds, namespace="ml_monitoring"
            )
    ...
    async def _check_performance_degradation(
        self, current_metrics: "ModelPerformanceMetrics", problem_type: str
    ):
        ...
        try:
            ...
            recent_metrics_key = f"model_metrics_recent:{self.model_id}"
            recent_metrics_data = await self.cache_manager.lrange(
                recent_metrics_key, 1, 10, namespace="ml_monitoring"
            )  # Skip current, get last 10
            ...
            if problem_type == "classification":
                ...
                historical_accuracies = [
                    m.get("accuracy") for m in recent_metrics_data if m.get("accuracy") is not None
                ]
                ...
                if current_metrics.accuracy < baseline_accuracy * 0.95:
                    await self._publish_performance_alert(
                        ...
                        "performance_degradation",
                        {
                            ...
                            "degradation_percent": (
                                (baseline_accuracy - current_metrics.accuracy) / baseline_accuracy
                            )
                            * 100,
                        },
                    )
                ...
                historical_r2_scores = [
                    m.get("r2_score") for m in recent_metrics_data if m.get("r2_score") is not None
                ]
                ...
                if current_metrics.r2_score < baseline_r2 * 0.95:
                    await self._publish_performance_alert(
                        ...
                        "performance_degradation",
                        {
                            ...
                            "degradation_percent": (
                                (baseline_r2 - current_metrics.r2_score) / baseline_r2
                            )
                            * 100,
                        },
                    )
    ...
    async def set_reference_data(
        ...
        try:
            ...
            reference_data = {
                ...
                "label_stats": self._calculate_prediction_stats(labels)
                if labels is not None
                else None,
            }
    ...
    async def monitor_batch(
        ...
        try:
            ...
            for feature in features.columns:
                if feature in self.reference_data.columns:
                    drift_result = self.feature_drift_detector.detect_drift(
                        self.reference_data[feature], features[feature], feature
                    )
            ...
            drift_metrics = {
                ...
                "avg_drift_score": sum(r.drift_score for r in drift_results) / len(drift_results)
                if drift_results
                else 0,
                "critical_alerts": sum(
                    1 for r in drift_results if r.severity == AlertSeverity.CRITICAL
                ),
    ...
    async def calculate_performance_metrics(
        self, predictions: pd.Series, labels: pd.Series, problem_type: str = "classification"
    ) -> ModelPerformanceMetrics:
        """Calculate model performance metrics."""

        try:
            metrics = ModelPerformanceMetrics(
                model_id=self.model_id, timestamp=datetime.utcnow(), sample_size=len(predictions)
            )
            ...
            if problem_type == "classification":
                ...
                performance_metrics_data.update(
                    {"rmse": metrics.rmse, "mae": metrics.mae, "r2_score": metrics.r2_score}
                )
            ...
            logger.error(
                f"Error calculating performance metrics for model {self.model_id}: {str(e)}"
            )
            return ModelPerformanceMetrics(
                model_id=self.model_id, timestamp=datetime.utcnow(), sample_size=0
            )
        ...
        try:
            ...
            summary = {
                ...
                "drift_detected_count": sum(
                    1 for r in drift_results if r.get("is_drift_detected", False)
                ),
        ...
        for column in features.columns:
            if pd.api.types.is_numeric_dtype(features[column]):
                ...
                stats[column] = {
                    ...
                    "most_frequent_freq": float(value_counts.iloc[0])
    ...
    async def _load_reference_data(self):
        ...
        try:
            ...
            # Query from database
            result = (
                self.supabase.table("model_reference_data")
                .select("*")
    ...
    async def _process_drift_alerts(self, drift_results: list[DriftResult]):
        ...
        try:
            for result in drift_results:
                if result.is_drift_detected and result.severity in [
                    AlertSeverity.HIGH,
    ...
    async def _update_performance_metrics(
        self, metrics: ModelPerformanceMetrics, problem_type: str
    ):
        ...
        try:
            if problem_type == "classification":
                if metrics.accuracy is not None:
                    self.metrics_collector.record_gauge(
                        "ml_model_accuracy", metrics.accuracy, tags={"model_id": self.model_id}
                    )

                if metrics.f1_score is not None:
                    self.metrics_collector.record_gauge(
                        "ml_model_f1_score", metrics.f1_score, tags={"model_id": self.model_id}
                    )

            else:  # regression
                if metrics.rmse is not None:
                    self.metrics_collector.record_gauge(
                        "ml_model_rmse", metrics.rmse, tags={"model_id": self.model_id}
                    )

                if metrics.r2_score is not None:
                    self.metrics_collector.record_gauge(
                        "ml_model_r2_score", metrics.r2_score, tags={"model_id": self.model_id}
                    )

        except Exception as e:
            logger.error(f"Error updating performance metrics: {str(e)}")

    async def _get_drift_results(
        self, start_date: datetime, end_date: datetime
    ) -> list[dict[str, Any]]:
        """Get drift results from database."""
        try:
            result = (
                self.supabase.table("model_drift_results")
                .select("*")
                .eq("model_id", self.model_id)
    ...
    async def _get_performance_metrics(
        ...
        try:
            result = (
                self.supabase.table("model_performance_metrics")
                .select("*")
                .eq("model_id", self.model_id)
                .gte("timestamp", start_date.isoformat())
                .lte("timestamp", end_date.isoformat())
    ...
    def _calculate_avg_performance(
        ...
        for metric_name in metric_names:
            values = [
                m.get(metric_name) for m in performance_metrics if m.get(metric_name) is not None
            ]
            if values:
    ...
    async def _generate_monitoring_charts(
        self, drift_results: list[dict[str, Any]], performance_metrics: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if performance_metrics:
                ...
                if "accuracy" in df_perf.columns:
                    fig_perf = px.line(
                        df_perf, x="timestamp", y="accuracy", title="Model Accuracy Over Time"
                    )
                elif "rmse" in df_perf.columns:
                    fig_perf = px.line(
                        df_perf, x="timestamp", y="rmse", title="Model RMSE Over Time"
                    )
                else:
                    fig_perf = None
        ...
        try:
            ...
            async with self.observability_platform.trace_operation(
                    ...
                    for feature in features.columns:
                        if feature in self.reference_data.columns:
                            ...
                            drift_result = await self._quick_drift_check(
                                self.reference_data[feature].sample(
                                    min(100, len(self.reference_data))
                                ),
                                features[feature],
                                feature,
                            )
                ...
                self.prometheus_collector.increment_counter(
                    "ml_model_inferences_total", 1, {"model_id": self.model_id, "status": "success"}
                )
                ...
                if predictions is not None:
                    ...
                    self.observability_platform.business_metrics.update_kpi_value(
                        f"ml_model_{self.model_id}_avg_prediction", avg_prediction
                    )
            ...
            self.prometheus_collector.increment_counter(
                "ml_model_inferences_total", 1, {"model_id": self.model_id, "status": "error"}
            )

            return {"error": str(e), "timestamp": datetime.utcnow().isoformat()}

    def _record_inference_metrics(
        self, features: pd.DataFrame, predictions: pd.Series, trace_id: str
    ):
        ...
        self.prometheus_collector.set_gauge(
            "ml_model_prediction_mean", pred_mean, {"model_id": self.model_id}
        )

        self.prometheus_collector.set_gauge(
            "ml_model_prediction_std", pred_std, {"model_id": self.model_id}
        )
    ...
    async def _quick_drift_check(
        self, reference_sample: pd.Series, current_data: pd.Series, feature_name: str
    ) -> DriftResult:
            ...
            dashboard_data = {
                ...
                "drift_summary": {
                    "total_features_monitored": len(
                        {r.get("feature_name") for r in weekly_drift if r.get("feature_name")}
                    ),
                    "drift_detected_24h": len(
                        [r for r in recent_drift if r.get("is_drift_detected")]
                    ),
                    "drift_detected_7d": len(
                        [r for r in weekly_drift if r.get("is_drift_detected")]
                    ),
                    "critical_drift_alerts": len(
                        [r for r in recent_drift if r.get("severity") == "critical"]
                    ),
                ...
                "performance_summary": {
                    ...
                    "last_performance_check": recent_performance[-1]["timestamp"]
                ...
                "recommendations": self._generate_health_recommendations(
                    recent_drift, recent_performance, health_score
                ),
            }
    ...
    def _calculate_model_health_score(
        self, drift_results: list[dict], performance_metrics: list[dict]
    ) -> dict[str, Any]:
    ...
    def _calculate_avg_drift_score(self, drift_results: list[dict]) -> float:
        ...
        scores = [
            r.get("drift_score", 0) for r in drift_results if r.get("drift_score") is not None
        ]
    ...
    def _calculate_performance_trend(self, performance_metrics: list[dict]) -> str:
        ...
        older_avg = (
            sum(values[-6:-3]) / 3
            if len(values) >= 6
            else sum(values[:-3]) / len(values[:-3])
            if len(values) > 3
            else values[0]
        )
    ...
    def _generate_health_recommendations(
        self, drift_results: list[dict], performance_metrics: list[dict], health_score: dict
    ) -> list[str]:
        ...
        if health_score["status"] == "critical":
            recommendations.append(
                "URGENT: Model health is critical. Immediate intervention required."
            )
        ...
        if critical_drifts > 0:
            recommendations.append(
                f"Critical drift detected in {critical_drifts} features. Consider immediate retraining."
            )
        ...
        if performance_metrics:
            ...
            if trend == "declining":
                recommendations.append(
                    "Performance is declining. Review recent data quality and consider model refresh."
                )
            elif trend == "improving":
                recommendations.append(
                    "Performance is improving. Current monitoring strategy is effective."
                )

        # SLO recommendations
        if health_score["performance_score"] < 30:
            recommendations.append(
                "Model performance below acceptable thresholds. Review SLO targets."
            )
...
def get_model_monitor(model_id: str, config: DriftConfig | None = None) -> ModelMonitor:
...
async def initialize_ml_monitoring() -> dict[str, Any]:
    ...
    try:
        ...
        from monitoring.enterprise_observability import SLO, MetricCategory, SLIMetric
...
async def get_ml_monitoring_dashboard() -> dict[str, Any]:
    ...
    try:
        ...
        # Get ML SLO status
        slo_status = observability_platform.slo_manager.calculate_slo_compliance(
            "ml_model_reliability"
        )
        ...
        unhealthy_models = len(
            [
                h
                for h in models_health.values()
                if h.get("health_score", {}).get("status") in ["poor", "critical"]
            ]
        )
        ...
        return {
            ...
            "summary": {
                ...
                "slo_compliance": slo_status.get("overall_compliance_rate", 0) * 100
...
def _generate_global_ml_recommendations(models_health: dict, slo_status: dict) -> list[str]:
    ...
    critical_models = [
        m for m, h in models_health.items() if h.get("health_score", {}).get("status") == "critical"
    ]
    if critical_models:
        recommendations.append(
            f"Critical: {len(critical_models)} models require immediate attention: {', '.join(critical_models)}"
        )

    poor_models = [
        m for m, h in models_health.items() if h.get("health_score", {}).get("status") == "poor"
    ]
    if poor_models:
        recommendations.append(
            f"Warning: {len(poor_models)} models showing poor health: {', '.join(poor_models)}"
        )
    ...
    if "error" not in slo_status:
        ...
        if compliance_rate < 0.95:
            recommendations.append(
                f"SLO compliance at {compliance_rate * 100:.1f}% - below 95% target"
            )
