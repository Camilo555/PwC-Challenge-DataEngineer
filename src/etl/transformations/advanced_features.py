...
class AdvancedFeatureEngineer:
    ...
    def create_temporal_features(
        self, df: pd.DataFrame, date_col: str = "invoice_timestamp"
    ) -> pd.DataFrame:
        ...
        logger.info(
            f"Created {len([col for col in df.columns if col not in [date_col]]) - len(df.columns) + 1} temporal features"
        )
        return df
    ...
    def create_rfm_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        rfm["frequency_score"] = pd.qcut(
            rfm["frequency_orders"].rank(method="first"),
            5,
            labels=[1, 2, 3, 4, 5],
            duplicates="drop",
        )
        rfm["monetary_score"] = pd.qcut(
            rfm["monetary_total"], 5, labels=[1, 2, 3, 4, 5], duplicates="drop"
        )
    ...
    def create_product_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        product_stats["product_revenue"] = (
            product_stats["product_quantity_sum"] * product_stats["product_unit_price_mean"]
        )
        product_stats["product_price_volatility"] = (
            product_stats["product_unit_price_std"] / product_stats["product_unit_price_mean"]
        ).fillna(0)
        ...
        logger.info(
            f"Created product features for {product_stats['stock_code'].nunique()} products"
        )
        return df
    ...
    def create_customer_behavior_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        customer_stats["customer_avg_days_between_orders"] = (
            customer_stats["customer_tenure_days"] / customer_stats["customer_invoice_no_nunique"]
        ).fillna(0)

        # Customer value metrics
        customer_stats["customer_total_spend"] = (
            customer_stats["customer_quantity_sum"] * customer_stats["customer_unit_price_mean"]
        )

        customer_stats["customer_avg_order_value"] = (
            customer_stats["customer_total_spend"] / customer_stats["customer_invoice_no_nunique"]
        )
        ...
        logger.info(
            f"Created behavior features for {customer_stats['customer_id'].nunique()} customers"
        )
        return df
    ...
    def create_market_basket_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        basket_stats = (
            ...
            .agg({"stock_code": ["count", "nunique"], "quantity": "sum", "unit_price": "mean"})
            .reset_index()
        )
        ...
        basket_stats["basket_total_value"] = (
            basket_stats["basket_total_quantity"] * basket_stats["basket_avg_price"]
        )
        ...
        logger.info(
            f"Created market basket features for {basket_stats['invoice_no'].nunique()} baskets"
        )
        return df
    ...
    def create_anomaly_detection_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "unit_price" in df.columns:
            ...
            df["price_outlier"] = (
                (df["unit_price"] < lower_bound) | (df["unit_price"] > upper_bound)
            ).astype(int)
        ...
        if "quantity" in df.columns and "unit_price" in df.columns:
            ...
            df["transaction_value_outlier"] = (
                (df["transaction_value"] < lower_bound) | (df["transaction_value"] > upper_bound)
            ).astype(int)
