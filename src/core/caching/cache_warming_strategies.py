...
from core.caching.redis_streams import publish_cache_invalidation, publish_cache_warming
    ...
    async def warm_cache(self, task: WarmingTask) -> bool:
        ...
        try:
            ...
            if result is not None:
                ...
                success = await cache_manager.set(task.cache_key, result, ttl, task.cache_namespace)
...
class BatchWarmingEngine(BaseWarmingEngine):
    ...
    def __init__(self, cache_manager: RedisCacheManager | None = None, batch_size: int = 50):
        super().__init__(cache_manager)
    ...
    async def _warm_batch_with_loader(
        self, loader_func: Callable, tasks: list[WarmingTask]
    ) -> dict[str, bool]:
...
class CacheWarmingOrchestrator:
    ...
    def __init__(self, cache_manager: RedisCacheManager | None = None):
        ...
        # Priority queues
        self.priority_queues = {priority: deque() for priority in WarmingPriority}
    ...
    async def schedule_warming_task(self, task: WarmingTask) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Scheduled warming task: {task.task_id} (priority: {task.priority.value})"
            )
            return True
        ...
        try:
            ...
            if success:
                ...
                if task.retry_count < task.max_retries:
                    ...
                    self.priority_queues[task.priority].append(task)
                    logger.debug(
                        f"Retrying warming task: {task.task_id} (attempt {task.retry_count})"
                    )
                else:
    ...
    async def _store_task_metadata(self, task: WarmingTask):
        ...
        await cache_manager.set(f"task_meta:{task.task_id}", metadata, 3600, self.namespace)
...
class CacheInvalidationManager:
    ...
    async def invalidate_cache(self, trigger: str, context: dict[str, Any] = None) -> int:
        ...
        try:
            ...
            logger.info(
                f"Invalidation triggered by '{trigger}': {total_invalidated} entries invalidated"
            )
    ...
    def _rule_matches_trigger(
        self, rule: InvalidationRule, trigger: str, context: dict[str, Any] = None
    ) -> bool:
    ...
    async def _execute_invalidation_rule(
        ...
        try:
            for pattern in rule.target_patterns:
                ...
                await publish_cache_invalidation(pattern, source="invalidation_manager")
    ...
    async def get_invalidation_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_invalidations_per_trigger": (
                ...
                if self.stats["invalidations_triggered"] > 0
                else 0
            ),
        }
...
# Global instances
_warming_orchestrator: CacheWarmingOrchestrator | None = None
...
async def warm_cache_key(
    ...
    strategy: WarmingStrategy = WarmingStrategy.IMMEDIATE,
) -> bool:
