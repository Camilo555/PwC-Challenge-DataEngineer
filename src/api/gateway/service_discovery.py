...
from messaging.rabbitmq_manager import MessagePriority, QueueType, RabbitMQManager
...
class ServiceInstance:
    ...
    def health_score(self) -> float:
        ...
        response_time_score = max(
            0.0, 1.0 - (self.avg_response_time / 1000)
        )  # Normalize to 1 second

        # Factor in connection count (lower relative load is better)
        connection_score = (
            1.0 if self.current_connections == 0 else max(0.1, 1.0 / self.current_connections)
        )
...
class ServiceRegistry:
            ...
            for instance in service_instances:
                if instance.id == instance_id:
                    ...
                    if old_status != status:
                        logger.info(
                            f"Service {service_name} instance {instance_id} status changed: {old_status} -> {status}"
                        )
    ...
    async def _publish_service_event(
        self, action: str, instance: ServiceInstance, metadata: dict[str, Any] = None
    ):
...
class ConsistentHashLoadBalancer(LoadBalancer):
    ...
    async def select_instance(
        ...
        if (
            service_name not in self.hash_ring
            or len(self.hash_ring[service_name]) != len(instances) * self.virtual_nodes
        ):
...
class ServiceDiscoveryClient:
    ...
    async def discover_service(
        ...
        if (
            use_cache
            and service_name in self.service_cache
            and current_time - self.last_cache_update.get(service_name, 0) < self.cache_ttl
        ):
...
_default_service_discovery: ServiceDiscoveryClient | None = None
