...
class SensitiveDataDetector:
    ...
    def detect_sensitive_data(self, text: str, context: str = "") -> list[DataDetectionResult]:
        ...
        for _data_type, patterns in self.patterns.items():
    ...
    def _find_pattern_matches(
        self, text: str, pattern: SensitiveDataPattern, context: str
    ...
    def _calculate_confidence(
        self, matched_text: str, context: str, keywords: list[str], base_confidence: float
    ) -> float:
    ...
    def _redact_text(self, text: str, data_type: SensitiveDataType) -> str:
        ...
        if data_type == SensitiveDataType.EMAIL:
            ...
            return (
                f"****-****-****-{clean_num[-4:]}" if len(clean_num) >= 4 else "****-****-****-****"
            )
...
class DLPPolicyEngine:
    ...
    def _initialize_default_policies(self):
        ...
        pci_policy = DLPPolicy(
            ...
            compliance_frameworks=[ComplianceFramework.PCI_DSS],
            applies_to={"locations": ["all"], "operations": ["create", "read", "update", "export"]},
        )
        ...
        hipaa_policy = DLPPolicy(
            ...
            compliance_frameworks=[ComplianceFramework.HIPAA],
            applies_to={"locations": ["all"], "users": ["healthcare_workers", "administrators"]},
        )
    ...
    def evaluate_policies(
        self, detection_results: list[DataDetectionResult], context: dict[str, Any]
    ...
    def _policy_applies(
        self, policy: DLPPolicy, result: DataDetectionResult, context: dict[str, Any]
    ) -> bool:
...
class EnterpriseDLPManager:
    ...
    def scan_data(
        self, data: str | dict[str, Any], context: dict[str, Any] = None
    ) -> dict[str, Any]:
        ...
        detection_results = self.detector.detect_sensitive_data(
            text_data, context.get("description", "")
        )
        ...
        report = {
            ...
            "data": final_data,
            "detections": len(detection_results),
            "sensitive_data_types": list({r.data_type.value for r in detection_results}),
            ...
            "metadata": {"timestamp": start_time.isoformat(), "context": context},
        }
    ...
    def get_incidents(
        ...
        data_type: SensitiveDataType | None = None,
    ...
    def get_dlp_dashboard(self) -> dict[str, Any]:
        ...
        return {
            ...
            "compliance_coverage": list(
                {
                    fw.value
                    for p in self.policy_engine.policies.values()
                    for fw in p.compliance_frameworks
                }
            ),
        }
