...
class MLAPMTracker:
    ...
    def __init__(
        self, service_name: str = "ml-platform", datadog_monitoring: DatadogMonitoring | None = None
    ):
        ...
        self.distributed_tracer = get_distributed_tracer(
            f"{service_name}-ml-tracing", datadog_monitoring
        )
        ...
        self.business_metrics = get_business_metrics_tracker(
            f"{service_name}-ml-business", datadog_monitoring
        )

        self.logger.info(f"Enhanced ML APM tracker initialized for {service_name}")

    @asynccontextmanager
    async def trace_ml_operation(
        self,
        operation_type: str,
        operation_name: str,
        ...
        with tracer.trace(
            ...
            try:
                ...
                self.logger.error(
                    f"ML operation failed: {operation_type}.{operation_name} - {str(e)}"
                )
                raise
                ...
                if self.datadog_monitoring:
                    await self._send_operation_metrics(
                        operation_type,
                        operation_name,
                        duration,
                        model_id,
    ...
    async def _send_operation_metrics(
        ...
        try:
            ...
            self.datadog_monitoring.counter("ml.operations.total", tags=tags)
            ...
            # Track errors
            if has_error:
                self.datadog_monitoring.counter("ml.operations.errors", tags=tags)
    ...
    async def trace_model_training(
        self, model_id: str, model_type: str, training_config: dict[str, Any]
    ):
    ...
    async def trace_data_preparation(
        self, dataset_size: int, features_count: int, preprocessing_steps: list[str]
    ) -> dict[str, Any]:
        ...
        if self.datadog_monitoring:
            self.datadog_monitoring.gauge(
                "ml.training.dataset_size", dataset_size, tags=["component:data_preparation"]
            )
    ...
    async def trace_model_evaluation(
        self, model_id: str, metrics: dict[str, float], test_samples: int
    ) -> dict[str, Any]:
        """Trace model evaluation."""

        add_custom_tags({"ml.model_id": model_id, "ml.test_samples": test_samples})
        ...
        if self.datadog_monitoring:
            ...
            for metric_name, metric_value in metrics.items():
                self.datadog_monitoring.gauge(
                    f"ml.model.evaluation.{metric_name}", metric_value, tags=tags
                )

            self.datadog_monitoring.gauge("ml.evaluation.test_samples", test_samples, tags=tags)

        return {"model_id": model_id, "metrics": metrics, "test_samples": test_samples}
    ...
    async def trace_model_inference(
        ...
        model_id: str,
        model_version: str,
        input_features: pd.DataFrame | dict[str, Any] | list,
    ):
        ...
        if isinstance(input_features, pd.DataFrame):
            ...
            feature_count = (
                ...
                if input_features and isinstance(input_features[0], list | dict)
                else 0
            )
        else:
            batch_size = 1
    ...
    async def trace_inference_preprocessing(
        self, model_id: str, input_size: int, preprocessing_steps: list[str]
    ) -> dict[str, Any]:
        ...
        if self.datadog_monitoring:
            self.datadog_monitoring.histogram(
                "ml.inference.preprocessing.input_size", input_size, tags=[f"model_id:{model_id}"]
            )
    ...
    async def trace_prediction_generation(
        self, model_id: str, prediction_count: int, prediction_confidence: float | None = None
    ) -> dict[str, Any]:
        """Trace prediction generation."""

        self.inference_count += prediction_count

        add_custom_tags({"ml.model_id": model_id, "ml.prediction_count": prediction_count})
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.counter(
                "ml.inference.predictions.total", prediction_count, tags=tags
            )

            if prediction_confidence is not None:
                self.datadog_monitoring.histogram(
                    "ml.inference.prediction_confidence", prediction_confidence, tags=tags
                )
    ...
    async def trace_feature_pipeline(
        self, pipeline_name: str, pipeline_version: str, input_sources: list[str]
    ):
        ...
        return self.trace_ml_operation(
            operation_type="feature_engineering", operation_name=pipeline_name, metadata=metadata
        )

    @trace_function(operation_name="ml.features.extraction", service_name="ml-platform")
    async def trace_feature_extraction(
        self, pipeline_name: str, raw_records: int, extracted_features: int
    ) -> dict[str, Any]:
    ...
    async def trace_feature_transformation(
        self,
        pipeline_name: str,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "ml.features.before_transformation", features_before, tags=tags
            )
            self.datadog_monitoring.gauge(
                "ml.features.after_transformation", features_after, tags=tags
            )
            self.datadog_monitoring.gauge(
                "ml.features.reduction_percent", feature_reduction, tags=tags
            )
    ...
    async def trace_experiment_assignment(
        self, experiment_id: str, user_id: str, assigned_variant: str, assignment_reason: str
    ) -> dict[str, Any]:
    ...
    async def trace_experiment_conversion(
        self,
        experiment_id: str,
        variant: str,
        conversion_type: str,
        ...
        if self.datadog_monitoring:
            ...
            if conversion_value is not None:
                self.datadog_monitoring.histogram(
                    "ml.ab_test.conversion_value", conversion_value, tags=tags
                )
    ...
    async def trace_drift_detection(
        self,
        model_id: str,
        feature_name: str,
        drift_score: float,
    ...
    async def trace_performance_monitoring(
        self,
        model_id: str,
        ...
        add_custom_tags({"ml.model_id": model_id})
        ...
        if self.datadog_monitoring:
            ...
            for metric_name, metric_value in performance_metrics.items():
                self.datadog_monitoring.gauge(
                    f"ml.model.performance.{metric_name}", metric_value, tags=tags
                )

            for degradation_name, degradation_value in degradation_metrics.items():
                self.datadog_monitoring.gauge(
                    f"ml.model.{degradation_name}", degradation_value, tags=tags
                )
    ...
    def get_ml_metrics_summary(self) -> dict[str, Any]:
        ...
        for exp_id, exp_data in self.experiments.items():
            ...
            experiment_stats[exp_id] = {
                ...
                "duration_hours": (datetime.utcnow() - exp_data["start_time"]).total_seconds()
                / 3600,
            }
...
_ml_apm_tracker: MLAPMTracker | None = None


def get_ml_apm_tracker(
    service_name: str = "ml-platform", datadog_monitoring: DatadogMonitoring | None = None
) -> MLAPMTracker:
...
async def trace_model_training_session(
    model_id: str, model_type: str, training_config: dict[str, Any]
):
...
async def trace_feature_pipeline_session(
    pipeline_name: str, pipeline_version: str, input_sources: list[str]
):
...
async def trace_ab_experiment_session(
    experiment_id: str, experiment_name: str, model_a_id: str, model_b_id: str, traffic_split: float
):
    """Convenience function for tracing A/B experiments."""
    tracker = get_ml_apm_tracker()
    return await tracker.trace_ab_experiment(
        experiment_id, experiment_name, model_a_id, model_b_id, traffic_split
    )
