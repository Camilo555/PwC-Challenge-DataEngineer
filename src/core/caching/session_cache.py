...
class RateLimiter:
    ...
    async def is_allowed(
        ...
        # Remove old entries
        pipe.zremrangebyscore(cache_manager._build_key(key, self.namespace), 0, window_start)

        # Count current entries
        pipe.zcard(cache_manager._build_key(key, self.namespace))

        # Add current request
        pipe.zadd(cache_manager._build_key(key, self.namespace), {str(uuid.uuid4()): current_time})
    ...
    async def get_rate_limit_info(
        ...
        await cache_manager.async_redis_client.zremrangebyscore(
            cache_manager._build_key(key, self.namespace), 0, window_start
        )
...
class SessionManager:
    ...
    ):  # Refresh when 50% of TTL remaining
        self.cache_manager = cache_manager
    ...
    async def create_session(
        ...
        await cache_manager.hset(
            session_key, "data", session_data.to_dict(), self.session_namespace
        )
        await cache_manager.expire(session_key, self.session_ttl, self.session_namespace)

        # Add to user's active sessions
        user_sessions_key = f"user_sessions:{user_id}"
        await cache_manager.sadd(user_sessions_key, session_id, self.session_namespace)
        ...
        await cache_manager.sadd(ip_sessions_key, session_id, self.session_namespace)
    ...
    async def invalidate_session(self, session_id: str) -> bool:
        ...
        await cache_manager.async_redis_client.srem(
            cache_manager._build_key(user_sessions_key, self.session_namespace), session_id
        )

        # Remove from IP sessions
        ip_sessions_key = f"ip_sessions:{session_data.ip_address}"
        await cache_manager.async_redis_client.srem(
            cache_manager._build_key(ip_sessions_key, self.session_namespace), session_id
        )
        ...
        await publish_cache_invalidation(
            f"session:{session_id}", self.session_namespace, source="session_manager"
        )
...
class UserDataCache:
    ...
    def __init__(
        self, cache_manager: RedisCacheManager | None = None, profile_ttl: int = 1800
    ):  # 30 minutes
        self.cache_manager = cache_manager
    ...
    async def update_user_preferences(self, user_id: str, preferences: dict[str, Any]) -> bool:
        ...
        await cache_manager.hset(
            profile_key, "preferences", json.dumps(preferences, default=str), self.namespace
        )
    ...
    async def cache_user_activity(
        self, user_id: str, activity: str, metadata: dict[str, Any] | None = None
    ) -> bool:
        ...
        activity_data = {"activity": activity, "timestamp": timestamp, "metadata": metadata or {}}
        ...
        await cache_manager.async_redis_client.zremrangebyrank(
            cache_manager._build_key(activity_key, self.namespace), 0, -101
        )
    ...
    async def get_user_activities(self, user_id: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        activities = await cache_manager.async_redis_client.zrevrange(
            cache_manager._build_key(activity_key, self.namespace), 0, limit - 1, withscores=True
        )
    ...
    async def invalidate_user_cache(self, user_id: str) -> bool:
        ...
        # Publish invalidation event
        await publish_cache_invalidation(
            f"user_data:{user_id}:*", self.namespace, pattern=True, source="user_cache"
        )
...
_user_data_cache: UserDataCache | None = None
