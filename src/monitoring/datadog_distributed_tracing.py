...
class DistributedTrace:
    ...
    correlation_context: TraceContext | None = None
...
class DataDogDistributedTracer:
    ...
    def __init__(
        self,
        service_name: str = "distributed-tracing",
    ...
    def generate_correlation_id(
        self, correlation_level: CorrelationLevel, context: dict[str, Any] | None = None
    ) -> str:
    ...
    def extract_trace_context(self, headers: dict[str, str]) -> TraceContext | None:
    ...
    @asynccontextmanager
    async def trace_distributed_operation(
        self,
        operation_name: str,
        trace_type: TraceType,
        correlation_level: CorrelationLevel,
        ...
        if not correlation_id:
            correlation_id = self.generate_correlation_id(correlation_level, business_context)
        ...
        with tracer.trace(
            ...
            distributed_trace = DistributedTrace(
                ...
                custom_tags={"correlation_id": correlation_id, "operation_name": operation_name},
            ...
            try:
                ...
                if total_completed > 0:
                    ...
                    self.trace_statistics["avg_duration"] = (
                        current_avg * (total_completed - 1) + new_duration
                    ) / total_completed
    ...
    async def _send_distributed_trace_metrics(self, trace: DistributedTrace):
        ...
        try:
            ...
            self.datadog_monitoring.gauge(
                "distributed.traces.services_involved", len(trace.services), tags=tags
            )

            # Send span count
            self.datadog_monitoring.gauge(
                "distributed.traces.spans_count", len(trace.spans), tags=tags
            )

            # Send error metrics
            if trace.error_count > 0:
                self.datadog_monitoring.counter(
                    "distributed.traces.errors", trace.error_count, tags=tags
                )
    ...
    def get_correlated_traces(self, correlation_id: str) -> list[DistributedTrace]:
        ...
        return [
            self.active_traces[trace_id] for trace_id in trace_ids if trace_id in self.active_traces
        ]

    def get_user_journey_traces(
        ...
        for trace in self.active_traces.values():
            if (
                trace.correlation_context
                and trace.correlation_context.user_id == user_id
    ...
    def get_trace_performance_analysis(self) -> dict[str, Any]:
        ...
        successful_correlations = sum(
            1 for traces in self.trace_correlations.values() if len(traces) > 1
        )
        correlation_success_rate = (
            successful_correlations / total_correlations if total_correlations > 0 else 0
        )
        ...
        return {
            ...
            "correlation_analysis": {
                ...
                "avg_traces_per_correlation": sum(
                    len(traces) for traces in self.trace_correlations.values()
                )
                / total_correlations
...
_distributed_tracer: DataDogDistributedTracer | None = None


def get_distributed_tracer(
    service_name: str = "distributed-tracing", datadog_monitoring: DatadogMonitoring | None = None
) -> DataDogDistributedTracer:
...
async def trace_business_workflow(workflow_name: str, workflow_id: str, user_id: str | None = None):
    """Convenience function for tracing business workflows."""
    tracer = get_distributed_tracer()

    business_context = {"workflow_name": workflow_name, "workflow_id": workflow_id}
    if user_id:
        business_context["user_id"] = user_id
...
async def trace_data_pipeline(pipeline_name: str, pipeline_id: str, layer: str):
    ...
    business_context = {"pipeline_name": pipeline_name, "pipeline_id": pipeline_id, "layer": layer}
...
async def trace_ml_operation(operation_name: str, model_id: str, experiment_id: str | None = None):
    """Convenience function for tracing ML operations."""
    tracer = get_distributed_tracer()

    business_context = {"model_id": model_id, "operation_name": operation_name}
    if experiment_id:
...
def propagate_trace_context(
    headers: dict[str, str], custom_context: dict[str, Any] | None = None
) -> dict[str, str]:
...
def extract_trace_context_from_headers(headers: dict[str, str]) -> TraceContext | None:
