...
class QueryBus:
    ...
    async def _get_cached_result(self, query: Query) -> QueryResult | None:
    ...
    async def _store_event(self, event: Event):
        ...
        try:
            ...
            self.kafka_manager.produce_message(
                topic=StreamingTopic.DOMAIN_EVENTS, message=event_data, key=event.aggregate_id
            )
...
class LoggingMiddleware:
    ...
    async def process(self, request: Command | Query):
        """Process request with logging."""
        logger.info(
            f"Processing {type(request).__name__} with ID {getattr(request, 'command_id', getattr(request, 'query_id', 'unknown'))}"
        )
        return request
...
class PerformanceMiddleware:
    ...
    async def process(self, request: Command | Query):
        ...
        if request_type not in self.metrics:
            self.metrics[request_type] = {"count": 0, "total_time": 0, "avg_time": 0}
...
_default_cqrs: CQRSMediator | None = None
