...
class RootCauseAnalysis:
    ...
    metric_analysis_summary: str | None = None
    trace_analysis_summary: str | None = None
...
class DataDogPostIncidentAnalysis:
    ...
    def __init__(
        ...
        incident_management: DataDogIncidentManagement | None = None,
    ):
    ...
    def _initialize_analysis_templates(self):
        ...
        try:
            ...
            self.analysis_templates["root_cause_analysis"] = {
                ...
                "optional_fields": ["contributing_factors", "timeline_factors", "why_analysis"],
                "analysis_steps": [
                    "data_collection",
            ...
            self.analysis_templates["impact_analysis"] = {
                ...
                "customer_factors": ["affected_customers", "customer_complaints", "churn_risk"],
                "technical_factors": [
                    "systems_affected",
                    "downtime_minutes",
    ...
    def _initialize_report_templates(self):
        ...
        try:
            ...
            self.report_templates["compliance_report"] = {
                ...
                "compliance_frameworks": ["SOX", "GDPR", "PCI_DSS", "HIPAA", "SOC2"],
            }
        ...
        try:
            ...
            for incident_id, incident in self.incident_management.incidents.items():
                if (
                    ...
                    and incident_id not in self.post_incident_reports
                    and incident.resolved_at
                    and (datetime.utcnow() - incident.resolved_at).total_seconds() > 3600
    ...
    async def _initiate_post_incident_analysis(self, incident: dict[str, Any]):
        ...
        try:
            ...
            improvements = await self._generate_improvement_actions(
                incident_data, rca, impact_analysis
            )
            for improvement in improvements:
                self.improvement_actions[improvement.action_id] = improvement

            # Create post-incident report
            report = await self._create_post_incident_report(
                incident_data, rca, impact_analysis, improvements
            )
    ...
    async def _perform_impact_analysis(self, incident_data: dict[str, Any]) -> IncidentImpact:
        ...
        try:
            ...
            impact = IncidentImpact(
                ...
                performance_degradation_percent=self._calculate_performance_degradation(
                    incident_data
                ),
            ...
            self.logger.info(
                f"Impact analysis completed for {incident_id}: ${impact.total_financial_impact:.2f} total impact"
            )
    ...
    def _calculate_performance_degradation(self, incident_data: dict[str, Any]) -> float:
        ...
        try:
            ...
            degradation = max(
                0, (response_time - baseline_response_time) / baseline_response_time * 100
            )
    ...
    async def _initiate_root_cause_analysis(
        ...
        try:
            ...
            rca.lessons_learned = await self._generate_lessons_learned(
                incident_data, root_cause_result
            )
            ...
            self.logger.info(
                f"Root cause analysis completed for {incident_id}: {rca.primary_root_cause}"
            )
    ...
    async def _analyze_timeline_patterns(
        self, incident_data: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if len(error_logs) > 5:
                contributing_factors.append(
                    f"Multiple error logs detected ({len(error_logs)} errors)"
                )
    ...
    async def _perform_5_whys_analysis(
        self, incident_data: dict[str, Any], root_cause_result: dict[str, Any]
    ) -> list[str]:
        """Perform 5 Whys analysis"""
                ...
                evidence.append(
                    {
                        ...
                        "value": value,
                        "significance": "high"
                        if self._is_anomalous_metric(metric, value)
                        else "normal",
                    }
                )
    ...
    async def _generate_improvement_actions(
        self, incident_data: dict[str, Any], rca: RootCauseAnalysis, impact: IncidentImpact
    ...
    async def _create_post_incident_report(
        self,
        ...
        try:
            ...
            report = PostIncidentReport(
                ...
                communication_analysis=await self._analyze_communication_effectiveness(
                    incident_data
                ),
                improvement_actions=improvements,
                response_metrics=await self._calculate_response_metrics(incident_data),
                comparison_to_similar_incidents=await self._compare_to_similar_incidents(
                    incident_id, rca
                ),
    ...
    async def _analyze_response_effectiveness(
        ...
        try:
            ...
            incident_details.get("time_to_resolve", 0)
            ...
            if response_actions:
                ...
                if action_success_rate < 0.8:
                    effectiveness["areas_for_improvement"].append(
                        "Improve response action success rate"
                    )
            else:
            ...
            if escalation_count <= 1:
                ...
                effectiveness["areas_for_improvement"].append(
                    "Improve communication and escalation"
                )

            # Overall score
            effectiveness["overall_score"] = int(
                statistics.mean(
                    [
    ...
    async def _analyze_communication_effectiveness(
        ...
        try:
            ...
            communication_events = [
                event
                for event in timeline
                if "notification" in event.get("event_type", "")
                or "escalation" in event.get("event_type", "")
            ]
            ...
            return {
                ...
                "effectiveness_score": max(
                    0, 100 - (escalation_count * 10)
                ),  # Deduct 10 points per escalation
                "recommendations": [
    ...
    async def _compare_to_similar_incidents(
        self, incident_id: str, rca: RootCauseAnalysis
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for existing_rca in self.incident_analyses.values():
                if (
                    ...
                    and existing_rca.root_cause_category == rca.root_cause_category
                ):
            ...
            if not similar_incidents:
                return {
                    ...
                    "patterns": [],
                    "recommendations": [
                        "First incident of this type - establish monitoring for patterns"
                    ],
                }
    ...
    async def _generate_executive_summary(
        self, incident_details: dict[str, Any], impact: IncidentImpact, rca: RootCauseAnalysis
    ) -> str:
        ...
        try:
            ...
            summary_parts.append(
                f"A {severity} incident occurred resulting in {downtime:.0f} minutes of system disruption."
            )

            # Business impact
            if impact.total_financial_impact > 0:
                summary_parts.append(
                    f"The incident resulted in an estimated ${impact.total_financial_impact:,.2f} in total business impact, affecting {impact.affected_customers:,} customers."
                )
            ...
            summary_parts.append(
                f"The incident was acknowledged within {response_time:.0f} minutes and resolved in {resolution_time:.0f} minutes."
            )

            # Key actions
            summary_parts.append(
                "Key improvement actions have been identified and assigned to prevent recurrence and enhance system resilience."
            )
    ...
    async def _extract_key_findings(
        self, rca: RootCauseAnalysis, impact: IncidentImpact
    ) -> list[str]:
        ...
        try:
            ...
            if impact.total_financial_impact > 10000:
                findings.append(
                    f"Significant financial impact of ${impact.total_financial_impact:,.2f}"
                )

            if impact.affected_customers > 1000:
                findings.append(
                    f"Large customer impact with {impact.affected_customers:,} customers affected"
                )
            ...
            if rca.contributing_factors:
                findings.append(
                    f"Multiple contributing factors identified: {', '.join(rca.contributing_factors[:3])}"
                )

            # System findings
            if len(impact.systems_affected) > 3:
                findings.append(
                    f"Multiple systems affected: {len(impact.systems_affected)} systems impacted"
                )
    ...
    async def _assess_compliance_impact(
        self, incident_details: dict[str, Any], impact: IncidentImpact
    ) -> list[str]:
        ...
        try:
            ...
            if impact.data_loss_gb > 0:
                compliance_impacts.extend(
                    ["GDPR - Data loss incident", "SOC2 - Data integrity violation"]
                )
            ...
            if impact.affected_customers > 5000:
                compliance_impacts.append(
                    "Potential GDPR notification requirement - Large customer impact"
                )
            ...
            if incident_details.get("severity") in ["sev1", "critical"]:
                compliance_impacts.append(
                    "SOX - Critical system failure affecting financial reporting"
                )
    ...
    async def _update_improvement_progress(self):
        pass
    ...
    def get_post_incident_report(
        self, incident_id: str, format_type: ReportFormat = ReportFormat.JSON
    ) -> str | None:
        ...
        try:
            ...
            return {
                ...
                "analysis_statistics": {
                    ...
                    "pending_actions": len(
                        [a for a in self.improvement_actions.values() if a.status == "open"]
                    ),
                    "completed_actions": len(
                        [a for a in self.improvement_actions.values() if a.status == "completed"]
                    ),
                },
                "root_cause_distribution": {
                    category.value: len(
                        [
                            rca
                            for rca in self.incident_analyses.values()
                            if rca.root_cause_category == category
                        ]
                    )
                ...
                "improvement_priorities": {
                    priority.value: len(
                        [a for a in self.improvement_actions.values() if a.priority == priority]
                    )
                ...
                "recent_activity": {
                    ...
                    "reports_last_7_days": len(
                        [
                            report
                            for report in self.post_incident_reports.values()
...
# Global instance
_post_incident_analysis: DataDogPostIncidentAnalysis | None = None
