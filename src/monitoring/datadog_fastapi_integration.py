...
from monitoring.datadog_apm_initialization import initialize_datadog_apm, shutdown_datadog_apm
from monitoring.datadog_apm_middleware import DataDogAPMConfig, DataDogAPMMiddleware
...
from monitoring.datadog_error_tracking import capture_exception, get_error_tracker
...
class DataDogFastAPIIntegrator:
    ...
    async def setup_datadog_integration(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"DataDog APM integration completed successfully in {integration_time:.2f}s"
            )

            # Send integration success metric
            if self.datadog_monitoring:
                await self.datadog_monitoring.counter(
                    "fastapi.integration.success", tags=[f"service:{self.service_name}"]
                )
            ...
            if self.datadog_monitoring:
                await self.datadog_monitoring.counter(
                    ...
                    tags=[f"service:{self.service_name}", f"error_type:{type(e).__name__}"],
                )
        ...
        async def apm_metrics():
            ...
            try:
                ...
                if self.business_metrics:
                    metrics_data[
                        "business_metrics"
                    ] = await self.business_metrics.calculate_realtime_kpis()
                ...
                if self.distributed_tracer:
                    metrics_data["tracing_metrics"] = (
                        self.distributed_tracer.get_trace_performance_analysis()
                    )
    ...
    def _setup_lifecycle_hooks(self):
        ...
        async def startup_apm():
            ...
            try:
                ...
                if (
                    self.profiler
                    and getattr(settings, "DD_PROFILING_ENABLED", "false").lower() == "true"
                ):
                ...
                if self.datadog_monitoring:
                    await self.datadog_monitoring.counter(
                        "fastapi.application.startup", tags=[f"service:{self.service_name}"]
                    )
        ...
        async def shutdown_apm():
            ...
            try:
                ...
                if self.datadog_monitoring:
                    await self.datadog_monitoring.counter(
                        "fastapi.application.shutdown", tags=[f"service:{self.service_name}"]
                    )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if self.integrator.distributed_tracer:
                trace_context = self.integrator.distributed_tracer.extract_trace_context(
            ...
            if self.integrator.distributed_tracer and hasattr(request.state, "trace_context"):
                response.headers.update(self.integrator.distributed_tracer.inject_trace_context({}))
            ...
            if self.integrator.error_tracker:
                await self.integrator.error_tracker.capture_error(
                    e, tags={"source": "fastapi_middleware", "endpoint": str(request.url.path)}
                )
...
async def setup_fastapi_datadog_integration(
    app: FastAPI, service_name: str
) -> DataDogFastAPIIntegrator:
...
def create_fastapi_with_datadog(
    service_name: str,
    title: str | None = None,
    description: str | None = None,
    version: str = "1.0.0",
) -> Tuple[FastAPI, DataDogFastAPIIntegrator]:
                ...
                try:
                    ...
                    if integrator and integrator.error_tracker:
                        await integrator.error_tracker.capture_error(
                            e, tags={"endpoint": func.__name__, "operation": op_name}
                        )
...
_fastapi_integrator: DataDogFastAPIIntegrator | None = None


def get_fastapi_integrator() -> DataDogFastAPIIntegrator | None:
