...
from etl.framework.resilience import CircuitBreaker, CircuitBreakerConfig, RetryHandler, RetryConfig
...
class ServiceHealthChecker(HealthChecker):
    ...
    async def check_health(self) -> HealthMetrics:
        ...
        try:
            ...
            if not is_healthy:
                issues.append(f"Service health check failed")
                health_status = SystemHealth.FAILED
...
class FailureAnalyzer:
    ...
    def analyze_failure(self, health_metrics: HealthMetrics, component: str) -> Optional[RecoveryPlan]:
