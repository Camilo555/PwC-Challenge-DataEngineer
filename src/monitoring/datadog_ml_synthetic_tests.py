...
class DataDogMLSyntheticTests:
    ...
    def __init__(
        ...
        base_url: str,
        ml_service_prefix: str = "ml-analytics",
    ):
    ...
    def _get_ml_model_configs(self) -> list[MLModelConfig]:
        ...
        return [
                ...
                expected_outputs=[
                    {
                        "segment": "premium",
                        "confidence": 0.92,
                        "segment_probabilities": {"premium": 0.92, "standard": 0.07, "basic": 0.01},
                    }
                ],
            ...
            MLModelConfig(
                ...
                expected_outputs=[
                    {"fraud_probability": 0.15, "risk_level": "low", "action_required": False}
                ],
    ...
    async def deploy_model_health_tests(
        self,
        locations: list[TestLocation] = None,
        frequency: TestFrequency = TestFrequency.EVERY_5_MINUTES,
    ) -> dict[str, str]:
        ...
        try:
            for model_config in self.ml_models:
                ...
                test_config = SyntheticTestConfig(
                    ...
                    if model_config.business_critical
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"ML Model Health - {model_config.name}", health_steps, test_config
                )
                ...
                self.logger.info(
                    f"Deployed ML model health test for {model_config.name}: {test_id}"
                )
    ...
    async def deploy_feature_pipeline_tests(
        self,
        locations: list[TestLocation] = None,
        ...
        try:
            for pipeline_config in self.feature_pipelines:
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"Feature Pipeline - {pipeline_config.name}", pipeline_steps, test_config
                )
                ...
                self.logger.info(
                    f"Deployed feature pipeline test for {pipeline_config.name}: {test_id}"
                )
                await asyncio.sleep(1)
    ...
    async def deploy_inference_accuracy_tests(
        self,
        locations: list[TestLocation] = None,
        frequency: TestFrequency = TestFrequency.EVERY_30_MINUTES,
    ) -> dict[str, str]:
        ...
        try:
            for model_config in self.ml_models:
                ...
                for i, (sample_input, _expected_output) in enumerate(
                    zip(model_config.sample_inputs, model_config.expected_outputs, strict=False)
                ):
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"ML Accuracy - {model_config.name}", accuracy_steps, test_config
                )

                deployed_accuracy_tests[f"{model_config.name}_accuracy"] = test_id

                self.logger.info(
                    f"Deployed inference accuracy test for {model_config.name}: {test_id}"
                )
    ...
    async def deploy_ab_test_integrity_tests(
        self, locations: list[TestLocation] = None, frequency: TestFrequency = TestFrequency.HOURLY
    ) -> dict[str, str]:
        ...
        try:
            for ab_config in self.ab_test_configs:
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"A/B Test - {ab_config.name}", ab_steps, test_config
                )
                ...
                self.logger.info(
                    f"Deployed A/B test integrity test for {ab_config.name}: {test_id}"
                )
                await asyncio.sleep(1)
    ...
    def _create_schema_assertions(self, schema: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for field, rules in schema.items():
            ...
            assertions.append({"type": "jsonpath", "path": f"$.{field}", "operator": "exists"})
    ...
    async def get_ml_test_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            for model_name, test_id in self.model_tests.items():
                ...
                if model_results:
                    model_success_rate = (
                        len([r for r in model_results if r.status == "passed"])
                        / len(model_results)
                        * 100
                    )
                    model_avg_time = sum(r.response_time for r in model_results) / len(
                        model_results
                    )
                    ...
                    model_performance[model_name] = {
                        ...
                        "sla_compliance": model_avg_time
                        <= next(
                            (m.latency_threshold for m in self.ml_models if m.name == model_name),
                            5000,
                        ),
                    }
            ...
            return {
                ...
                "summary": {
                    "total_active_tests": len(self.model_tests)
                    + len(self.feature_tests)
                    + len(self.ab_tests),
                ...
                "ml_pipeline_health": {
                    ...
                    "model_types_covered": list({m.model_type.value for m in self.ml_models}),
