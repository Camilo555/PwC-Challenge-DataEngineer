...
class SecurityTestRequest(BaseModel):
    ...
    test_categories: list[str] = Field(
        default=["authentication", "authorization"], description="Test categories to run"
    )
    intensity: str = Field(default="medium", regex="^(low|medium|high|aggressive)$")
    compliance_standards: list[str] = Field(
        default=["owasp_top10"], description="Compliance standards to validate against"
    )
    include_destructive_tests: bool = Field(
        default=False, description="Include potentially destructive tests"
    )
...
class SecurityTestingFramework:
    ...
    async def run_comprehensive_security_assessment(
        self, request: SecurityTestRequest, user_context: dict[str, Any] = None
        ...
        try:
            ...
            for category in request.test_categories:
                category_tests = await self._run_category_tests(category, request, user_context)
            ...
            if request.compliance_standards:
                compliance_tests = await self._run_compliance_validation(
                    request.compliance_standards, request, user_context
                )
            ...
            report.risk_level = self._determine_risk_level(
                report.overall_score, report.test_results
            )
            ...
            self.logger.info(
                f"Security assessment completed: {assessment_id}, Score: {report.overall_score:.2f}"
            )
    ...
    async def run_penetration_tests(
        self, request: PenetrationTestRequest, user_context: dict[str, Any] = None
    ) -> dict[str, Any]:
            ...
            async with httpx.AsyncClient(
                ...
                for endpoint in request.target_endpoints:
                    endpoint_results = await self._run_endpoint_penetration_tests(
                        client, endpoint, request.test_types, request.depth, rate_limiter
                    )

                    results["vulnerabilities"].extend(endpoint_results.get("vulnerabilities", []))
                    results["successful_attacks"].extend(
                        endpoint_results.get("successful_attacks", [])
                    )
    ...
    async def validate_compliance_controls(
        self, request: ComplianceValidationRequest, user_context: dict[str, Any] = None
    ) -> dict[str, Any]:
    ...
    async def _run_category_tests(
        self, category: str, request: SecurityTestRequest, user_context: dict[str, Any]
    ...
    async def _test_authentication_security(
        self, request: SecurityTestRequest, user_context: dict[str, Any]
    ) -> list[SecurityTestResult]:
    ...
    async def _test_input_validation(
        ...
        if blocked_payloads == len(sqli_payloads):
            ...
            sqli_test.recommendations.append(
    ...
    async def _run_compliance_validation(
        self, standards: list[str], request: SecurityTestRequest, user_context: dict[str, Any]
    ) -> list[SecurityTestResult]:
    ...
    def _determine_risk_level(
        self, security_score: float, test_results: list[SecurityTestResult]
    ) -> str:
        """Determine overall risk level"""

        # Count critical/high severity failures
        critical_failures = sum(
            1 for test in test_results if not test.passed and test.severity == TestSeverity.CRITICAL
        )
        high_failures = sum(
            1 for test in test_results if not test.passed and test.severity == TestSeverity.HIGH
        )
    ...
    def _generate_security_recommendations(
        ...
        failed_categories = {test.category for test in test_results if not test.passed}
    ...
    def _load_auth_bypass_payloads(self) -> list[str]:
        ...
        return ["../admin", "/admin", "?admin=true", "&admin=1", "jwt=null"]
    ...
    async def _run_endpoint_penetration_tests(
        ...
        results = {"endpoint": endpoint, "vulnerabilities": [], "successful_attacks": []}
        ...
        for payload in self.injection_payloads:
            try:
                ...
                if any(
                    ...
                    for indicator in ["sql", "error", "exception", "mysql"]
                ):
    ...
    def _assess_penetration_test_risk(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        critical_vulns = sum(
            1 for v in results["vulnerabilities"] if v.get("severity") == "CRITICAL"
        )
    ...
    def _generate_pentest_recommendations(self, results: dict[str, Any]) -> list[str]:
        ...
        vuln_types = {v.get("type") for v in results["vulnerabilities"]}
    ...
    async def _validate_compliance_standard(
        self, standard: str, request: ComplianceValidationRequest, user_context: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _validate_pci_dss_full_compliance(self) -> dict[str, Any]:
        ...
        return {
            ...
            "critical_issues": [],
            "recommendations": ["Regular security testing", "Enhanced network segmentation"],
        }
    ...
    async def _validate_security_policies(self, scope: str) -> list[dict[str, Any]]:
        ...
        return [
            {"policy": "Password Policy", "compliant": True, "issues": []},
            {
    ...
    async def _analyze_data_flow_compliance(self, standards: list[str]) -> list[dict[str, Any]]:
        ...
        return [
            {"data_flow": "User Registration", "compliance_issues": [], "risk_level": "low"},
            {
...
_testing_framework: SecurityTestingFramework | None = None
...
async def run_security_assessment(
    ...
    current_user: dict[str, Any] = Depends(
        lambda: {"user_id": "system", "permissions": ["security_test"]}
    ),
) -> dict[str, Any]:
    """Run comprehensive security assessment"""
...
async def validate_compliance(
    ...
    current_user: dict[str, Any] = Depends(
        lambda: {"user_id": "system", "permissions": ["compliance_test"]}
    ),
) -> dict[str, Any]:
    """Validate compliance controls and policies"""
