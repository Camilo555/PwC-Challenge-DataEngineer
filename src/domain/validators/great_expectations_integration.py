...
class GreatExpectationsManager:
        ...
        try:
            ...
            try:
                ...
                self.context = gx.get_context(
                    context_root_dir=self.context_root_dir, cloud_mode=False
                )
    ...
    def _load_existing_assets(self):
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.data_sources)} data sources and {len(self.expectation_suites)} expectation suites"
            )
        except Exception as e:
    ...
    def create_expectation_suite(
        self, suite_name: str, overwrite: bool = False
    ) -> ExpectationSuite:
        ...
        try:
            ...
            for template in profile.expectation_templates:
                expectation_config = ExpectationConfiguration(
                    ...
                    kwargs={"column": template.column, **template.kwargs},
                    meta={
            ...
            self.logger.info(
                f"Added {len(profile.expectation_templates)} expectations to suite {suite_name}"
            )
    ...
    def create_common_expectations(
        self, suite_name: str, df: pd.DataFrame, columns_config: dict[str, dict] = None
    ):
        ...
        try:
                    ...
                    if col_config.get("min_value") is not None:
                        suite.add_expectation(
                            ExpectationConfiguration(
                                ...
                                meta={
                                    "severity": "high",
                                    "description": f"Values in {column} should be within valid range",
                                },
                            )
                        )
                    ...
                    if col_config.get("max_length"):
                        suite.add_expectation(
                            ExpectationConfiguration(
                                ...
                                meta={
                                    "severity": "medium",
                                    "description": f"String length in {column} should be within limits",
                                },
                            )
                        )
                    ...
                    if col_config.get("regex_pattern"):
                        suite.add_expectation(
                            ExpectationConfiguration(
                                ...
                                kwargs={"column": column, "regex": col_config["regex_pattern"]},
                                meta={
                                    "severity": "medium",
                                    "description": f"Values in {column} should match expected pattern",
                                },
                            )
                        )
                ...
                if col_config.get("unique", False):
                    suite.add_expectation(
                        ExpectationConfiguration(
                            ...
                            meta={
                                "severity": "high",
                                "description": f"Values in {column} should be unique",
                            },
                        )
                    )
                ...
                if col_config.get("allowed_values"):
                    suite.add_expectation(
                        ExpectationConfiguration(
                            ...
                            kwargs={"column": column, "value_set": col_config["allowed_values"]},
                            meta={
                                "severity": "high",
                                "description": f"Values in {column} should be from allowed set",
                            },
                        )
                    )
            ...
            self.logger.info(
                f"Created {len(suite.expectations)} common expectations for suite {suite_name}"
            )
    ...
    def validate_dataframe(
        self, df: pd.DataFrame, suite_name: str, datasource_name: str = "pandas_datasource"
    ) -> DataQualityReport:
    ...
    def _process_validation_results(
        self, validation_result: Any, dataset_name: str, df: pd.DataFrame
    ) -> DataQualityReport:
        ...
        try:
            ...
            if report.total_expectations > 0:
                report.success_percentage = (
                    report.successful_expectations / report.total_expectations
                ) * 100
        ...
        try:
            ...
            if report.success_percentage < 50:
                recommendations.append(
                    "Critical: Data quality is very poor. Consider data source investigation."
                )
            elif report.success_percentage < 80:
                recommendations.append(
                    "Warning: Data quality needs improvement. Review failed expectations."
                )
            ...
            if critical_failures > 0:
                recommendations.append(
                    f"Urgent: Address {critical_failures} critical data quality issues immediately."
                )
            ...
            if problematic_columns:
                recommendations.append(
                    f"Focus on columns with multiple issues: {', '.join(problematic_columns[:5])}"
                )
            ...
            if uniqueness_issues:
                recommendations.append(
                    "Review duplicate records and implement deduplication process."
                )
    ...
    customer_profile = DataQualityProfile(
        ...
        expectation_templates=[
            ...
            ExpectationTemplate(
                ...
                column="country",
                kwargs={
                    "value_set": ["United States", "United Kingdom", "Germany", "France", "Canada"]
                },
                severity=ExpectationSeverity.MEDIUM,
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            # Validate DataFrame
            print("\nRunning validation...")
            report = gx_manager.validate_dataframe(df=sample_data, suite_name=suite_name)
            ...
            for profile in profiles:
                print(
                    f"- Profile: {profile.profile_name} ({len(profile.expectation_templates)} expectations)"
                )
