...
from collections.abc import Callable as AsyncCallable
        ...
        try:
            message = EnterpriseMessage(
                ...
                metadata=MessageMetadata(
                    priority=priority, headers={"pattern": "pub_sub", "event_type": event_type}
                ),
            )
    ...
    async def publish_async(
        ...
        try:
            ...
            return await self.rabbitmq.publish_message_async(
                message=message, exchange_name=self.exchange_name, routing_key=""
            )
        ...
        def consume_messages():
            try:
                with self.rabbitmq.connection_pool.get_connection() as connection:
                    ...
                    self.logger.info(
                        f"Started subscriber {subscriber_id} for events: {event_types}"
                    )
    ...
    async def call_async(
        ...
        try:
            ...
            response_q = await channel.declare_queue(
                response_queue, auto_delete=True, arguments={"x-message-ttl": timeout * 1000}
            )
            ...
            try:
                with self.rabbitmq.connection_pool.get_connection() as connection:
                    ...
                    channel.basic_consume(
                        queue=self.request_queue, on_message_callback=handle_request
                    )
...
class WorkQueue(MessagePattern):
    ...
    def add_task(
        ...
        try:
            ...
            success = self.rabbitmq.publish_message(
                message=message, exchange_name=exchange_name, routing_key=routing_key
            )
    ...
    def start_workers(self, num_workers: int = 3, queue_name: str | None = None):
        ...
        for i in range(num_workers):
            worker = threading.Thread(
                target=self._worker_process, args=(f"worker_{i + 1}", queue_name), daemon=True
            )
            worker.start()
                ...
                def process_task(ch, method, properties, body):
                    try:
                        ...
                        if task_type in self.task_handlers:
                            start_time = time.time()
                            try:
                                self.task_handlers[task_type](task_data.get("data", {}))
                                execution_time = (time.time() - start_time) * 1000
    ...
    def publish(
        ...
        try:
            message = EnterpriseMessage(
                ...
                payload=data,
                metadata=MessageMetadata(
                    priority=priority, headers={"pattern": "topic", "routing_key": routing_key}
                ),
            )

            return self.rabbitmq.publish_message(
                message=message, exchange_name=self.exchange_name, routing_key=routing_key
            )
            ...
            try:
                with self.rabbitmq.connection_pool.get_connection() as connection:
                    ...
                    for pattern in routing_patterns:
                        channel.queue_bind(
                            exchange=self.exchange_name, queue=queue_name, routing_key=pattern
                        )
...
class MessagePatternFactory:
    ...
    def create_publisher_subscriber(
        rabbitmq_manager: EnterpriseRabbitMQManager | None = None,
    ) -> PublisherSubscriber:
    ...
    @staticmethod
    def create_work_queue(rabbitmq_manager: EnterpriseRabbitMQManager | None = None) -> WorkQueue:
