...
class DataFrameOperations(ABC):
    ...
    @abstractmethod
    def write_parquet(
        self, df: Any, path: str, partition_cols: list[str] | None = None, mode: str = "overwrite"
    ) -> None:
    ...
    @abstractmethod
    def create_scd2_records(
        self, df: Any, business_key_cols: list[str], hash_col: str, effective_date_col: str
    ) -> Any:
    ...
    @abstractmethod
    def merge_delta(
        self, target_path: str, source_df: Any, merge_keys: list[str], update_cols: list[str]
    ) -> None:
...
class PolarsEngine(DataFrameOperations):
    ...
    def filter(
        self, df: pl.DataFrame | pl.LazyFrame, condition: pl.Expr
    ) -> pl.DataFrame | pl.LazyFrame:
    ...
    def groupby_agg(
        self, df: pl.DataFrame | pl.LazyFrame, by: list[str], aggs: dict[str, str]
    ) -> pl.DataFrame | pl.LazyFrame:
    ...
    def with_columns(
        self, df: pl.DataFrame | pl.LazyFrame, expressions: dict[str, pl.Expr]
    ) -> pl.DataFrame | pl.LazyFrame:
        """Add or modify columns using Polars expressions."""
        return df.with_columns([expr.alias(name) for name, expr in expressions.items()])

    def window_dedup_latest(
        self, df: pl.DataFrame | pl.LazyFrame, partition_cols: list[str], order_col: str
    ) -> pl.DataFrame | pl.LazyFrame:
        """Deduplicate keeping latest record per partition."""
        return (
            df.with_columns(
    ...
    def _build_partition_path(
        self, base_path: str, partition_cols: list[str], values: tuple
    ) -> str:
    ...
    def _custom_merge(
        self, target: pl.DataFrame, source: pl.DataFrame, keys: list[str], update_cols: list[str]
    ) -> pl.DataFrame:
...
class SparkEngine(DataFrameOperations):
    ...
    def join(
        self, left: SparkDataFrame, right: SparkDataFrame, on: str | list[str], how: str = "inner"
    ) -> SparkDataFrame:
    ...
    def groupby_agg(
        self, df: SparkDataFrame, by: list[str], aggs: dict[str, str]
    ) -> SparkDataFrame:
        """Group by and aggregate with named outputs."""
        agg_exprs = []
        for col, func in aggs.items():
    ...
    def window_dedup_latest(
        self, df: SparkDataFrame, partition_cols: list[str], order_col: str
    ) -> SparkDataFrame:
        """Deduplicate keeping latest record per partition."""
    ...
    def create_scd2_records(
        ...
        return (
            df.withColumn("end_date", F.lit(date(9999, 12, 31)))
...
def get_dataframe_engine(
    ...
    engine_config = EngineConfig(engine_type=engine_enum, **(config or {}))
