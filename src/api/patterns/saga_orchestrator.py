...
from messaging.rabbitmq_manager import MessagePriority, RabbitMQManager, TaskMessage
...
class SagaOrchestrator:
        ...
        while True:
            try:
                pending_sagas = [
                    ...
                    if saga.status
                    in [SagaStatus.PENDING, SagaStatus.IN_PROGRESS, SagaStatus.COMPENSATING]
                ]
    ...
    async def _execute_compensation_steps(self, saga: SagaTransaction):
        ...
        completed_steps = [
            step for step in reversed(saga.steps) if step.status == StepStatus.COMPLETED
        ]
        ...
        for step in completed_steps:
            if step.status != StepStatus.COMPENSATED:
                ...
                if step.status == StepStatus.FAILED:
                    ...
                    logger.error(
                        f"Compensation failed for step {step.step_id} in SAGA {saga.saga_id}"
                    )
                    await self._publish_saga_event(
                        saga, "compensation_failed", {"step_id": step.step_id, "error": step.error}
                    )
                    return
    ...
    async def _execute_step(self, saga: SagaTransaction, step: SagaStep):
        ...
        try:
            if handler_key in self.step_handlers:
                ...
                await self._publish_saga_event(
                    saga, "step_completed", {"step_id": step.step_id, "result": result}
                )
            else:
            ...
            await self._publish_saga_event(
                saga, "step_failed", {"step_id": step.step_id, "error": str(e)}
            )
    ...
    async def _compensate_step(self, saga: SagaTransaction, step: SagaStep):
        ...
        try:
            if handler_key in self.compensation_handlers:
                ...
                await self._publish_saga_event(
                    saga, "step_compensated", {"step_id": step.step_id, "result": result}
                )
            else:
    ...
    async def _send_step_message(self, saga: SagaTransaction, step: SagaStep):
        ...
        await self.rabbitmq_manager.publish_task_async(
            message, queue_name=f"saga_{step.service_name}", routing_key=step.action
        )
    ...
    async def _send_compensation_message(self, saga: SagaTransaction, step: SagaStep):
        ...
        await self.rabbitmq_manager.publish_task_async(
            message, queue_name=f"saga_{step.service_name}", routing_key=step.compensation_action
        )
    ...
    async def handle_step_response(
        ...
        if success:
            ...
            await self._publish_saga_event(
                saga, "step_completed", {"step_id": step_id, "result": result}
            )
        else:
            step.status = StepStatus.FAILED
            step.error = error
            ...
            await self._publish_saga_event(
                saga, "step_failed", {"step_id": step_id, "error": error}
            )
    ...
    async def _publish_saga_event(
        self, saga: SagaTransaction, event_type: str, extra_data: dict[str, Any] = None
    ):
    ...
    def create_order_saga(order_data: dict[str, Any]) -> SagaTransaction:
        ...
        steps = [
            SagaStep(
                ...
                payload={"order_id": order_data["order_id"], "items": order_data["items"]},
            ),
            SagaStep(
            ...
            SagaStep(
                ...
                payload={"order_id": order_data["order_id"], "status": "confirmed"},
            ),
        ]
