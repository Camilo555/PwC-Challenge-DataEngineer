...
class NotificationProvider(ABC):
    ...
    async def send_notification(
        self, recipient: str, subject: str, message: str, severity: AlertSeverity
    ) -> bool:
        """Send notification"""
        pass
...
class EmailNotificationProvider(NotificationProvider):
    ...
    def __init__(
        self, smtp_server: str, smtp_port: int, username: str, password: str, use_tls: bool = True
    ):
    ...
    async def send_notification(
        ...
        try:
            ...
            response = self.client.chat_postMessage(
                channel=channel, text=f"*{severity.value.upper()} Alert*", attachments=attachments
            )
...
Please investigate when possible.
    ...
    async def _is_in_cooldown(self, rule: AlertRule, component_id: str) -> bool:
        ...
        for alert_context in self.active_alerts.values():
            if (
                alert_context.rule.id == rule.id
                and alert_context.alert.component_id == component_id
                and alert_context.state not in [AlertState.RESOLVED]
            ):
    ...
    async def _should_suppress_alert(self, alert: Alert) -> bool:
        ...
        try:
            ...
            for alert_context in self.active_alerts.values():
                if (
                    alert_context.alert.component_type == alert.component_type
                    and alert_context.alert.severity == alert.severity
                    and alert_context.created_at > cutoff_time
                ):
                    similar_alerts += 1
            ...
            for _rule_id, rule_config in self.suppression_rules.items():
        ...
        try:
            ...
            if not template:
                ...
                message = template.body_template.format(
                    ...
                    availability_percent=alert_context.context_data.get(
            ...
            for channel in escalation.channels:
                ...
                for recipient in escalation.recipients:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to send notification to {recipient} via {channel.value}: {e}"
                        )
            ...
            self.logger.info(
                f"Sent {success_count} notifications for alert: {alert_context.alert.title}"
            )
    ...
    async def _process_auto_resolution(self):
        ...
        for alert_id, alert_context in list(self.active_alerts.items()):
            if (
                alert_context.alert.severity != AlertSeverity.CRITICAL
                and alert_context.state not in [AlertState.RESOLVED]
                and (current_time - alert_context.created_at).total_seconds()
    ...
    async def _cleanup_old_alerts(self):
        ...
        for alert_id, alert_context in self.active_alerts.items():
            if (
                ...
                and alert_context.resolved_at
                and (current_time - alert_context.resolved_at).total_seconds() > cleanup_threshold
            ):
    ...
    async def resolve_alert(
        self, alert_id: str, resolved_by: str = "system", auto_resolved: bool = False
    ) -> bool:
        """Resolve an alert"""
        if alert_id not in self.active_alerts:
            return False
    ...
    def get_active_alerts(
        self, component_id: str = None, severity: AlertSeverity = None
    ) -> list[dict[str, Any]]:
    ...
    def get_alert_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            "total_active": len(
                [a for a in self.active_alerts.values() if a.state != AlertState.RESOLVED]
            ),
    ...
    try:
        ...
        if hasattr(settings, "webhook_url"):
            webhook_provider = WebhookNotificationProvider(
                webhook_url=settings.webhook_url, headers=getattr(settings, "webhook_headers", {})
            )
