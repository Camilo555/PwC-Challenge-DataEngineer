        ...
        try:
            ...
            if health_status == 'unhealthy':
                statsd.event(
                    ...
                    text="Container health check is failing",
                    alert_type='error',
                    tags=tags
                )
        ...
        try:
            ...
            for tool, results in vulnerabilities.items():
                if isinstance(results, dict) and 'total' in results:
                    ...
                    statsd.gauge('docker.security.vulnerabilities.critical',
                               results.get('critical', 0), tags=tags + [f'scanner:{tool}'])
                    statsd.gauge('docker.security.vulnerabilities.high',
                               results.get('high', 0), tags=tags + [f'scanner:{tool}'])
                    statsd.gauge('docker.security.vulnerabilities.medium',
                               results.get('medium', 0), tags=tags + [f'scanner:{tool}'])
                    statsd.gauge('docker.security.vulnerabilities.low',
                               results.get('low', 0), tags=tags + [f'scanner:{tool}'])
                    statsd.gauge('docker.security.vulnerabilities.total',
    ...
    async def run_monitoring_loop(self):
        ...
        while True:
            try:
                ...
                await self.monitor_docker_daemon()
