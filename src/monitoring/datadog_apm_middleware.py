...
class DataDogAPMConfig:
    ...
    def __init__(self):
        ...
        self.enable_runtime_metrics = (
            getattr(settings, "DD_RUNTIME_METRICS_ENABLED", "true").lower() == "true"
        )
        self.enable_logs_injection = (
            getattr(settings, "DD_LOGS_INJECTION", "true").lower() == "true"
        )

        # Sampling settings
        self.sample_rate = float(getattr(settings, "DD_TRACE_SAMPLE_RATE", 1.0))
        self.analytics_enabled = (
            getattr(settings, "DD_TRACE_ANALYTICS_ENABLED", "true").lower() == "true"
        )
...
class DataDogAPMMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, config: DataDogAPMConfig | None = None):
        super().__init__(app)
        self.config = config or DataDogAPMConfig()
        self.datadog_monitoring: DatadogMonitoring | None = None
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        HTTPPropagator.extract(request.headers)
        ...
        with tracer.trace(
            ...
            try:
                ...
                with tracer.trace(
                    "business.process_request", service=self.config.service_name
                ) as business_span:
                    business_span.set_tag(
                        "business.component", self._identify_business_component(request.url.path)
                    )
                    business_span.set_tag(
                        "business.operation",
                        self._identify_business_operation(request.method, request.url.path),
                    )
                ...
                logger.error(
                    f"Request processing error: {error_message}",
                    extra={
    ...
    async def _extract_user_info(self, request: Request) -> dict[str, Any] | None:
    ...
    async def _send_request_metrics(
        self, request: Request, response: Response, response_time_ms: float, user_info: dict | None
    ):
        ...
        try:
            ...
            if user_info:
                ...
                if user_info.get("user_id"):
                    tags.append(
                        f"user_type:{'api' if user_info['auth_method'] == 'api_key' else 'user'}"
                    )
            ...
            if self.config.enable_business_metrics:
                ...
                self.datadog_monitoring.counter(
                    "business.component.requests", tags=tags + [f"business_component:{component}"]
                )
                ...
                if response.status_code >= 400:
                    self.datadog_monitoring.counter(
                        "business.component.errors", tags=tags + [f"business_component:{component}"]
                    )

                # Track user activity
                if user_info and user_info.get("user_id"):
                    self.datadog_monitoring.counter(
                        "business.user.activity", tags=tags + [f"user_id:{user_info['user_id']}"]
                    )
...
def setup_datadog_apm_middleware(app: FastAPI, config: DataDogAPMConfig | None = None):
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            with DataDogTraceContext(
                operation_name=op_name, service_name=service_name, resource_name=res_name, tags=tags
            ) as span:
                try:
...
def add_custom_metric(name: str, value: float, tags: dict[str, str] | None = None):
