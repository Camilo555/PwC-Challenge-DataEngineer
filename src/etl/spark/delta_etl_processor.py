...
from pyspark.sql.functions import (
    ...
    current_timestamp, to_timestamp, date_format, year, month, dayofmonth,
    sum as spark_sum, count, avg, max as spark_max, min as spark_min,
    stddev, percentile_approx, countDistinct, desc, asc,
    dense_rank, row_number, lag, lead, first, last,
    hash, md5, concat_ws, split, explode, collect_list,
    monotonically_increasing_id, broadcast, lit, expr
)
...
# Factory function
def create_delta_etl_processor(spark: Optional[SparkSession] = None) -> DeltaETLProcessor:
