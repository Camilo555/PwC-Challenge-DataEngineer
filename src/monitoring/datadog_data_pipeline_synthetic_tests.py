...
class DataDogDataPipelineSyntheticTests:
    ...
    def _get_database_configs(self) -> list[DatabaseConfig]:
        ...
        return [
            ...
            DatabaseConfig(
                ...
                performance_queries=[
                    {"name": "Cache Hit Rate", "query": "INFO stats", "expected_max_time": 100}
                ],
    ...
    def _get_data_quality_configs(self) -> list[DataQualityConfig]:
        ...
        return [
            DataQualityConfig(
                ...
                quality_rules=[
                    {"rule": "amount > 0", "description": "Transaction amount must be positive"},
                    {
            ...
            DataQualityConfig(
                ...
                quality_rules=[
                    {"rule": "price > 0", "description": "Product price must be positive"},
                    {
    ...
    async def deploy_etl_pipeline_tests(
        self,
            ...
            for pipeline_config in self.etl_pipelines:
                ...
                pipeline_steps = [
                    ...
                    APITestStep(
                        ...
                        extract_variables=[
                            {"name": "LAST_RUN_TIME", "type": "jsonpath", "path": "$.last_run_time"}
                        ],
                    ),
                    APITestStep(
                    ...
                    APITestStep(
                        ...
                        assertions=[
                            {"type": "statusCode", "operator": "is", "target": 200},
                            {
                                ...
                                "path": "$.data_age_hours",
                                "operator": "lessThan",
                                "target": pipeline_config.expected_sla.get(
                ...
                test_config = SyntheticTestConfig(
                    ...
                    if pipeline_config.business_critical
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"ETL Pipeline - {pipeline_config.name}", pipeline_steps, test_config
                )
                ...
                self.logger.info(
                    f"Deployed ETL pipeline test for {pipeline_config.name}: {test_id}"
                )
    ...
    async def deploy_database_tests(
        self,
        locations: list[TestLocation] = None,
        ...
        try:
            for db_config in self.databases:
                ...
                for _i, query_test in enumerate(db_config.performance_queries):
                    db_steps.append(
                        APITestStep(
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"Database - {db_config.name}", db_steps, test_config
                )
    ...
    async def deploy_streaming_tests(
        self,
        locations: list[TestLocation] = None,
        frequency: TestFrequency = TestFrequency.EVERY_5_MINUTES,
    ) -> dict[str, str]:
        ...
        try:
            for stream_config in self.streaming_configs:
                    ...
                    APITestStep(
                        ...
                        assertions=[
                            {"type": "statusCode", "operator": "is", "target": 200},
                            {
                                ...
                                "operator": "greaterThan",
                                "target": stream_config.throughput_threshold
                                * 0.1,  # At least 10% of expected throughput
                            },
                        ],
                    ),
                ]
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"Stream - {stream_config.name}", streaming_steps, test_config
                )
    ...
    async def deploy_data_quality_tests(
        self,
        locations: list[TestLocation] = None,
            ...
            for quality_config in self.quality_configs:
                ...
                quality_steps = [
                    ...
                    APITestStep(
                        ...
                        body=json.dumps({"rules": quality_config.quality_rules}),
                        assertions=[
                            {"type": "statusCode", "operator": "is", "target": 200},
                            {
                                "type": "jsonpath",
                                "path": "$.rules_passed",
                                "operator": "greaterThan",
                                "target": len(quality_config.quality_rules)
                                * 0.9,  # 90% of rules must pass
                            },
                        ],
                    ),
                ]
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"Data Quality - {quality_config.name}", quality_steps, test_config
                )
    ...
    async def get_data_pipeline_test_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            for pipeline_name, test_id in self.etl_tests.items():
                ...
                if pipeline_results:
                    pipeline_success_rate = (
                        len([r for r in pipeline_results if r.status == "passed"])
                        / len(pipeline_results)
                        * 100
                    )
                    pipeline_avg_time = sum(r.response_time for r in pipeline_results) / len(
                        pipeline_results
                    )
            ...
            return {
                ...
                "summary": {
                    "total_active_tests": len(self.etl_tests)
                    + len(self.database_tests)
                    + len(self.streaming_tests)
                    + len(self.quality_tests),
