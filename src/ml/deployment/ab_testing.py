...
class TrafficSplitter:
    ...
    def _random_assignment(self) -> str:
        ...
        # Treatment variants
        for i, _model_id in enumerate(self.config.treatment_model_ids):
            variant_id = f"treatment_{i}"
            variant_prob = self.config.traffic_allocation.get(
                variant_id, 0.5 / len(self.config.treatment_model_ids)
            )
...
class StatisticalAnalyzer:
    ...
    def analyze_metric(
        ...
        if metric_type == "continuous":
            ...
            ci_lower = (
                effect_size - t_critical * pooled_se / control_mean if control_mean != 0 else 0
            )
            ci_upper = (
                effect_size + t_critical * pooled_se / control_mean if control_mean != 0 else 0
            )
    ...
    def _bootstrap_ci(
        self, control_data: np.ndarray, treatment_data: np.ndarray, n_bootstrap: int = 1000
    ) -> tuple[float, float]:
        ...
        for _ in range(n_bootstrap):
            ...
            treatment_bootstrap = np.random.choice(
...
class ExperimentManager:
    ...
    async def _publish_experiment_event(
        self, config: ExperimentConfig, event_type: str, event_data: dict[str, Any] | None = None
    ):
        """Publish experiment lifecycle events to RabbitMQ."""
        if (
            not self.rabbitmq_manager
            or not config.enable_messaging
            or not config.publish_experiment_events
        ):
            return
    ...
    async def _stream_experiment_event(
        self, config: ExperimentConfig, event_type: str, event_data: dict[str, Any] | None = None
    ):
        """Stream experiment events to Kafka."""
        if (
            not self.kafka_manager
            or not config.enable_streaming
            or not config.stream_experiment_metrics
        ):
            return
        ...
        try:
            ...
            success = self.kafka_manager.produce_message(
                topic=StreamingTopic.METRICS, message=payload, key=config.experiment_id
            )
    ...
    async def _stream_metrics_real_time(
        self, experiment_id: str, variant: str, metric: str, value: float
    ):
    ...
    async def create_experiment(self, config: ExperimentConfig) -> bool:
        ...
        try:
            ...
            # Store in database
            self.supabase.table("ab_experiments").insert(experiment_data).execute()
            ...
            await self._publish_experiment_event(
                config,
                "created",
                {
            ...
            await self._stream_experiment_event(
                config,
                "experiment_created",
                {
            ...
            self.metrics_collector.increment_counter(
                "ab_testing_experiments_created_total", tags={"experiment_name": config.name}
            )
    ...
    async def start_experiment(self, experiment_id: str) -> bool:
        ...
        try:
            ...
            await self.redis_client.hset(
                f"experiment:{experiment_id}", "status", json.dumps(ExperimentStatus.RUNNING.value)
            )

            logger.info(f"Experiment started: {experiment_id}")

            self.metrics_collector.increment_counter(
                "ab_testing_experiments_started_total", tags={"experiment_id": experiment_id}
            )
    ...
    async def stop_experiment(self, experiment_id: str) -> bool:
        ...
        try:
            ...
            self.metrics_collector.increment_counter(
                "ab_testing_experiments_stopped_total", tags={"experiment_id": experiment_id}
            )
    ...
    async def assign_variant(self, experiment_id: str, user_context: dict[str, Any]) -> str | None:
        """Assign user to experiment variant."""
        try:
    ...
    async def record_event(
        self, experiment_id: str, user_context: dict[str, Any], event_type: str, event_value: float
    ) -> bool:
        ...
        try:
            ...
            if self.cache_manager:
                ...
                await self.cache_manager.expire(
                    cache_key, 86400 * 7, namespace="ab_testing"
                )  # 7 days TTL
    ...
    async def analyze_experiment(self, experiment_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            for metric in all_metrics:
                ...
                for i, _treatment_model_id in enumerate(config.treatment_model_ids):
                    variant_id = f"treatment_{i}"
                    treatment_data = await self._get_metric_data(experiment_id, variant_id, metric)

                    if (
                        len(control_data) >= config.minimum_sample_size
                        and len(treatment_data) >= config.minimum_sample_size
                    ):
                        # Perform statistical analysis
                        test_result = analyzer.analyze_metric(control_data, treatment_data, metric)
            ...
            self.metrics_collector.increment_counter(
                "ab_testing_analyses_completed_total", tags={"experiment_id": experiment_id}
            )
    ...
    async def _get_experiment_config(self, experiment_id: str) -> ExperimentConfig | None:
        ...
        # Query database
        result = (
            self.supabase.table("ab_experiments")
            .select("*")
    ...
    async def _get_experiment_status(self, experiment_id: str) -> ExperimentStatus | None:
    ...
    async def _get_user_variant(
        self, experiment_id: str, user_context: dict[str, Any]
    ) -> str | None:
        """Get user's variant assignment."""
        cache_key = f"assignment:{experiment_id}:{user_context.get('user_id', user_context.get('session_id', 'unknown'))}"
        ...
        query = (
            self.supabase.table("ab_assignments")
            .select("variant")
            .eq("experiment_id", experiment_id)
        )
    ...
    async def get_experiment_status_report(self, experiment_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            for variant in ["control"] + [
                f"treatment_{i}" for i in range(len(config.treatment_model_ids))
            ]:
                events_result = (
                    self.supabase.table("ab_events")
                ...
                sample_sizes[variant] = len(
                    {event.get("user_id") for event in events_result.data if event.get("user_id")}
                )
            ...
            # Calculate progress
            progress = (
                min(100, (total_sample_size / config.minimum_sample_size) * 100)
            ...
            report = {
                ...
                "time_remaining_days": (config.end_date - datetime.utcnow()).days
                if config.end_date > datetime.utcnow()
                else 0,
...
class ABTestingFramework:
    ...
    async def create_model_comparison_experiment(
        ...
        if success:
            ...
            logger.info(
                f"Model comparison experiment created and started: {experiment_name} ({experiment_id})"
            )
    ...
    async def check_experiment_for_early_stopping(self, experiment_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if primary_results:
                ...
                if (
                    significant_count / total_comparisons >= 0.5
                ):  # Majority of comparisons are significant
                    return {
                        "should_stop": True,
    ...
    async def get_active_experiments(self) -> list[dict[str, Any]]:
        ...
        try:
            result = (
                self.experiment_manager.supabase.table("ab_experiments")
                .select("*")
            ...
            for exp_data in result.data or []:
                status_report = await self.experiment_manager.get_experiment_status_report(
    ...
    async def cleanup_completed_experiments(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            result = (
                ...
                .eq("status", ExperimentStatus.COMPLETED.value)
                .lt("actual_end_date", cutoff_date.isoformat())
                .execute()
            )
