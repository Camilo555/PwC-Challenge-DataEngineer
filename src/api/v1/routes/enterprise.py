...
from api.gateway.service_registry import ServiceInstance, get_service_registry
from api.patterns.cqrs_framework import get_cqrs_framework
from api.patterns.saga_orchestrator import OrderSagaBuilder, get_saga_orchestrator
...
async def register_service(request: ServiceRegistrationRequest):
    ...
    if success:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to register service"
        )
...
async def list_active_sagas():
    ...
    return {"total_active_sagas": len(sagas), "sagas": sagas}
...
async def enterprise_health_check():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "components": {
                "service_registry": {"status": "healthy", "registered_services": service_count},
                "saga_orchestrator": {"status": "healthy", "active_sagas": active_saga_count},
                "cqrs_framework": {"status": "healthy", "handlers_registered": True},
            },
            "timestamp": time.time(),
        }
