...
class ETLAPMTracker:
    ...
    async def trace_etl_operation(
        ...
        with tracer.trace(
            ...
            try:
                ...
                self.logger.error(
                    f"ETL operation failed: {layer}.{operation_type}.{operation_name} - {str(e)}"
                )
                raise
                ...
                if self.datadog_monitoring:
                    await self._send_etl_operation_metrics(
                        operation_type,
                        layer,
                        operation_name,
                        duration,
    ...
    async def _send_etl_operation_metrics(
        self,
        operation_type: str,
        layer: str,
        operation_name: str,
        ...
        try:
            ...
            self.datadog_monitoring.histogram(
                f"etl.layer.{layer}.duration", duration * 1000, tags=tags
            )
    ...
    async def trace_bronze_ingestion(
        self, dataset_name: str, source_type: str, pipeline_id: str | None = None
    ):
    ...
    async def trace_bronze_validation(
        self,
        dataset_name: str,
        record_count: int,
        ...
        validation_success_rate = (
            passed_validations / len(validation_results) if validation_results else 1.0
        )
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "etl.bronze.validation_success_rate", validation_success_rate, tags=tags
            )
            self.datadog_monitoring.gauge(
                "etl.bronze.validations_failed", failed_validations, tags=tags
            )
            ...
            for rule_name, result in validation_results.items():
                ...
                if result.get("passed", False):
                    self.datadog_monitoring.counter(
                        "etl.bronze.validations.passed", tags=validation_tags
                    )
                else:
                    self.datadog_monitoring.counter(
                        "etl.bronze.validations.failed", tags=validation_tags
                    )
    ...
    async def trace_bronze_schema_detection(
        self,
        dataset_name: str,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "etl.bronze.schema_changes", schema_changes_count, tags=tags
            )
    ...
    async def trace_silver_transformation(
        self, dataset_name: str, transformation_type: str, pipeline_id: str | None = None
    ):
        """Context manager for tracing silver layer transformations."""

        metadata = {"transformation_type": transformation_type, "data_quality_enforcement": True}
    ...
    async def trace_silver_cleaning(
        self,
        dataset_name: str,
        records_input: int,
        records_output: int,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "etl.silver.data_quality_score", data_quality_score, tags=tags
            )
    ...
    async def trace_silver_enrichment(
        self,
        dataset_name: str,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "etl.silver.records_enriched", records_enriched, tags=tags
            )
            self.datadog_monitoring.gauge(
                "etl.silver.enrichment_success_rate", enrichment_success_rate, tags=tags
            )
            self.datadog_monitoring.gauge(
                "etl.silver.external_api_calls", external_api_calls, tags=tags
            )
    ...
    async def trace_gold_aggregation(
        self, dataset_name: str, aggregation_type: str, pipeline_id: str | None = None
    ):
        """Context manager for tracing gold layer aggregations."""

        metadata = {"aggregation_type": aggregation_type, "business_ready": True}
    ...
    async def trace_gold_star_schema(
        self,
        dataset_name: str,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "etl.gold.fact_table_records", fact_table_records, tags=tags
            )
            self.datadog_monitoring.gauge("etl.gold.dimension_count", dimension_count, tags=tags)
            self.datadog_monitoring.gauge(
                "etl.gold.data_freshness_minutes", data_freshness_minutes, tags=tags
            )
            ...
            for dim_name, dim_records in dimension_tables.items():
                ...
                self.datadog_monitoring.gauge(
                    "etl.gold.dimension_records", dim_records, tags=dim_tags
                )
    ...
    async def trace_gold_business_metrics(
        self,
        dataset_name: str,
        business_metrics: dict[str, float],
        kpi_calculations: dict[str, float],
    ) -> dict[str, Any]:
        ...
        if self.datadog_monitoring:
            ...
            for metric_name, metric_value in business_metrics.items():
                ...
                self.datadog_monitoring.gauge(
                    f"etl.business.{metric_name}", metric_value, tags=metric_tags
                )
    ...
    async def trace_data_quality_assessment(
        self,
        dataset_name: str,
        layer: str,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "etl.data_quality.overall_score", overall_quality_score, tags=tags
            )
            ...
            for dimension, score in quality_dimensions.items():
                ...
                self.datadog_monitoring.gauge(
                    f"etl.data_quality.dimension.{dimension}", score, tags=dimension_tags
                )
    ...
    async def trace_pipeline_execution(
        self, pipeline_id: str, pipeline_name: str, stages: list[str], schedule_type: str = "batch"
    ):
    ...
    async def trace_pipeline_stage(
        self,
        pipeline_id: str,
        stage_name: str,
        stage_order: int,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.histogram(
                "etl.stage.duration", stage_duration * 1000, tags=tags
            )
    ...
    def get_etl_performance_summary(self) -> dict[str, Any]:
        ...
        for layer, stats in self.layer_statistics.items():
            if stats["runs"] > 0:
                ...
                throughput = (
                    stats["records_processed"] / stats["total_time"]
        ...
        for pipeline_id, pipeline_data in self.active_pipelines.items():
            ...
            successful_stages = sum(
                1 for success in pipeline_data["stage_status"].values() if success
            )
            ...
            pipeline_summary[pipeline_id] = {
                ...
                "duration_hours": (datetime.utcnow() - pipeline_data["start_time"]).total_seconds()
                / 3600,
            }
    ...
    def _calculate_quality_trend(self, measurements: list[dict[str, Any]]) -> str:
        ...
        older_avg = (
            sum(m["score"] for m in measurements[-6:-3]) / 3
            if len(measurements) >= 6
            else measurements[0]["score"]
        )
...
_etl_apm_tracker: ETLAPMTracker | None = None


def get_etl_apm_tracker(
    service_name: str = "etl-pipeline", datadog_monitoring: DatadogMonitoring | None = None
) -> ETLAPMTracker:
...
async def trace_bronze_processing_session(
    dataset_name: str, source_type: str, pipeline_id: str | None = None
):
...
async def trace_silver_processing_session(
    dataset_name: str, transformation_type: str, pipeline_id: str | None = None
):
...
async def trace_gold_processing_session(
    dataset_name: str, aggregation_type: str, pipeline_id: str | None = None
):
...
async def trace_pipeline_session(
    pipeline_id: str, pipeline_name: str, stages: list[str], schedule_type: str = "batch"
):
