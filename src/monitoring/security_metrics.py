...
from core.monitoring.metrics import MetricPoint, MetricsCollector, MetricType
from core.security.compliance_framework import ComplianceFramework
from core.security.enterprise_dlp import DLPAction, SensitiveDataType
...
class SecurityMetricsCollector:
    ...
    def _update_threat_metrics(self, event: SecurityMetricEvent):
        ...
        if "response_time_seconds" in event.details:
            ...
            self.threat_metrics.avg_response_time_seconds = (
                (current_avg * (total_threats - 1)) + response_time
            ) / total_threats
    ...
    def _update_access_control_metrics(self, event: SecurityMetricEvent):
        ...
        if "decision_time_ms" in event.details:
            ...
            self.access_metrics.avg_decision_time_ms = (
                (current_avg * (total_requests - 1)) + decision_time
            ) / total_requests
    ...
    def _send_to_core_metrics(self, event: SecurityMetricEvent):
        ...
        try:
            ...
            self.metrics_collector.increment_counter("security_events_total", labels=labels)
    ...
    def get_threat_detection_metrics(self, time_window_hours: int = 24) -> dict[str, Any]:
        """Get threat detection metrics"""
        cutoff_time = datetime.now() - timedelta(hours=time_window_hours)
        recent_events = [
            e
            for e in self.security_events
            if e.timestamp >= cutoff_time and e.metric_type == SecurityMetricType.THREAT_DETECTION
        ]
        ...
        return {
            ...
            "top_threat_types": sorted(threats_by_type.items(), key=lambda x: x[1], reverse=True)[
                :5
            ],
        }

    def get_dlp_effectiveness_metrics(self, time_window_hours: int = 24) -> dict[str, Any]:
        """Get DLP effectiveness metrics"""
        cutoff_time = datetime.now() - timedelta(hours=time_window_hours)
        dlp_events = [
            e
            for e in self.security_events
            if e.timestamp >= cutoff_time and e.metric_type == SecurityMetricType.DLP_VIOLATION
        ]
        ...
        return {
            ...
            "most_detected_types": sorted(data_types.items(), key=lambda x: x[1], reverse=True)[:5],
        }

    def get_compliance_adherence_metrics(self, time_window_hours: int = 24) -> dict[str, Any]:
        """Get compliance adherence metrics"""
        cutoff_time = datetime.now() - timedelta(hours=time_window_hours)
        compliance_events = [
            e
            for e in self.security_events
            if e.timestamp >= cutoff_time and e.metric_type == SecurityMetricType.COMPLIANCE_CHECK
        ]
        ...
        return {
            ...
            "overall_compliance_rate": (total_checks - violations) / total_checks
            if total_checks > 0
            else 1.0,
    ...
    def get_access_control_analytics(self, time_window_hours: int = 24) -> dict[str, Any]:
        """Get access control analytics"""
        cutoff_time = datetime.now() - timedelta(hours=time_window_hours)
        access_events = [
            e
            for e in self.security_events
            if e.timestamp >= cutoff_time and e.metric_type == SecurityMetricType.ACCESS_CONTROL
        ]
        ...
        for event in access_events:
            if event.user_id:
                ...
                if event.outcome == "denied" or event.severity in [
        ...
        return {
            ...
            "top_requesting_users": sorted(user_requests.items(), key=lambda x: x[1], reverse=True)[
                :10
            ],
            "suspicious_users": dict(suspicious_users),
            "high_risk_requests": len(
                [e for e in access_events if event.details.get("risk_score", 0) > 8]
            ),
        }
    ...
    def _generate_security_recommendations(
        self, threat_score: float, dlp_score: float, compliance_score: float, access_score: float
    ) -> list[str]:
    ...
    async def _background_processor(self):
        ...
        while self._processing_enabled:
            try:
                ...
                cutoff_day_key = cutoff_day.replace(
                ...
                with self._lock:
                    ...
                    cutoff_hour_key = cutoff_hour.replace(
...
def record_compliance_check(
    ...
    collector = get_security_metrics_collector()
    severity = (
        SecurityEventSeverity.CRITICAL if outcome == "violation" else SecurityEventSeverity.INFO
    )
    event = SecurityMetricEvent(
        event_id=event_id,
