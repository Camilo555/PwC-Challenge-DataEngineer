...
class DataDogBusinessMetricsTracker:
    ...
    def __init__(
        self,
        service_name: str = "business-intelligence",
    ...
    async def _send_kpi_to_datadog(self, kpi_def: KPIDefinition, kpi_value: KPIValue):
        ...
        try:
            ...
            self.datadog_monitoring.gauge(
                f"business.kpi.{kpi_def.name}", kpi_value.value, tags=dd_tags
            )

            # Send category summary
            self.datadog_monitoring.counter(
                f"business.kpi.category.{kpi_def.category.value}", tags=dd_tags
            )
    ...
    async def _send_kpi_alert(
        self, kpi_def: KPIDefinition, kpi_value: KPIValue, alert_level: AlertLevel, message: str
    ):
        ...
        try:
            if self.datadog_monitoring:
                ...
                self.datadog_monitoring.counter("business.kpi.alerts", tags=tags)
    ...
    async def _send_business_event_to_datadog(self, event: BusinessEvent):
        ...
        try:
            tags = [f"event_type:{event.event_type}", f"event_name:{event.event_name}"]
            ...
            self.datadog_monitoring.counter(f"business.events.{event.event_type}", tags=tags)

            # Track event value if present
            if event.value is not None:
                self.datadog_monitoring.histogram(
                    f"business.events.{event.event_type}.value", event.value, tags=tags
                )
    ...
    async def track_revenue(
        ...
        await self.track_business_event(
            BusinessEvent(
                event_type="revenue",
    ...
    async def track_customer_acquisition(
        self, customer_id: str, acquisition_channel: str, acquisition_cost: float | None = None
    ):
        ...
        if acquisition_cost:
            await self.track_kpi(
                "customer_acquisition_cost", acquisition_cost, tags={"channel": acquisition_channel}
            )
    ...
    async def track_error_occurrence(
        ...
        await self.track_business_event(
            BusinessEvent(
                ...
                user_id=user_id,
                properties={"severity": severity, "component": component},
            )
        )
    ...
    async def calculate_realtime_kpis(self) -> dict[str, Any]:
        ...
        try:
            ...
            event_summaries = {}
            [e for e in self.business_events if e.timestamp >= last_hour]
            ...
            # Calculate averages
            for _event_type, summary in event_summaries.items():
            ...
            self.realtime_calculations = {
                ...
                "summary": {
                    ...
                    "kpis_with_data": len(
                        [k for k in calculations.values() if k["latest_value"] is not None]
                    ),
                    "kpis_critical": len(
                        [k for k in calculations.values() if k["status"] == "critical"]
                    ),
                    "kpis_warning": len(
                        [k for k in calculations.values() if k["status"] == "warning"]
                    ),
    ...
    def get_kpi_dashboard_data(self) -> dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "statistics": {
                ...
                "categories": list({kpi.category.value for kpi in self.kpi_definitions.values()}),
            },
        }
...
_business_metrics_tracker: DataDogBusinessMetricsTracker | None = None


def get_business_metrics_tracker(
    service_name: str = "business-intelligence", datadog_monitoring: DatadogMonitoring | None = None
) -> DataDogBusinessMetricsTracker:
...
async def track_kpi_value(kpi_name: str, value: float, tags: dict[str, str] | None = None):
...
async def track_revenue_event(amount: float, currency: str = "USD", customer_id: str | None = None):
    """Convenience function for tracking revenue."""
    tracker = get_business_metrics_tracker()
    return await tracker.track_revenue(amount, currency, customer_id)


async def track_user_event(user_id: str, activity_type: str, session_id: str | None = None):
...
async def track_feature_event(
    feature_name: str, user_id: str | None = None, usage_value: float | None = None
):
...
async def track_business_operation(
    operation_name: str, category: str, expected_duration_seconds: float | None = None
):
    ...
    await tracker.track_business_event(
        BusinessEvent(
