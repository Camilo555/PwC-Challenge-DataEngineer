...
class BaseSparkProcessor(ABC):
    ...
    def __init__(self, spark_session: SparkSession | None = None):
        self.spark = spark_session or SparkSessionManager().get_or_create_session(
    ...
    def _calculate_data_quality_score(self, df: DataFrame, quality_checks: list[dict]) -> float:
        ...
        for check in quality_checks:
            ...
            if score < threshold:
                logger.warning(
                    f"Data quality issue in {column}: {check_type} score {score:.3f} below threshold {threshold}"
                )
    ...
    def _record_processing_metrics(self, stage: str, result: dict[str, Any]):
        ...
        self.processing_stats.update({"processing_time_seconds": processing_time, **result})
    ...
    def _save_quality_report(self, df: DataFrame, stage: str, quality_checks: list[dict]):
        ...
        try:
            ...
            report_file = (
                report_dir / f"quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
        ...
        for source_name, config in self.source_configs.items():
            try:
                ...
                self._save_quality_report(
                    df_transformed, f"bronze_{source_name}", config["quality_checks"]
                )
        ...
        self.processing_stats.update(
            {
                ...
                "data_quality_score": sum(r["data_quality_score"] for r in results.values())
                / len(results)
                if results
                else 0,
            }
    ...
    def _apply_bronze_transformations(self, df: DataFrame, source_name: str) -> DataFrame:
        ...
        if "Description" in df.columns:
            df_with_metadata = df_with_metadata.withColumn(
                "Description", trim(regexp_replace(col("Description"), r"[^\w\s]", ""))
            )

        if "Country" in df.columns:
            df_with_metadata = df_with_metadata.withColumn("Country", trim(upper(col("Country"))))
...
class EnhancedSilverProcessor(BaseSparkProcessor):
    ...
    def _read_bronze_data(self) -> DataFrame | None:
        ...
        try:
            bronze_paths = ["./data/bronze/retail_transactions", "./data/bronze/sample_data"]
    ...
    def _apply_silver_transformations(self, df: DataFrame) -> DataFrame:
        ...
        df_enhanced = df_typed.withColumn(
            "TotalPrice", col("Quantity") * col("UnitPrice")
        ).withColumn(
            "IsValidTransaction",
    ...
    def _save_silver_data(self, df: DataFrame, output_path: str):
        ...
        try:
            ...
            df_partitioned = df.withColumn("year", year("InvoiceDate")).withColumn(
...
class EnhancedGoldProcessor(BaseSparkProcessor):
    ...
    def _build_sales_by_country(self, df: DataFrame) -> dict[str, Any]:
        ...
        try:
            sales_by_country = (
                df.filter(col("IsValidTransaction"))
                .groupBy("Country")
                .agg(
                    spark_sum("TotalPrice").alias("total_revenue"),
    ...
    def _build_customer_analytics(self, df: DataFrame) -> dict[str, Any]:
        ...
        try:
            customer_analytics = (
                df.filter(col("IsValidTransaction") & col("CustomerID").isNotNull())
            ...
            Path(output_path).mkdir(parents=True, exist_ok=True)
            customer_analytics.write.mode("overwrite").partitionBy("customer_segment").parquet(
                output_path
            )
    ...
    def _build_rfm_analysis(self, df: DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            rfm_with_scores = (
                rfm_metrics.withColumn(
                    "recency_score", ntile(5).over(Window.orderBy(col("recency")))
                )
