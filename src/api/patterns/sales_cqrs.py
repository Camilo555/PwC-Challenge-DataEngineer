...
class GetSalesAnalyticsQuery(Query):
    ...
    metrics: list[str] = Field(
        default_factory=lambda: ["total_sales", "order_count", "avg_order_value"]
    )
...
class SaleUpdatedEvent(Event):
    ...
    def __init__(self, sale_id: str, updated_fields: dict[str, Any], **kwargs):
        super().__init__(
            ...
            aggregate_id=sale_id,
            aggregate_type="Sale",
            payload={"sale_id": sale_id, "updated_fields": updated_fields, **kwargs},
        )
...
class SaleDeletedEvent(Event):
    ...
    def __init__(self, sale_id: str, reason: str | None = None, **kwargs):
        super().__init__(
            event_type="SaleDeleted",
            aggregate_id=sale_id,
            aggregate_type="Sale",
            payload={"sale_id": sale_id, "reason": reason, **kwargs},
        )
...
class GetSaleQueryHandler(QueryHandler[GetSaleQuery, dict[str, Any]]):
    ...
    async def handle(self, query: GetSaleQuery) -> QueryResult[dict[str, Any]]:
        ...
        try:
            ...
            return QueryResult(query_id=query.query_id, data=None, metadata={"error": str(e)})
...
class GetSalesByCustomerQueryHandler(QueryHandler[GetSalesByCustomerQuery, list[dict[str, Any]]]):
    ...
    async def handle(self, query: GetSalesByCustomerQuery) -> QueryResult[list[dict[str, Any]]]:
        ...
        try:
            ...
            return QueryResult(query_id=query.query_id, data=[], metadata={"error": str(e)})
...
async def setup_sales_cqrs(
    mediator, sales_repository, analytics_service, audit_service, notification_service
):
    """Setup sales CQRS handlers."""

    # Register command handlers
    mediator.register_command_handler(
        CreateSaleCommand, CreateSaleCommandHandler(sales_repository, mediator.event_bus)
    )
    mediator.register_command_handler(
        UpdateSaleCommand, UpdateSaleCommandHandler(sales_repository, mediator.event_bus)
    )
    mediator.register_command_handler(
        DeleteSaleCommand, DeleteSaleCommandHandler(sales_repository, mediator.event_bus)
    )

    # Register query handlers
    mediator.register_query_handler(GetSaleQuery, GetSaleQueryHandler(sales_repository))
    mediator.register_query_handler(
        GetSalesByCustomerQuery, GetSalesByCustomerQueryHandler(sales_repository)
    )
    mediator.register_query_handler(
        GetSalesAnalyticsQuery, GetSalesAnalyticsQueryHandler(analytics_service)
    )
