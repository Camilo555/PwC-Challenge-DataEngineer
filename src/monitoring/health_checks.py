...
class ServiceAvailabilityMetrics:
    ...
    mttr_seconds: float | None = None  # Mean Time To Recovery
    mtbf_seconds: float | None = None  # Mean Time Between Failures
...
class HealthChecker:
    ...
    def _check_api_health(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                ...
                timestamp=datetime.now(),
                details={
                    "url": f"http://localhost:{getattr(settings, 'api_port', 8000)}/api/v1/health"
                },
    ...
    def _check_etl_processes(self) -> HealthCheck:
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    if proc.info["name"] == "python.exe" or proc.info["name"] == "python":
                        ...
                        if any(
                            keyword in cmdline for keyword in ["bronze", "silver", "gold", "etl"]
                        ):
                            etl_processes.append({"pid": proc.info["pid"], "cmdline": cmdline})
            ...
            return HealthCheck(
                ...
                response_time_ms=0,
                timestamp=datetime.now(),
                details={"process_count": len(etl_processes), "processes": etl_processes},
            )
    ...
    def _check_system_network(self) -> HealthCheck:
        ...
        try:
            ...
            if network_io.errin > 1000 or network_io.errout > 1000:
                status = HealthStatus.WARNING
                issues.append(
                    f"High network errors: in={network_io.errin}, out={network_io.errout}"
                )

            if network_io.dropin > 100 or network_io.dropout > 100:
                status = HealthStatus.WARNING
                issues.append(
                    f"Network packet drops detected: in={network_io.dropin}, out={network_io.dropout}"
                )
    ...
    def _check_system_processes(self) -> HealthCheck:
        ...
        try:
            ...
            sleeping_processes = len(
                [p for p in processes if p.info["status"] == psutil.STATUS_SLEEPING]
            )
    ...
    def _check_redis_health(self) -> HealthCheck:
        ...
        try:
            ...
            memory_usage_percent = (
                (used_memory_mb / max_memory_mb * 100) if max_memory_mb > 0 else 0
            )
    ...
    def _get_storage_recommendation(self, usage_percent: float, days_until_full: int | None) -> str:
    ...
    def _calculate_sla_metrics(self, check_name: str) -> dict[str, Any]:
        ...
        recent_results = [
            result for result in self.check_history[check_name] if result.timestamp > cutoff_time
        ]
