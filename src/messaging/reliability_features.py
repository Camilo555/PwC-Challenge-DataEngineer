...
class CircuitBreakerConfig:
    ...
    expected_exceptions: list[str] = field(
        default_factory=lambda: ["AMQPConnectionError", "TimeoutError"]
    )
...
class MessageReliabilityHandler:
    ...
    def setup_default_circuit_breakers(self):
        ...
        configs = {
            ...
            QueueType.SYSTEM_ALERTS.value: CircuitBreakerConfig(
                failure_threshold=3, timeout_seconds=60, half_open_max_calls=2
            ),
            QueueType.REALTIME_ANALYTICS.value: CircuitBreakerConfig(
                failure_threshold=10, timeout_seconds=45, half_open_max_calls=5
            ),
            "default": CircuitBreakerConfig(
                failure_threshold=5, timeout_seconds=60, half_open_max_calls=3
            ),
        }

        for queue_name, config in configs.items():
            self.circuit_breakers[queue_name] = AdvancedCircuitBreaker(
                name=queue_name, config=config, logger=self.logger
            )
    ...
    def _start_cleanup_threads(self):
        ...
        dedup_cleanup_thread = threading.Thread(
            target=self._deduplication_cleanup_worker, daemon=True
        )
        dedup_cleanup_thread.start()

        # Circuit breaker reset thread
        breaker_reset_thread = threading.Thread(
            target=self._circuit_breaker_reset_worker, daemon=True
        )
    ...
    def publish_reliable_message(
        ...
        try:
            ...
            try:
                if enable_circuit_breaker:
                    success = breaker.execute(
                        self._do_publish_message, message, exchange_name, routing_key
                    )
                else:
    ...
    async def publish_reliable_message_async(
        ...
        try:
            ...
            if enable_deduplication and self._is_duplicate_message(message, effective_queue):
                self.dedup_stats["duplicates_rejected"] += 1
                self.logger.debug(
                    f"Async duplicate message rejected: {message.metadata.message_id}"
                )
                return True
            ...
            try:
                if enable_circuit_breaker:
                    ...
                    success = await self._do_publish_message_async(
    ...
    def _do_publish_message(
        self, message: EnterpriseMessage, exchange_name: str | None, routing_key: str | None
    ) -> bool:
        ...
        while True:
            try:
                ...
                for message_hash, timestamp in self.hash_timestamps.items():
                    ...
                    ).total_seconds() > self.dedup_config.cache_ttl_seconds:
                ...
                if expired_hashes:
                    self.logger.debug(
                        f"Cleaned up {len(expired_hashes)} expired deduplication hashes"
                    )
    ...
    def _trim_deduplication_cache(self):
        ...
        try:
            ...
            sorted_hashes = sorted(self.hash_timestamps.items(), key=lambda x: x[1], reverse=True)
            ...
            self.logger.info(
                f"Trimmed deduplication cache: removed {len(hashes_to_remove)} entries"
            )
    ...
    def _should_retry(
        self, exception: Exception, queue_name: str, retry_config: RetryConfig | None = None
    ) -> bool:
    ...
    def _schedule_delayed_retry(self, message: EnterpriseMessage, queue_name: str, delay: float):
        ...
        try:
            ...
            with self.rabbitmq.connection_pool.get_connection() as connection:
                ...
                self.logger.debug(
                    f"Scheduled retry for message {message.metadata.message_id} in {delay} seconds"
                )
    ...
    async def _schedule_delayed_retry_async(
        self, message: EnterpriseMessage, queue_name: str, delay: float
    ):
    ...
    def _retry_message_immediately(self, message: EnterpriseMessage, queue_name: str):
        ...
        try:
            success = self.rabbitmq.publish_message(message=message, routing_key=queue_name)
    ...
    def _send_to_dead_letter_queue(self, message: EnterpriseMessage, error_reason: str):
        ...
        try:
            ...
            success = self.rabbitmq.publish_message(
                message=dead_letter_message, routing_key=QueueType.DEAD_LETTER.value
            )
    ...
    def recover_from_dead_letter_queue(self, message_id: str, target_queue: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Recovered message {message_id} from dead letter queue to {target_queue}"
            )
            return True
    ...
    def create_priority_queue_manager(
        self, queue_name: str, max_priority: int = 10
...
class AdvancedCircuitBreaker:
    ...
    def _record_success(self, execution_time: float):
        ...
        with self.lock:
            ...
            self.recent_calls.append(
                {"timestamp": current_time, "success": True, "execution_time": execution_time}
            )
    ...
    def _record_failure(self, error: str):
        ...
        with self.lock:
            ...
            # Record call result
            self.recent_calls.append({"timestamp": current_time, "success": False, "error": error})
    ...
    def get_failure_rate(self, window_seconds: int = 60) -> float:
        ...
        with self.lock:
            ...
            relevant_calls = [
                call for call in self.recent_calls if call["timestamp"] >= window_start
            ]
...
class PriorityQueueManager:
    ...
    def __init__(
        ...
        queue_name: str,
        max_priority: int,
        rabbitmq_manager: EnterpriseRabbitMQManager,
        logger,
    ):
    ...
    def _setup_priority_queue(self):
        ...
        try:
            with self.rabbitmq.connection_pool.get_connection() as connection:
                ...
                self.logger.info(
                    f"Priority queue {self.queue_name} setup with max priority {self.max_priority}"
                )
    ...
    def publish_priority_message(
        self, message: EnterpriseMessage, priority: int | None = None
    ) -> bool:
        ...
        try:
            ...
            return self.rabbitmq.publish_message(message=message, routing_key=self.queue_name)
...
_reliability_handler: MessageReliabilityHandler | None = None
