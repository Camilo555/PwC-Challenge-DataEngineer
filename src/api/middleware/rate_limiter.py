...
from messaging.rabbitmq_manager import RabbitMQManager, StateMessage
...
class AdaptiveRateLimiter:
    ...
    def update_user_behavior(self, user_id: str, behavior_score_delta: float):
        ...
        self.user_behavior_scores[user_id] = 0.7 * current_score + 0.3 * (
            1.0 + behavior_score_delta
        )
...
class DistributedSlidingWindowRateLimiter:
    ...
    def __init__(self, rabbitmq_manager: RabbitMQManager, kafka_manager: KafkaManager):
        ...
        self.local_state: dict[str, dict[str, Any]] = defaultdict(
            lambda: {"requests": [], "last_updated": time.time()}
        )
        ...
        # Metrics tracking
        self.metrics = {"total_requests": 0, "blocked_requests": 0, "adaptive_adjustments": 0}
    ...
    def _update_local_state(self, key: str, requests: list[float]):
        ...
        self.local_state[key] = {"requests": requests, "last_updated": time.time()}
    ...
    async def _publish_state_update(self, key: str, requests: list[float], window: int):
        ...
        try:
            state_data = {"requests": requests, "window": window, "updated_at": time.time()}
    ...
    async def _publish_rate_limit_event(
    ...
    async def is_allowed(
        ...
        if user_id:
            ...
            if limit != original_limit:
                ...
                logger.debug(
                    f"Adaptive limit adjustment for {user_id}: {original_limit} -> {limit}"
                )
        ...
        if user_id:
            ...
            metadata["behavior_score"] = round(
                self.adaptive_limiter.user_behavior_scores.get(user_id, 1.0), 2
            )
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.rate_limiter = DistributedSlidingWindowRateLimiter(
            self.rabbitmq_manager, self.kafka_manager
        )
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            if user_id:
                ...
                response.headers["X-RateLimit-Behavior-Score"] = str(
                    metadata.get("behavior_score", 1.0)
                )
...
class RateLimitStateManager:
    ...
    def start_state_consumer(self):
        ...
        self.rabbitmq_manager.consume_state_updates(callback=handle_state_update, auto_ack=True)


# Factory function to create rate limit middleware with common configurations
def create_rate_limit_middleware(
    app, api_limits: dict[str, dict[str, Any]] | None = None
) -> RateLimitMiddleware:
    ...
    async def test_rate_limiter():
        ...
        for i in range(15):
            allowed, metadata = await rate_limiter.is_allowed(
                key="test_client", limit=10, window=60, burst_limit=12
            )
