...
class AirflowConfig:
    ...
    def _load_dag_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "enable_slack_notifications": os.getenv("AIRFLOW_SLACK_NOTIFICATIONS", "false").lower()
            == "true",
...
def validate_dag_dependencies():
    ...
    required_packages = ["pandas", "sqlalchemy", "fastapi", "pyspark"]
    ...
    logger.info(
        f"DAG dependency validation: {'PASSED' if validation_results['all_valid'] else 'FAILED'}"
    )
