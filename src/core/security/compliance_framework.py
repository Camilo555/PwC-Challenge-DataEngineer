...
from core.security.advanced_security import ActionType, AuditLogger, SecurityEvent
...
class ComplianceViolation:
    ...
    user_id: str | None
    session_id: str | None
    source_event: SecurityEvent | None
...
class ComplianceEngine:
    ...
    async def _assess_control(
        self, control: ComplianceControl, assessor_id: str
    ...
    async def _check_data_encryption(self) -> dict[str, Any]:
        ...
        try:
            ...
            if config.encryption_enabled:
                ...
                score += 0.1
                evidence.append(
                    {"type": "configuration", "description": "Encryption enabled", "value": True}
                )
            else:
    ...
    async def _check_data_deletion_capability(self) -> dict[str, Any]:
        ...
        status = (
            ComplianceStatus.COMPLIANT if score >= 0.8 else ComplianceStatus.PARTIALLY_COMPLIANT
        )

        return {"status": status, "score": score, "findings": findings, "evidence": evidence}
    ...
    async def _create_violation(self, assessment: ComplianceAssessment):
        ...
        violation = ComplianceViolation(
            ...
            + timedelta(days=30 if assessment.risk_level == RiskLevel.HIGH else 60),
    ...
    async def _create_remediation_actions(self, violation: ComplianceViolation):
        ...
        action = RemediationAction(
            ...
            if violation.severity in [RiskLevel.HIGH, RiskLevel.CRITICAL]
    ...
    def get_compliance_dashboard(self) -> dict[str, Any]:
        ...
        compliant_count = len(
            [a for a in recent_assessments if a.status == ComplianceStatus.COMPLIANT]
        )
        non_compliant_count = len(
            [a for a in recent_assessments if a.status == ComplianceStatus.NON_COMPLIANT]
        )
        ...
        for framework in ComplianceFramework:
            ...
            if framework_assessments:
                compliant = len(
                    [a for a in framework_assessments if a.status == ComplianceStatus.COMPLIANT]
                )
        ...
        for severity in RiskLevel:
            count = len(
                [v for v in self.violations if v.severity == severity and v.status == "open"]
            )
        ...
        return {
            "overall_compliance": {
                ...
                "compliance_rate": compliant_count / len(recent_assessments)
            ...
            "recent_activity": {
                ...
                "resolved_violations_30d": len(
                    [
                        v
                        for v in self.violations
                        if v.status == "resolved" and v.timestamp >= last_30d
                    ]
                ),
            },
            "remediation_status": {
                ...
                "in_progress": len(
                    [a for a in self.remediation_actions if a.status == "in_progress"]
                ),
    ...
    def _generate_recommendations(
        self, assessments: list[ComplianceAssessment], violations: list[ComplianceViolation]
    ) -> list[str]:
        ...
        failed_controls = [
            a.control_id for a in assessments if a.status == ComplianceStatus.NON_COMPLIANT
        ]

        if len(failed_controls) > len(set(failed_controls)) * 0.5:
            recommendations.append(
                "Consider implementing automated compliance checks for frequently failing controls"
            )

        high_risk_violations = [
            v for v in violations if v.severity in [RiskLevel.HIGH, RiskLevel.CRITICAL]
        ]
        if high_risk_violations:
            recommendations.append(
                f"Prioritize resolution of {len(high_risk_violations)} high-risk compliance violations"
            )

        if len(violations) > 10:
            recommendations.append(
                "Consider implementing continuous compliance monitoring to detect issues earlier"
            )
