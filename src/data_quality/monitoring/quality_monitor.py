...
class QualityMonitor:
    ...
    def _start_monitoring_threads(self):
        ...
        try:
            ...
            metric_thread = threading.Thread(
                target=self._metric_evaluation_loop, name="MetricEvaluator", daemon=True
            )
            metric_thread.start()
            ...
            alert_thread = threading.Thread(
                target=self._alert_management_loop, name="AlertManager", daemon=True
            )
            alert_thread.start()
            ...
            dashboard_thread = threading.Thread(
                target=self._dashboard_update_loop, name="DashboardUpdater", daemon=True
            )
            ...
            cleanup_thread = threading.Thread(
                target=self._cleanup_loop, name="DataCleanup", daemon=True
            )
    ...
    def create_alert_rule(self, name: str, dataset_pattern: str, metric_name: str, **kwargs) -> str:
        ...
        try:
            with self._lock:
                ...
                dashboard.alerts_last_24h = len(
                    [a for a in self.alert_history if a.triggered_at >= last_24h]
                )
        ...
        try:
            with self._lock:
                ...
                return {
                    ...
                    "by_dataset": dict(
                        sorted(by_dataset.items(), key=lambda x: x[1], reverse=True)[:10]
                    ),
                    "by_metric": dict(
    ...
    def _metric_evaluation_loop(self):
        ...
        while self.monitoring_status == MonitoringStatus.ACTIVE:
            try:
                ...
                recent_metrics = [m for m in self.metrics_buffer if m.timestamp >= recent_time]
    ...
    def _check_threshold_violation(
        self, rule: AlertRule, metric: QualityMetric
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            if rule.percentage_change_threshold is not None:
                historical_avg = self._get_historical_average(
                    metric.dataset_id, metric.metric_name, hours=24
                )

                if historical_avg is not None and historical_avg != 0:
                    change_percent = abs(
                        (metric.metric_value - historical_avg) / historical_avg * 100
                    )
    ...
    def _trigger_alert(self, rule: AlertRule, metric: QualityMetric, violation: dict[str, Any]):
        """Trigger a new alert"""
        try:
            with self._lock:
    ...
    def _is_in_cooldown(self, rule: AlertRule, metric: QualityMetric) -> bool:
        ...
        try:
            ...
            recent_alerts = [
                ...
                if (
                    a.alert_rule_id == rule.rule_id
                    and a.dataset_id == metric.dataset_id
                    and a.metric_name == metric.metric_name
                    and a.triggered_at >= cooldown_time
                )
            ]
    ...
    def _get_historical_average(
        self, dataset_id: str, metric_name: str, hours: int = 24
    ) -> float | None:
    ...
    def _get_remediation_steps(self, rule: AlertRule, violation: dict[str, Any]) -> list[str]:
        ...
        try:
            with self._lock:
                ...
                for alert in self.active_alerts.values():
                    ...
                    escalation_time = alert.triggered_at + timedelta(
                        minutes=rule.escalation_minutes
                    )
                ...
                for alert_id, alert in self.active_alerts.items():
                    ...
                    if current_time - alert.triggered_at > stale_threshold:
                        ...
                        recent_metrics = [
                            ...
                            for m in self.metrics_buffer
                            if (
                                m.dataset_id == alert.dataset_id
                                and m.metric_name == alert.metric_name
                                and m.timestamp >= current_time - timedelta(hours=1)
                            )
                        ]
    ...
    def _calculate_dataset_statistics(self) -> dict[str, int]:
        ...
        try:
            ...
            for dataset_id, metrics in datasets.items():
                ...
                quality_scores = [
                    m.metric_value for m in metrics if "quality_score" in m.metric_name.lower()
                ]
                ...
                if quality_scores:
                    ...
                    dataset_alerts = [
                        a for a in self.active_alerts.values() if a.dataset_id == dataset_id
                    ]
    ...
    def _calculate_overall_quality_score(self) -> float:
        ...
        try:
            ...
            quality_metrics = [
                m.metric_value
                for m in self.metrics_buffer
                if (m.timestamp >= recent_time and "quality_score" in m.metric_name.lower())
            ]
    ...
    def _analyze_quality_trend(self) -> str:
        ...
        try:
            ...
            quality_metrics = [
                m
                for m in self.metrics_buffer
                if (m.timestamp >= last_24h and "quality_score" in m.metric_name.lower())
            ]
            ...
            second_half_avg = sum(m.metric_value for m in sorted_metrics[mid:]) / (
                len(sorted_metrics) - mid
            )
    ...
    def _get_quality_trend_data(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for hour, values in sorted(hourly_data.items()):
                trend_data.append(
                    {"timestamp": hour.isoformat(), "value": sum(values) / len(values)}
                )
    ...
    def _get_alert_trend_data(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            recent_alerts = [a for a in self.alert_history if a.triggered_at >= last_24h] + list(
            ...
            for i in range(24):
                ...
                trend_data.append({"timestamp": hour.isoformat(), "value": alert_count})
