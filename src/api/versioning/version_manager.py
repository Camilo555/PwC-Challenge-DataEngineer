...
class APIVersionManager:
    ...
    def __init__(self, strategy: VersioningStrategy = VersioningStrategy.PATH):
        ...
        self.content_type_pattern = re.compile(
    ...
    def validate_version(self, version: str) -> APIVersion:
        ...
        if version_info.deprecation_level == DeprecationLevel.OBSOLETE:
            raise HTTPException(
                status_code=410, detail=f"API version '{version}' is no longer supported"
            )
    ...
    def get_deprecation_headers(self, version: str) -> dict[str, str]:
        ...
        if version_info.deprecation_level in [DeprecationLevel.DEPRECATED, DeprecationLevel.SUNSET]:
            ...
            if version_info.deprecation_date:
                headers["Sunset"] = version_info.deprecation_date.strftime(
                    "%a, %d %b %Y %H:%M:%S GMT"
                )
    ...
    def create_versioned_route(
        self, app: FastAPI, path: str, methods: list[str], version_handlers: dict[str, Callable]
    ) -> None:
        ...
        for method in methods:
            app.add_api_route(
                path=path, endpoint=versioned_handler, methods=[method], tags=["versioned"]
            )
...
def create_enterprise_version_manager() -> APIVersionManager:
    ...
    manager.register_compatibility_rule(
        BackwardCompatibilityRule(
            from_version="1.0",
            to_version="2.0",
            field_mappings={"invoice_date": "transaction_date", "customer_country": "country"},
