...
class TimeFeatureTransformer(BaseFeatureTransformer):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "TimeFeatureTransformer":
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for date_col in self.date_columns:
            ...
            X_transformed[f"{date_col}_is_weekend"] = (
                X_transformed[date_col].dt.dayofweek.isin([5, 6]).astype(int)
            )

            # Cyclical features
            X_transformed[f"{date_col}_month_sin"] = np.sin(
                2 * np.pi * X_transformed[date_col].dt.month / 12
            )
            X_transformed[f"{date_col}_month_cos"] = np.cos(
                2 * np.pi * X_transformed[date_col].dt.month / 12
            )
            X_transformed[f"{date_col}_dayofweek_sin"] = np.sin(
                2 * np.pi * X_transformed[date_col].dt.dayofweek / 7
            )
            X_transformed[f"{date_col}_dayofweek_cos"] = np.cos(
                2 * np.pi * X_transformed[date_col].dt.dayofweek / 7
            )

            # Time since features
            reference_date = X_transformed[date_col].max()
            X_transformed[f"{date_col}_days_since"] = (
                reference_date - X_transformed[date_col]
            ).dt.days
...
class AggregationFeatureTransformer(BaseFeatureTransformer):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "AggregationFeatureTransformer":
            ...
            for agg_col in self.agg_columns:
                for func in self.agg_functions:
                    ...
                    if feature_name in self.reference_stats:
                        X_transformed[feature_name] = (
                            X_transformed[group_col]
                            .map(self.reference_stats[feature_name])
                            .fillna(0)
                        )
...
class InteractionFeatureTransformer(BaseFeatureTransformer):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "InteractionFeatureTransformer":
...
class FeatureEngineeringPipeline:
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "FeatureEngineeringPipeline":
        ...
        try:
            ...
            if categorical_features and self.config.categorical_encoding == "onehot":
                X_processed = pd.get_dummies(
                    X_processed, columns=categorical_features, drop_first=True, dummy_na=True
                )
            ...
            if self.config.scaling_method and numerical_features:
                ...
                if self.scaler:
                    numerical_features_current = [
                        col for col in numerical_features if col in X_processed.columns
                    ]
            ...
            if self.config.feature_selection_method and y is not None:
                if self.config.feature_selection_method == "kbest":
                    score_func = (
                        f_classif
                        if self.config.selection_score_func == "f_classif"
                        else f_regression
                    )
                    self.feature_selector = SelectKBest(
                        score_func=score_func, k=min(self.config.k_features, X_processed.shape[1])
                    )
            ...
            self.metrics_collector.increment_counter(
                "feature_engineering_fit_total", tags={"status": "success"}
            )
            self.metrics_collector.record_histogram(
                "feature_engineering_fit_duration_seconds", duration
            )
            ...
            self.metrics_collector.increment_counter(
                "feature_engineering_fit_total", tags={"status": "error"}
            )
            raise
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Collect metrics
            self.metrics_collector.increment_counter(
                "feature_engineering_transform_total", tags={"status": "success"}
            )
            self.metrics_collector.record_histogram(
                "feature_engineering_transform_duration_seconds", duration
            )
            ...
            self.metrics_collector.increment_counter(
                "feature_engineering_transform_total", tags={"status": "error"}
            )
            raise
    ...
    def detect_feature_drift(
        self, X_reference: pd.DataFrame, X_current: pd.DataFrame
    ) -> dict[str, float]:
        ...
        for col in common_columns:
            if X_reference[col].dtype in ["float64", "int64"]:
                ...
                js_div = 0.5 * self._kl_divergence(ref_probs, m) + 0.5 * self._kl_divergence(
                    curr_probs, m
                )
...
def create_sales_feature_pipeline() -> FeatureEngineeringPipeline:
    ...
    time_transformer = TimeFeatureTransformer(
        date_columns=["InvoiceDate", "created_at", "updated_at"], config=config
    )
...
class RFMFeatureTransformer(BaseFeatureTransformer):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "RFMFeatureTransformer":
        ...
        self.rfm_stats["frequency_quintiles"] = (
            rfm["Frequency"].quantile([0.2, 0.4, 0.6, 0.8]).tolist()
        )
        self.rfm_stats["monetary_quintiles"] = (
            rfm["Monetary"].quantile([0.2, 0.4, 0.6, 0.8]).tolist()
        )
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        rfm["F_Score"] = rfm["Frequency"].apply(
            lambda x: score_frequency_monetary(x, self.rfm_stats["frequency_quintiles"])
        )
        rfm["M_Score"] = rfm["Monetary"].apply(
            lambda x: score_frequency_monetary(x, self.rfm_stats["monetary_quintiles"])
        )
        ...
        X_transformed = X_transformed.merge(
            rfm_features, left_on=self.customer_id_col, right_index=True, how="left"
        )
...
class OutlierFeatureTransformer(BaseFeatureTransformer):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "OutlierFeatureTransformer":
            ...
            if self.method == "isolation_forest":
                ...
                if col in self.outlier_bounds:
                    ...
                    X_transformed[f"{col}_is_outlier"] = (
                        (X_transformed[col] < lower_bound) | (X_transformed[col] > upper_bound)
                    ).astype(int)

                    # Cap outliers
                    X_transformed[f"{col}_capped"] = X_transformed[col].clip(
...
class FeaturePipeline:
    ...
    def get_pipeline(self, name: str) -> FeatureEngineeringPipeline | None:
        """Get a registered pipeline."""
        return self.pipelines.get(name)

    async def execute_pipeline(
        self, pipeline_name: str, input_data: dict[str, Any], feature_names: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "features": transformed_df.to_dict("records")[0]
                if len(transformed_df) == 1
                else transformed_df.to_dict("records"),
    ...
    async def list_pipelines(self, status_filter: str | None = None) -> list[dict[str, Any]]:
    ...
    async def validate_pipeline(
        self, pipeline_name: str, sample_data: dict[str, Any] | None = None
    ) -> dict[str, Any]:
...
def create_comprehensive_sales_pipeline() -> FeatureEngineeringPipeline:
    ...
    rfm_transformer = RFMFeatureTransformer(
        customer_id_col="CustomerID", date_col="InvoiceDate", amount_col="TotalAmount"
    )
    ...
    outlier_transformer = OutlierFeatureTransformer(
        columns=["Quantity", "UnitPrice", "TotalAmount"], method="iqr", factor=1.5
    )
