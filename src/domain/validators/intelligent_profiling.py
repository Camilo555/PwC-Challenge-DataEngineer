...
class IntelligentDataProfiler:
    ...
    def profile_dataset(self, df: pd.DataFrame, dataset_name: str = None) -> DatasetProfile:
        ...
        self.logger.info(
            f"Starting dataset profiling: {dataset_name} ({df.shape[0]} rows, {df.shape[1]} columns)"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Dataset profiling completed in {profile.profiling_duration:.2f}s, quality score: {profile.dataset_quality_score:.2f}"
            )
    ...
    def _profile_column(self, series: pd.Series, column_name: str) -> ColumnProfile:
        ...
        profile = ColumnProfile(
            column_name=column_name, data_type=str(series.dtype), total_count=len(series)
        )
        ...
        try:
            ...
            if not numeric_series.empty:
                ...
                if self.profiling_level in [ProfileLevel.ADVANCED, ProfileLevel.COMPREHENSIVE]:
                    ...
                    profile.histogram = {
                        f"{bins[i]:.2f}-{bins[i + 1]:.2f}": int(hist[i]) for i in range(len(hist))
                    }
    ...
    def _profile_datetime_column(self, series: pd.Series, profile: ColumnProfile):
        ...
        try:
            ...
            if not dt_series.empty:
                ...
                # Date range analysis
                dt_series.max() - dt_series.min()
                profile.quality_metrics.timeliness_score = self._calculate_timeliness_score(
                    dt_series
                )
    ...
    def _calculate_format_consistency(self, series: pd.Series) -> float:
        ...
        try:
            if series.dtype == "object":
                ...
                lengths = str_series.str.len()
                length_consistency = (
                    (lengths == lengths.mode().iloc[0]).sum() / len(lengths)
                ...
                consistency = (
                    length_consistency * 0.4 + case_consistency * 0.3 + char_consistency * 0.3
                )
    ...
    def _detect_column_anomalies(
        self, series: pd.Series, profile: ColumnProfile
    ...
    def _detect_dataset_anomalies(self, df: pd.DataFrame, profile: DatasetProfile):
        ...
        try:
            ...
            if len(df) == 0:
                profile.dataset_anomalies.append(
                    AnomalyDetection(
    ...
    def _calculate_column_quality_metrics(
        self, series: pd.Series, profile: ColumnProfile
    ) -> DataQualityMetrics:
        ...
        try:
            ...
            if profile.inferred_type == DataType.EMAIL:
                email_valid = (
                    series.dropna().apply(lambda x: bool(self.email_pattern.match(str(x)))).mean()
                )
            ...
            if profile.inferred_type == DataType.DATETIME:
                metrics.timeliness_score = self._calculate_timeliness_score(
    ...
    def _calculate_dataset_quality_score(self, profile: DatasetProfile) -> float:
        ...
        try:
            ...
            column_scores = [
                col.quality_metrics.overall_score for col in profile.column_profiles.values()
            ]
        ...
        try:
            ...
            if profile.null_percentage > 30:
                recommendations.append(
                    f"Consider null value handling strategy for {profile.column_name}"
                )
            ...
            if profile.inferred_type in [DataType.TEXT_SHORT, DataType.TEXT_LONG]:
                if profile.avg_length and profile.avg_length > 1000:
                    recommendations.append(
                        f"Consider text compression or truncation for {profile.column_name}"
                    )
            ...
            for anomaly in profile.detected_anomalies:
                if anomaly.severity in ["high", "critical"]:
                    recommendations.append(
                        f"Address {anomaly.anomaly_type.value} in {profile.column_name}"
                    )
    ...
    def _generate_dataset_recommendations(self, profile: DatasetProfile) -> list[str]:
        ...
        try:
            ...
            if profile.dataset_quality_score < 0.7:
                recommendations.append(
                    "Dataset quality is below acceptable threshold - comprehensive review needed"
                )
            ...
            if problematic_columns:
                recommendations.append(
                    f"Focus on improving quality of columns: {', '.join(problematic_columns[:5])}"
                )

            # Correlation insights
            high_correlations = [
                (cols, corr) for cols, corr in profile.correlations.items() if abs(corr) > 0.9
            ]
            ...
            # Anomaly insights
            critical_anomalies = [
                anomaly for anomaly in profile.dataset_anomalies if anomaly.severity == "critical"
            ]
        ...
        sample_data = pd.DataFrame(
            {
                ...
                "purchase_date": pd.to_datetime(
                    ["2024-01-15", "2024-01-16", "2023-12-01", "2024-01-17", None, "2024-01-18"]
                ),
                "description": [
                    "Short text",
                    "This is a much longer description with many more characters to test text length analysis",
                    "Medium length text here",
                    "Another short one",
                    "Long text with lots of details about the product and its features and benefits",
                    "Brief",
                ],
                "phone": [
                    "+1-555-123-4567",
