...
class DataDogDatabaseMonitor:
    ...
    async def monitor_elasticsearch(self, es_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            for _node_id, node_data in node_stats['nodes'].items():
            ...
            for _index_name, index_data in index_stats['indices'].items():
    ...
    async def _check_elasticsearch_alerts(self, es_name: str, metrics: dict[str, Any], tags: list[str]):
        ...
        self.performance_baselines['elasticsearch']

        # Check cluster status
        if metrics.get('cluster_status') == 'red':
            statsd.event(
                title=f"Elasticsearch Cluster Status Alert - {es_name}",
                text="Cluster status is RED - immediate attention required",
                alert_type='error',
                tags=tags
            )
            ...
            statsd.event(
                ...
                text="Cluster status is YELLOW - monitoring required",
                alert_type='warning',
                tags=tags
            )
