...
from core.security.advanced_security import ActionType, get_security_manager
...
class EnhancedAuthenticationService:
        ...
        try:
            ...
            if token in self.revoked_tokens:
                return AuthenticationResult(
                    success=False, error_message="Token has been revoked", risk_score=8.0
                )
            ...
            try:
                ...
                return AuthenticationResult(
                    success=False, error_message="Token has expired", risk_score=3.0
                )
            except jwt.InvalidTokenError as e:
                return AuthenticationResult(
                    success=False, error_message=f"Invalid token: {str(e)}", risk_score=7.0
                )
            ...
            security_checks = await self._perform_token_security_checks(
                token_claims, source_ip, token
            )
            ...
            return AuthenticationResult(
                success=False, error_message=f"Authentication failed: {str(e)}", risk_score=5.0
            )

    async def authenticate_api_key(
        self, api_key: str, source_ip: str = None
        ...
        try:
            ...
            if not api_key.startswith("pwc_"):
                return AuthenticationResult(
                    success=False, error_message="Invalid API key format", risk_score=6.0
                )
            ...
            for _key_id, key_info in self.api_keys.items():
            ...
            if not api_key_info:
                ...
                return AuthenticationResult(
                    success=False, error_message="Invalid API key", risk_score=7.0
                )

            # Check if API key is enabled
            if not api_key_info.enabled:
                return AuthenticationResult(
                    success=False, error_message="API key is disabled", risk_score=5.0
                )

            # Check expiration
            if api_key_info.expires_at and datetime.now() > api_key_info.expires_at:
                return AuthenticationResult(
                    success=False, error_message="API key has expired", risk_score=4.0
                )
            ...
            if api_key_info.rate_limit:
                if not await self._check_api_key_rate_limit(api_key_info, source_ip):
                    return AuthenticationResult(
                        success=False, error_message="API key rate limit exceeded", risk_score=6.0
                    )
            ...
            return AuthenticationResult(
                ...
                metadata={"key_id": api_key_info.key_id, "authentication_method": "api_key"},
            )
    ...
    async def authenticate_oauth2(
        self, authorization_code: str, redirect_uri: str, client_id: str, source_ip: str = None
    ) -> AuthenticationResult:
        """Authenticate using OAuth2 authorization code flow"""

        try:
            # Exchange authorization code for access token
            token_data = await self._exchange_oauth2_code(
                authorization_code, redirect_uri, client_id
            )
            ...
            permissions = await self._determine_oauth2_permissions(
                user_info, token_data.get("scope", "")
            )
            ...
            token_claims = TokenClaims(
                ...
                metadata={"oauth2_provider": client_id, "oauth2_user_info": user_info},
            )
    ...
    async def create_jwt_token(
        self, token_claims: TokenClaims, expires_in: int | None = None
    ) -> str:
    ...
    async def create_api_key(
        ...
        try:
            ...
            api_key_info = ApiKeyInfo(
                ...
                metadata={"created_by": user_id, "creation_method": "api"},
            )
    ...
    async def _perform_token_security_checks(
        self, token_claims: TokenClaims, source_ip: str, token: str
    ) -> dict[str, Any]:
        """Perform additional security checks on token"""

        checks = {"additional_risk": 0.0, "anomalies": [], "recommendations": []}
    ...
    async def _check_api_key_rate_limit(self, api_key_info: ApiKeyInfo, source_ip: str) -> bool:
        ...
        self.auth_attempts[key] = [
            attempt for attempt in self.auth_attempts[key] if attempt >= cutoff
        ]
    ...
    async def _exchange_oauth2_code(
        self, authorization_code: str, redirect_uri: str, client_id: str
    ) -> dict[str, Any] | None:
    ...
    async def _get_oauth2_user_info(self, access_token: str) -> dict[str, Any] | None:
    ...
    async def _determine_oauth2_permissions(
        self, user_info: dict[str, Any], scopes: str
    ) -> list[str]:
