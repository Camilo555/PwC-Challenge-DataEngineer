...
def ingest_raw_data(**context):
    ...
    try:
        ...
        if initial_count != final_count:
            logger.warning(
                f"Removed {initial_count - final_count} records with missing invoice numbers"
            )
        ...
        bronze_path.mkdir(parents=True, exist_ok=True)
        output_file = (
            bronze_path / f"error_recovery_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.parquet"
        )
        df.to_parquet(output_file, index=False)
...
def process_bronze_to_silver(**context):
    ...
    df = (
        df.dropna(subset=["InvoiceNo", "StockCode"])
        if "InvoiceNo" in df.columns
        else df.dropna(subset=["invoice_no", "stock_code"])
    )
