...
from monitoring.datadog_apm_middleware import add_custom_metric, add_custom_tags, trace_function
...
from monitoring.datadog_error_tracking import get_error_tracker
...
class StreamingAPMTracker:
    ...
    def __init__(
        self,
        service_name: str = "streaming-platform",
        ...
        self.distributed_tracer = get_distributed_tracer(
            f"{service_name}-streaming-tracing", datadog_monitoring
        )
        self.error_tracker = get_error_tracker(
            f"{service_name}-streaming-errors", datadog_monitoring
        )
        self.profiler = get_datadog_profiler(
            f"{service_name}-streaming-profiler", datadog_monitoring
        )
        self.business_metrics = get_business_metrics_tracker(
            f"{service_name}-streaming-business", datadog_monitoring
        )

        self.logger.info(f"Enhanced Streaming APM tracker initialized for {service_name}")

    @asynccontextmanager
    async def trace_streaming_operation(
        self,
        operation_type: str,
        component: str,
        operation_name: str,
        topic_or_queue: str | None = None,
        ...
        with tracer.trace(
            ...
            try:
                ...
                self.logger.error(
                    f"Streaming operation failed: {component}.{operation_type}.{operation_name} - {str(e)}"
                )
                raise
                ...
                if self.datadog_monitoring:
                    await self._send_streaming_operation_metrics(
                        operation_type,
                        component,
                        operation_name,
                        duration,
    ...
    async def _send_streaming_operation_metrics(
        self,
        operation_type: str,
        component: str,
        operation_name: str,
        ...
        try:
            ...
            self.datadog_monitoring.histogram(
                "streaming.operations.duration", duration * 1000, tags=tags
            )
    ...
    async def trace_kafka_producer(
        self, topic: str, partition: int | None = None, producer_id: str | None = None
    ):
        """Context manager for tracing Kafka producer operations."""

        metadata = {"partition": partition, "producer_id": producer_id or "default"}
    ...
    @trace_function(
        operation_name="streaming.kafka.message_send", service_name="streaming-platform"
    )
    async def trace_kafka_message_send(
        self,
        topic: str,
        partition: int,
        offset: int,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.histogram(
                "streaming.kafka.message_size", message_size, tags=tags
            )
    ...
    async def trace_kafka_consumer(
        self, topic: str, consumer_group: str, consumer_id: str | None = None
    ):
        """Context manager for tracing Kafka consumer operations."""

        metadata = {"consumer_group": consumer_group, "consumer_id": consumer_id or "default"}
    ...
    @trace_function(
        operation_name="streaming.kafka.message_consume", service_name="streaming-platform"
    )
    async def trace_kafka_message_consume(
        self,
        topic: str,
        partition: int,
        offset: int,
        ...
        if self.datadog_monitoring:
            tags = [f"topic:{topic}", f"partition:{partition}", f"consumer_group:{consumer_group}"]
            ...
            if processing_time_ms is not None:
                self.datadog_monitoring.histogram(
                    "streaming.kafka.processing_time", processing_time_ms, tags=tags
                )
    ...
    @trace_function(
        operation_name="streaming.kafka.batch_process", service_name="streaming-platform"
    )
    async def trace_kafka_batch_processing(
        self,
        topic: str,
        consumer_group: str,
        batch_size: int,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.histogram(
                "streaming.kafka.batch_processing_time", processing_time_ms, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.kafka.batch_success_rate", success_rate, tags=tags
            )
            self.datadog_monitoring.gauge("streaming.kafka.throughput_mps", throughput, tags=tags)

            if error_count > 0:
                self.datadog_monitoring.counter(
                    "streaming.kafka.batch_errors", error_count, tags=tags
                )
    ...
    async def trace_rabbitmq_publisher(
        self, exchange: str, routing_key: str, queue: str | None = None
    ):
        """Context manager for tracing RabbitMQ publisher operations."""

        metadata = {"exchange": exchange, "routing_key": routing_key, "queue": queue}
    ...
    @trace_function(
        operation_name="streaming.rabbitmq.message_publish", service_name="streaming-platform"
    )
    async def trace_rabbitmq_message_publish(
        self,
        exchange: str,
        routing_key: str,
        message_size: int,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.histogram(
                "streaming.rabbitmq.message_size", message_size, tags=tags
            )

            if priority is not None:
                priority_tags = tags + [f"priority:{priority}"]
                self.datadog_monitoring.counter(
                    "streaming.rabbitmq.priority_messages", tags=priority_tags
                )
    ...
    async def trace_rabbitmq_consumer(
        self, queue: str, exchange: str, consumer_tag: str | None = None
    ):
        """Context manager for tracing RabbitMQ consumer operations."""

        metadata = {"exchange": exchange, "consumer_tag": consumer_tag or "default"}
    ...
    @trace_function(
        operation_name="streaming.rabbitmq.message_consume", service_name="streaming-platform"
    )
    async def trace_rabbitmq_message_consume(
        self,
        queue: str,
        exchange: str,
        routing_key: str,
        ...
        if self.datadog_monitoring:
            ...
            if processing_time_ms is not None:
                self.datadog_monitoring.histogram(
                    "streaming.rabbitmq.processing_time", processing_time_ms, tags=tags
                )
    ...
    async def trace_stream_processor(
        self,
        processor_name: str,
        processor_type: str,
    ...
    @trace_function(
        operation_name="streaming.processor.message_transform", service_name="streaming-platform"
    )
    async def trace_stream_transformation(
        self,
        processor_name: str,
        ...
        throughput = (
            input_message_count / (transformation_time_ms / 1000)
            if transformation_time_ms > 0
            else 0
        )
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "streaming.processor.input_messages", input_message_count, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.processor.output_messages", output_message_count, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.processor.message_ratio", message_ratio, tags=tags
            )
            self.datadog_monitoring.histogram(
                "streaming.processor.transformation_time", transformation_time_ms, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.processor.throughput_mps", throughput, tags=tags
            )
    ...
    @trace_function(
        operation_name="streaming.processor.aggregation", service_name="streaming-platform"
    )
    async def trace_stream_aggregation(
        self,
        processor_name: str,
        ...
        if self.datadog_monitoring:
            ...
            self.datadog_monitoring.gauge(
                "streaming.aggregation.records_aggregated", records_aggregated, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.aggregation.output_records", output_records, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.aggregation.ratio", aggregation_ratio, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.aggregation.window_count", window_count, tags=tags
            )
    ...
    @trace_function(
        operation_name="streaming.analytics.real_time_calculation",
        service_name="streaming-platform",
    )
    async def trace_realtime_analytics(
        self,
        analytics_type: str,
        ...
        calculation_throughput = (
            data_points / (calculation_time_ms / 1000) if calculation_time_ms > 0 else 0
        )
        ...
        if self.datadog_monitoring:
            tags = [f"analytics_type:{analytics_type}", f"calculation:{calculation_name}"]

            self.datadog_monitoring.gauge("streaming.analytics.data_points", data_points, tags=tags)
            self.datadog_monitoring.histogram(
                "streaming.analytics.calculation_time", calculation_time_ms, tags=tags
            )
            self.datadog_monitoring.gauge(
                "streaming.analytics.throughput", calculation_throughput, tags=tags
            )

            if result_value is not None:
                self.datadog_monitoring.gauge(
                    f"streaming.analytics.result.{calculation_name}", result_value, tags=tags
                )
    ...
    def get_streaming_performance_summary(self) -> dict[str, Any]:
        ...
        kafka_summary = {
            ...
            "message_balance": self.kafka_metrics["messages_produced"]
            - self.kafka_metrics["messages_consumed"],
        }
        ...
        rabbitmq_summary = {
            ...
            "message_balance": self.rabbitmq_metrics["messages_published"]
            - self.rabbitmq_metrics["messages_consumed"],
        }
        ...
        for group_name, group_data in self.consumer_groups.items():
            ...
            current_lags = [
                lag_info["lag"]
                for lag_info in group_data["lag_monitoring"].values()
            ...
            consumer_group_summary[group_name] = {
                ...
                "last_commit_minutes_ago": (
                    current_time - group_data["last_commit"]
                ).total_seconds()
                / 60,
            }
        ...
        for processor_id, processor_data in self.stream_processors.items():
            duration_hours = (
                datetime.utcnow() - processor_data["start_time"]
            ).total_seconds() / 3600
            throughput = (
                processor_data["messages_processed"] / duration_hours if duration_hours > 0 else 0
            )
            error_rate = (
                processor_data["errors"] / processor_data["messages_processed"]
                if processor_data["messages_processed"] > 0
                else 0
            )
        ...
        return {
            ...
            "system_health": {
                ...
                "overall_message_throughput": (
                    kafka_summary["messages_produced"]
                    + kafka_summary["messages_consumed"]
                    + rabbitmq_summary["messages_published"]
                    + rabbitmq_summary["messages_consumed"]
                ),
            },
        }
...
_streaming_apm_tracker: StreamingAPMTracker | None = None


def get_streaming_apm_tracker(
    service_name: str = "streaming-platform", datadog_monitoring: DatadogMonitoring | None = None
) -> StreamingAPMTracker:
...
async def trace_kafka_producer_session(
    topic: str, partition: int | None = None, producer_id: str | None = None
):
...
async def trace_kafka_consumer_session(
    topic: str, consumer_group: str, consumer_id: str | None = None
):
...
async def trace_rabbitmq_publisher_session(
    exchange: str, routing_key: str, queue: str | None = None
):
...
async def trace_rabbitmq_consumer_session(
    queue: str, exchange: str, consumer_tag: str | None = None
):
...
async def trace_stream_processor_session(
    processor_name: str, processor_type: str, input_sources: list[str], output_sinks: list[str]
):
    """Convenience function for tracing stream processor operations."""
    tracker = get_streaming_apm_tracker()
    return await tracker.trace_stream_processor(
        processor_name, processor_type, input_sources, output_sinks
    )
