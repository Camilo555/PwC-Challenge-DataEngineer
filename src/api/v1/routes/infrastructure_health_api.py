...
async def get_component_health(
    ...
    try:
        await rate_limiter.check_rate_limit(
            f"component_health_{current_user.get('id', 'anonymous')}"
        )
...
async def get_alerts(
    ...
    state: AlertState | None = Query(None, description="Filter by state"),
...
async def get_dashboard_data(
    ...
    try:
        ...
        time_range = {"start": datetime.now() - timedelta(hours=hours), "end": datetime.now()}
...
async def run_load_test(
    ...
    try:
        ...
        if request.target_component not in health_manager.monitors:
            raise HTTPException(
                status_code=404, detail=f"Component {request.target_component} not found"
            )
