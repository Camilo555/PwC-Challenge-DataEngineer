...
from .base import BaseMLComponent, MLConfig, MLExperiment, ModelMetrics
...
class ModelMetadata(BaseModel):
    ...
    validation_metrics: ModelMetrics | None = Field(default=None, description="Validation metrics")
    ...
    target_names: list[str] | None = Field(default=None)
    ...
    average_prediction_time: float | None = Field(default=None)
    model_size_mb: float | None = Field(default=None)
...
class ModelRegistry:
    ...
    def save_registry(self) -> None:
        ...
        try:
            registry_data = {
                model_id: metadata.dict() for model_id, metadata in self.models.items()
            }
...
class ModelManager(BaseMLComponent):
    ...
    def predict(self, model_id: str, data: pd.DataFrame | np.ndarray) -> np.ndarray:
        ...
        if metadata:
            ...
            if metadata.average_prediction_time:
                new_avg_time = (
                    (metadata.average_prediction_time * metadata.prediction_count) + prediction_time
                ) / new_count
            else:
    ...
    def validate_model(
        self, model_id: str, validation_data: pd.DataFrame, validation_target: pd.Series
    ) -> ModelMetrics:
        ...
        validation_metrics = self._calculate_metrics(
            validation_target, predictions, self.registry.get_model(model_id).model_type
        )

        # Update model with validation metrics
        self.registry.update_model(
            model_id, validation_metrics=validation_metrics, status=ModelStatus.VALIDATED
        )
    ...
    def get_model_performance_report(self, model_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                "validation_metrics": metadata.validation_metrics.to_dict()
            ...
            "configuration": {
                ...
                "features": metadata.feature_names[:10]
                if len(metadata.feature_names) > 10
                else metadata.feature_names,
            },
        }
    ...
    def fit(self, data: pd.DataFrame | np.ndarray, **kwargs) -> "ModelManager":
