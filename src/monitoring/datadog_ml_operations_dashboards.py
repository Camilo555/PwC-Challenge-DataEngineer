...
class MLMetric:
    ...
    model_types: list[str] | None = None  # classification, regression, clustering, etc.
...
class ModelDefinition:
    ...
    expected_performance: dict[str, float] | None = None
...
class DataDogMLOperationsDashboards:
    ...
    def __init__(
        self,
        service_name: str = "ml-operations",
        datadog_monitoring: DatadogMonitoring | None = None,
        dashboard_manager: DataDogDashboardManager | None = None,
        business_metrics: DataDogBusinessMetricsTracker | None = None,
    ):
    ...
    def _initialize_dashboard_definitions(self):
        ...
        self.dashboard_definitions[MLDashboardType.MODEL_DRIFT] = {
            ...
            "widgets": [
                ...
                {"type": "alert_list", "title": "Drift Alerts", "query": "tag:drift_detected"},
            ],
        }
    ...
    async def create_ml_dashboard(
        self,
        dashboard_type: MLDashboardType,
    ...
    async def create_all_ml_dashboards(self) -> dict[MLDashboardType, str | None]:
        ...
        self.logger.info(
            f"ML dashboard creation completed: {success_count}/{total_count} successful"
        )

        # Track overall success
        if self.datadog_monitoring:
            self.datadog_monitoring.gauge(
                "ml.dashboards.created_total", success_count, tags=[f"service:{self.service_name}"]
            )
        ...
        try:
            ...
            if self.datadog_monitoring:
                ...
                for metric_name, value in metrics.items():
                    self.datadog_monitoring.gauge(f"ml.model.{metric_name}", value, tags=tags)
    ...
    async def _check_model_performance_thresholds(
        self, model: ModelDefinition, metrics: dict[str, float]
    ):
        ...
        try:
            for metric_name, value in metrics.items():
                if metric_name in self.ml_metrics:
                    ...
                    if model.expected_performance and metric_name in model.expected_performance:
    ...
    async def track_model_drift(
        ...
        try:
            ...
            if self.datadog_monitoring:
                ...
                # Track drift score
                self.datadog_monitoring.gauge("ml.drift.score", drift_score, tags=tags)

                # Track drift detection events
                if is_drift_detected:
                    self.datadog_monitoring.counter("ml.drift.detected", tags=tags)

                    # Alert on critical drift
                    if drift_score > 0.5:  # Critical drift threshold
                        self.datadog_monitoring.counter("ml.drift.critical", tags=tags)
        ...
        try:
            ...
            if variant_data["users"] > 0:
                variant_data["conversion_rate"] = (
                    variant_data["conversions"] / variant_data["users"]
                )
            ...
            if self.datadog_monitoring:
                ...
                self.datadog_monitoring.gauge(
                    "ml.experiments.conversion_rate", variant_data["conversion_rate"], tags=tags
                )

                if conversion_value:
                    self.datadog_monitoring.histogram(
                        "ml.experiments.conversion_value", conversion_value, tags=tags
                    )
                ...
                if total_variants >= 2 and experiment["total_users"] >= 100:
                    ...
                    self.datadog_monitoring.gauge(
                        "ml.experiments.significance", significance, tags=tags
                    )
    ...
    async def track_feature_quality(
        self, feature_name: str, quality_metrics: dict[str, float], pipeline: str | None = None
    ) -> bool:
        ...
        try:
            ...
            if len(self.feature_quality_scores[feature_name]) > 1000:
                self.feature_quality_scores[feature_name] = self.feature_quality_scores[
                    feature_name
                ][-1000:]

            # Send to DataDog
            if self.datadog_monitoring:
                tags = [f"feature_name:{feature_name}", f"service:{self.service_name}"]
                ...
                for metric_name, value in quality_metrics.items():
                    self.datadog_monitoring.gauge(
                        f"ml.features.quality.{metric_name}", value, tags=tags
                    )

                # Calculate overall quality score
                overall_score = sum(quality_metrics.values()) / len(quality_metrics)
                self.datadog_monitoring.gauge(
                    "ml.features.quality.overall_score", overall_score, tags=tags
                )
    ...
    def get_ml_operations_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            for _model_id, predictions in self.model_predictions.items():
            ...
            recent_drifts = [
                d
                for d in self.drift_detections
            ...
            summary = {
                ...
                "drift_monitoring": {
                    ...
                    "models_with_drift": len(
                        {d["model_id"] for d in recent_drifts if d["drift_detected"]}
                    ),
                },
                "experiments": {
                ...
                "feature_quality": {"features_monitored": features_monitored},
                "dashboards": {
...
_ml_operations_dashboards: DataDogMLOperationsDashboards | None = None


def get_ml_operations_dashboards(
    service_name: str = "ml-operations",
    datadog_monitoring: DatadogMonitoring | None = None,
    dashboard_manager: DataDogDashboardManager | None = None,
    business_metrics: DataDogBusinessMetricsTracker | None = None,
) -> DataDogMLOperationsDashboards:
...
async def track_model_performance(
    model_id: str, metrics: dict[str, float], model_version: str | None = None
) -> bool:
...
async def track_model_drift(
    model_id: str, drift_type: DriftType, drift_score: float, feature_name: str | None = None
) -> bool:
...
async def track_ab_experiment(
    experiment_id: str, variant: str, conversion_type: str, conversion_value: float | None = None
) -> bool:
    """Convenience function for tracking A/B experiments"""
    ml_dashboards = get_ml_operations_dashboards()
    return await ml_dashboards.track_ab_experiment(
        experiment_id, variant, conversion_type, conversion_value
    )


async def create_all_ml_dashboards() -> dict[MLDashboardType, str | None]:
