...
from src.ml.deployment.model_server import ModelMetadata, ModelRegistry, ModelServer
...
class DeploymentConfig:
    ...
    training_config: ModelConfig | None = None
...
class DeploymentPipeline:
    ...
    async def deploy_model(
        ...
        try:
            logger.info(
                f"Starting deployment pipeline: {config.model_name} v{config.model_version}"
            )
            ...
            logger.info(
                f"Deployment pipeline completed: {config.model_name} v{config.model_version}"
            )

            # Record metrics
            self.metrics_collector.increment_counter(
                "ml_deployment_completed_total",
                tags={"model_name": config.model_name, "model_type": training_result.model_type},
            )
            self.metrics_collector.record_histogram(
                "ml_deployment_duration_seconds", deployment_result["duration_seconds"]
            )
            ...
            self.metrics_collector.increment_counter(
                ...
                tags={"model_name": config.model_name, "error_type": type(e).__name__},
            )
    ...
    async def _run_quality_gates(
        self, model_id: str, training_result: TrainingResult, config: DeploymentConfig
    ) -> dict[str, Any]:
        ...
        try:
            ...
            model_file_check = {
                ...
                "passed": Path(training_result.model_path).exists()
            ...
            return {"passed": passed, "checks": checks, "timestamp": datetime.utcnow()}
    ...
    async def _test_model_loading(self, model_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not load_success:
                return {"name": "model_loading", "passed": False, "error": "Failed to load model"}
            ...
            try:
                ...
                return {"name": "model_loading", "passed": True, "test_prediction": str(prediction)}
    ...
    async def _setup_ab_testing(self, model_id: str, config: DeploymentConfig) -> dict[str, Any]:
        ...
        try:
            if not config.control_model_id:
                ...
                return {"success": False, "error": "No control model specified for A/B testing"}
    ...
    async def _deploy_to_production(
        self, model_id: str, config: DeploymentConfig
    ) -> dict[str, Any]:
    ...
    async def promote_model(self, experiment_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            early_stopping_check = (
                await self.ab_testing_framework.check_experiment_for_early_stopping(experiment_id)
            )
            ...
            for result_key, result_data in analysis.get("results", {}).items():
                if (
                    ...
                    and result_data.get("treatment_mean", 0) > best_score
                ):
            ...
            if not best_model_id:
                return {"success": False, "reason": "No model showed significant improvement"}
...
class MLOpsOrchestrator:
    ...
    async def train_and_deploy(
        self, data: pd.DataFrame, target_column: str, deployment_config: DeploymentConfig
    ) -> dict[str, Any]:
        ...
        try:
            ...
            trainer_config = deployment_config.training_config or ModelConfig(
                ...
                primary_metric="f1"
                if deployment_config.problem_type == "classification"
                else "rmse",
                n_trials=50,
            )
            ...
            if not training_results:
                return {"success": False, "error": "No models were successfully trained"}
            ...
            deployment_result = await self.deployment_pipeline.deploy_model(
                training_result=best_result, config=deployment_config
            )
    ...
    async def get_system_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"timestamp": datetime.utcnow(), "overall_status": "unhealthy", "error": str(e)}
...
def create_deployment_pipeline(
    ...
    ab_testing_framework: ABTestingFramework | None = None,
) -> DeploymentPipeline:
