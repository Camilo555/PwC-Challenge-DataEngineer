...
class RealtimeAggregationEngine:
    ...
    def __init__(
        self,
        spark: SparkSession,
        config: GoldProcessingConfig,
        cache_manager: EventCache | None = None,
        messaging_manager: HybridMessagingArchitecture | None = None,
    ):
        self.spark = spark
        self.config = config
    ...
    def create_aggregation_stream(
        self, df: DataFrame, aggregation_name: str, use_cache: bool = True
    ) -> DataFrame:
        ...
        try:
            ...
            watermarked_df = filtered_df.withWatermark(
                agg_def.watermark_column, agg_def.watermark_delay
            )

            # Create aggregation based on type
            if agg_def.aggregation_type == AggregationType.TUMBLING_WINDOW:
                result_df = self._create_tumbling_window_aggregation(watermarked_df, agg_def)
            elif agg_def.aggregation_type == AggregationType.SLIDING_WINDOW:
                result_df = self._create_sliding_window_aggregation(watermarked_df, agg_def)
            elif agg_def.aggregation_type == AggregationType.SESSION_WINDOW:
                result_df = self._create_session_window_aggregation(watermarked_df, agg_def)
            elif agg_def.aggregation_type == AggregationType.GLOBAL_AGGREGATE:
                result_df = self._create_global_aggregation(watermarked_df, agg_def)
            else:
                raise ValueError(f"Unsupported aggregation type: {agg_def.aggregation_type}")

            # Add metadata
            result_df = (
                result_df.withColumn("aggregation_name", lit(aggregation_name))
    ...
    def _create_tumbling_window_aggregation(
        self, df: DataFrame, agg_def: AggregationDefinition
    ) -> DataFrame:
    ...
    def _create_sliding_window_aggregation(
        ...
        group_cols = [
            window(col(agg_def.watermark_column), agg_def.window_duration, agg_def.slide_duration)
        ]
        group_cols.extend([col(c) for c in agg_def.group_by_columns])
    ...
    def _create_session_window_aggregation(
        ...
        group_cols = [session_window(col(agg_def.watermark_column), agg_def.window_duration)]
        group_cols.extend([col(c) for c in agg_def.group_by_columns])
    ...
    def check_alert_conditions(
        self, metrics_df: DataFrame, use_cache: bool = True
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in metrics_data:
                ...
                for rule_id, alert_rule in self.alert_rules.items():
                    ...
                    if condition_met:
                        alert_id = (
                            f"{rule_id}_{alert_rule.metric_name}_{int(datetime.now().timestamp())}"
                        )
...
class RealtimeGoldProcessor:
    ...
    def __init__(self, spark: SparkSession, config: GoldProcessingConfig = None):
        self.spark = spark
        ...
        self.aggregation_engine = RealtimeAggregationEngine(
            spark, config, self.cache_manager, self.messaging_manager
        )
    ...
    def start_all_aggregations(
        self, silver_stream: DataFrame, output_base_path: str, checkpoint_base_path: str
    ...
    def start_single_aggregation(
        ...
        try:
            ...
            query = (
                ...
                .foreachBatch(
                    lambda batch_df, batch_id: self._process_gold_batch(
    ...
    def _cache_gold_results(self, aggregation_name: str, batch_id: int, results: dict[str, Any]):
        ...
        try:
            if self.cache_manager and self.config.enable_cqrs:
                ...
                self.cache_manager.set(
                    cache_key, json.dumps(results), ttl=self.config.read_model_cache_ttl
                )
    ...
    def _send_gold_notification(
        self, event_type: str, aggregation_name: str, batch_id: int, data: dict[str, Any]
    ):
    ...
    def _process_gold_batch(
        self, batch_df: DataFrame, batch_id: int, aggregation_name: str, output_path: str
    ):
        ...
        try:
            ...
            self._send_gold_notification(
                "batch_started", aggregation_name, batch_id, {"record_count": batch_count}
            )

            # Write to Delta Lake with optimized partitioning
            writer = batch_df.write.format("delta").mode("append").option("mergeSchema", "true")
            ...
            if self.metrics_collector:
                self.metrics_collector.increment_counter(
                    "gold_aggregations_processed", {"aggregation_name": aggregation_name}
                )
            ...
            self.logger.debug(f"Batch {batch_id} for {aggregation_name} processed successfully")
            ...
            self._send_gold_notification("error", aggregation_name, batch_id, {"error": str(e)})
            raise
    ...
    def get_cached_results(self, aggregation_name: str, batch_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to get cached results for {aggregation_name}, batch {batch_id}: {e}"
            )
        return None
...
def create_realtime_gold_processor(
    spark: SparkSession, config: GoldProcessingConfig = None
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.appName("RealtimeGoldProcessor")
