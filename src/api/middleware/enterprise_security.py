...
from core.security.enhanced_access_control import (
    AccessDecision,
    AccessRequest,
...
class EnterpriseSecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self,
        app,
        config: SecurityOrchestrationConfig | None = None,
    ...
    async def _pre_request_security_check(
        ...
        try:
            ...
            context.update(
                {"blocked": True, "status_code": 500, "message": "Security check failed"}
            )
            return context
    ...
    async def _extract_user_identity(self, request: Request) -> str | None:
    ...
    async def _check_access_control(
        self, request: Request, user_id: str, context: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _perform_threat_detection(
        self, request: Request, context: dict[str, Any]
    ) -> dict[str, Any]:
        """Basic threat detection analysis"""

        threat_indicators = {"high_risk": False, "risk_score": 0.0, "indicators": []}
    ...
    async def _pre_compliance_check(
        self, request: Request, context: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _apply_dlp_processing(
        self, response_data: Any, request: Request, context: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _add_security_metadata(
        self, response: Response, security_context: dict[str, Any], correlation_id: str
    ) -> None:
    ...
    def _create_security_response(
        self, status_code: int, message: str, details: dict[str, Any], correlation_id: str
    ) -> JSONResponse:
...
class WebSocketSecurityMiddleware:
    ...
    def __init__(self, config: SecurityOrchestrationConfig | None = None):
        self.logger = get_logger(__name__)
        self.config = config or SecurityOrchestrationConfig()
        self.access_manager = get_access_control_manager()
        self.active_connections: dict[str, dict[str, Any]] = {}

    async def authenticate_websocket(self, websocket, token: str | None = None) -> dict[str, Any]:
    ...
    async def _validate_websocket_token(self, token: str) -> str | None:
    ...
    async def _check_websocket_access(self, user_id: str, websocket) -> dict[str, Any]:
        ...
        try:
            ...
            access_request = AccessRequest(
                ...
                context={"connection_type": "websocket", "endpoint": str(websocket.url)},
            )
            ...
            return {
                "allowed": evaluation.decision
                in [AccessDecision.ALLOW, AccessDecision.CONDITIONAL_ALLOW],
                "reason": evaluation.reason,
                "permissions": ["security_dashboard"]
...
_enterprise_security_middleware: EnterpriseSecurityMiddleware | None = None
_websocket_security_middleware: WebSocketSecurityMiddleware | None = None


def get_enterprise_security_middleware(
    config: SecurityOrchestrationConfig | None = None, exclude_paths: list[str] | None = None
