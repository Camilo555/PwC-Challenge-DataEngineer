...
class LogEntry:
    ...
    def to_datadog_format(self) -> dict[str, Any]:
        ...
        return {
            ...
            "logger": {"name": f"{self.service}.{self.source.value}", "level": self.level.value},
            "host": self.host,
            "service": self.service,
            "source": self.source.value,
            "env": self.environment,
            "version": self.version,
            "dd": {"trace_id": self.trace_id, "span_id": self.span_id} if self.trace_id else {},
            "usr": {"id": self.user_id, "session_id": self.session_id} if self.user_id else {},
            "network": {"client": {"ip": self.ip_address}} if self.ip_address else {},
            "http": {
            ...
            if any([self.status_code, self.url, self.user_agent, self.duration_ms])
            else {},
            "kubernetes": {"pod_name": self.kubernetes_pod, "namespace": self.kubernetes_namespace}
            if self.kubernetes_pod
            else {},
            "container": {"id": self.container_id} if self.container_id else {},
            "error": self.exception if self.exception else {},
            "custom": {**self.tags, **self.metadata},
        }
    ...
    async def _ship_to_datadog_http(self, logs: list[LogEntry]):
        ...
        try:
            ...
            response = requests.post(
                self.config.datadog_intake_url, json=formatted_logs, headers=headers, timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Failed to ship logs to DataDog: {response.status_code} - {response.text}"
                )
    ...
    async def _ship_to_local_storage(self, logs: list[LogEntry]):
        ...
        try:
            ...
            for log_entry in logs:
                ...
                # Write log entry
                log_file = (
                    storage_path / f"{log_entry.service}_{current_time.strftime('%Y%m%d')}.log"
                )
...
class ContainerLogCollector:
    ...
    async def _parse_container_log(self, log_line: str, container) -> LogEntry | None:
...
class KubernetesLogCollector:
        ...
        try:
            ...
            for container in pod.spec.containers:
                try:
                    ...
                    self.logger.debug(
                        f"Could not get logs for container {container.name} in pod {pod.metadata.name}: {e}"
                    )
    ...
    async def _parse_pod_log(self, log_line: str, pod, container_name: str) -> LogEntry | None:
...
class ApplicationLogCollector:
    ...
    async def _collect_application_logs(self) -> list[LogEntry]:
        ...
        try:
            ...
            for log_source in log_sources:
                ...
                if log_path.exists():
                    file_logs = await self._parse_log_file(
                        log_path, log_source["service"], log_source["source"]
                    )
    ...
    async def _parse_log_file(
        self, log_path: Path, service: str, source: LogSource
    ) -> list[LogEntry]:
    ...
    async def _parse_log_line(self, line: str, service: str, source: LogSource) -> LogEntry | None:
    ...
    async def _collection_cycle(self):
        ...
        try:
            ...
            self.metrics.histogram(
                "log_collection.cycle_duration",
                cycle_duration,
                tags=["collector:enterprise_datadog"],
            )
            self.metrics.gauge(
                "log_collection.buffer_size",
                self.log_buffer.qsize(),
                tags=["collector:enterprise_datadog"],
            )
            self.metrics.counter(
                "log_collection.logs_collected",
                len(filtered_logs),
                tags=["collector:enterprise_datadog"],
            )

            self.logger.debug(
                f"Collection cycle completed in {cycle_duration:.2f}s, collected {len(filtered_logs)} logs"
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if current_stats["start_time"]:
            current_stats["uptime_seconds"] = (
                datetime.now() - current_stats["start_time"]
            ).total_seconds()
...
# Factory function
def create_enterprise_log_collector(
    config: LogCollectionConfig | None = None,
) -> EnterpriseDataDogLogCollector:
