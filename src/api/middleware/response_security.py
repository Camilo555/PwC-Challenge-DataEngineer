...
class ComplianceFilter:
    ...
    def filter_for_gdpr(
        self, data: Any, user_consent: dict[str, bool] = None
    ) -> tuple[Any, list[str]]:
                ...
                if isinstance(value, str):
                    ...
                    if re.search(r"\b\+?[\d\s\-\(\)]{10,}\b", value):
                        if not user_consent.get("process_phone", False):
                            filtered_data[key] = re.sub(
                                r"\b\+?[\d\s\-\(\)]{10,}\b", "[PHONE_REDACTED]", value
                            )
                    ...
                    for pci_type, pattern in pci_patterns.items():
                        ...
                        for match in matches:
                            if pci_type == "credit_card":
                                ...
                                filtered_value = re.sub(
                                    pattern, f"[{pci_type.upper()}_REDACTED]", filtered_value
                                )
    ...
    def apply_compliance_filtering(
        self, data: Any, frameworks: list[str], user_context: dict[str, Any] = None
        ...
        try:
            for framework in frameworks:
                if framework.lower() == "gdpr":
                    filtered_data, redactions = self.filter_for_gdpr(
                        filtered_data, user_context.get("gdpr_consent", {})
                    )
            ...
            if len(all_redactions) > 0:
                ...
                compliance_metadata["compliance_score"] = max(
                    0.5, 1.0 - (len(all_redactions) * 0.1)
                )
...
class PIIRedactionEngine:
        ...
        try:
            ...
            if scan_result.get("detections", 0) > 0:
                redacted_data = self._apply_intelligent_redaction(
                    data, scan_result, redaction_level, user_permissions, preserve_format
                )
    ...
    def _recursive_redact(
        self, data: Any, patterns: dict[SensitiveDataType | str, str], show_partial: bool = True
    ) -> Any:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                for data_type, pattern in patterns.items():
                    if isinstance(key, str) and re.search(pattern, key, re.IGNORECASE):
                        redacted[
                            f"[REDACTED_{data_type.value.upper() if hasattr(data_type, 'value') else str(data_type).upper()}]"
                        ] = "[REDACTED]"
                        key_redacted = True
                        break
            ...
            for data_type, pattern in patterns.items():
                ...
                for match in reversed(matches):  # Reverse to maintain string positions
                    ...
                    redacted_str = (
                        redacted_str[: match.start()] + replacement + redacted_str[match.end() :]
                    )
    ...
    def _redact_email(self, email: str) -> str:
        ...
        if len(parts) == 2:
            ...
            domain = parts[1]
            redacted_username = (
                username[:2] + "*" * max(1, len(username) - 2) if len(username) > 2 else "***"
            )
...
class ResponseSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        try:
            ...
            secured_data, security_metadata = await self._apply_response_security(
                response_data, user_context, request
            )
    ...
    async def _apply_response_security(
        self, response_data: Any, user_context: dict[str, Any], request
    ) -> tuple[Any, dict[str, Any]]:
        ...
        try:
            ...
            redaction_level = await self._determine_redaction_level(
                processed_data, user_context, request
            )
            ...
            if self.enable_compliance_filtering:
                processed_data, compliance_redactions, compliance_metadata = (
                    self.compliance_filter.apply_compliance_filtering(
                        processed_data, self.compliance_frameworks, user_context
                    )
                )
            ...
            security_score = await self._calculate_security_score(
                redaction_metadata, security_metadata.get("compliance_filtering", {}), user_context
            )
    ...
    async def _determine_redaction_level(
        self, data: Any, user_context: dict[str, Any], request
    ) -> str:
    ...
    def _add_security_headers(
        self, response: Response, security_metadata: dict[str, Any], user_context: dict[str, Any]
    ):
        ...
        if "pii_redaction" in security_metadata:
            ...
            response.headers["X-PII-Redacted"] = str(
                redaction_meta.get("total_redactions", 0) > 0
            ).lower()
            response.headers["X-Redaction-Level"] = redaction_meta.get(
                "redaction_level", "standard"
            )

        if "compliance_filtering" in security_metadata:
            compliance_meta = security_metadata["compliance_filtering"]
            response.headers["X-Compliance-Applied"] = ",".join(
                compliance_meta.get("frameworks_applied", [])
            )
            response.headers["X-Compliance-Score"] = str(
                compliance_meta.get("compliance_score", 1.0)
            )
...
_response_security_middleware: ResponseSecurityMiddleware | None = None
