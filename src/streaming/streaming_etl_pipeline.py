        ...
        class MetricsListener:
            ...
            def on_progress(self, query_progress):
                try:
                    ...
                    if progress:
                        self.processor.metrics.records_processed += progress.get(
                            "inputRowsPerSecond", 0
                        )
                        self.processor.metrics.processing_rate = progress.get(
                            "inputRowsPerSecond", 0
                        )
...
class StreamingBronzeProcessor(StreamingETLProcessor):
    ...
    def transform_stream(self, df: DataFrame) -> DataFrame:
        ...
        try:
            transformed = (
                df.withColumn("bronze_id", expr("uuid()"))
    ...
    def _add_quality_flags(self, df: DataFrame) -> DataFrame:
        """Add data quality flags"""
        return (
            df.withColumn("is_valid_invoice", col("invoice_no").isNotNull())
...
class StreamingSilverProcessor(StreamingETLProcessor):
    ...
    def transform_stream(self, df: DataFrame) -> DataFrame:
        ...
        try:
            ...
            transformed = (
                cleaned_df.withColumn("silver_id", expr("uuid()"))
                .withColumn("total_amount", col("quantity") * col("unit_price"))
                .withColumn("processed_timestamp", current_timestamp())
                .withColumn(
                    "invoice_datetime", expr("to_timestamp(invoice_date, 'MM/dd/yyyy HH:mm')")
                )
    ...
    def _apply_business_rules(self, df: DataFrame) -> DataFrame:
        ...
        return (
            ...
            .withColumn("is_weekend", expr("dayofweek(invoice_datetime) in (1, 7)"))
        )
...
class StreamingETLOrchestrator:
    ...
    def _start_silver_from_delta(self) -> StreamingQuery:
        ...
        bronze_stream = (
            self.spark.readStream.format("delta").option("ignoreChanges", "true").load(bronze_path)
        )
    ...
    def _start_gold_from_delta(self) -> StreamingQuery:
        ...
        silver_stream = (
            self.spark.readStream.format("delta").option("ignoreChanges", "true").load(silver_path)
        )
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.appName("StreamingETLPipeline")
