...
class ValidationRule:
    ...
    def __init__(
        self, name: str, condition: Callable[[Any], bool], message: str, severity: str = "error"
    ):
        self.name = name
...
class DataCleaner(DataFrameTransformationStrategy):
    ...
    def transform(self, data: pd.DataFrame, **kwargs) -> TransformationResult:
        ...
        metadata = {"original_shape": data.shape, "cleaning_steps": [], "validation_results": {}}
...
def create_common_validation_rules() -> list[ValidationRule]:
    ...
    return [
        ValidationRule("not_null", lambda x: pd.notna(x), "Value cannot be null", "error"),
        ValidationRule(
            "positive_number",
