...
class MetricsCollector:
    ...
    def _setup_default_thresholds(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "consumer_lag": {"warning": 100, "critical": 500},
            "error_rate": {
                "warning": 0.05,  # 5%
                "critical": 0.10,  # 10%
            },
            ...
            for queue_type in QueueType:
                ...
                try:
                    ...
                    if "error" not in stats:
                        ...
                        if current_metrics:
                            time_diff = (
                                datetime.utcnow() - current_metrics.timestamp
                            ).total_seconds()
                            if time_diff > 0:
                                message_diff = (
                                    stats["message_count"] - current_metrics.message_count
                                )
        ...
        for metric_name, metric_value, metric_type in metrics_to_record:
            self._record_metric(
                MetricPoint(
                    ...
                    tags={"queue": queue_name, "component": "queue"},
                )
            )
    ...
    def record_message_processing_time(self, queue_name: str, processing_time_ms: float):
        ...
        self._record_metric(
            MetricPoint(
                ...
                tags={"queue": queue_name, "component": "message_processing"},
            )
        )
    ...
    def _check_alert_conditions(self):
        ...
        try:
            ...
            for _queue_name, metrics in self.queue_metrics.items():
    ...
    def get_current_metrics(self) -> dict[str, Any]:
        ...
        return {
            "queue_metrics": {
                name: metrics.to_dict() for name, metrics in self.queue_metrics.items()
            },
...
class PerformanceOptimizer:
    ...
    def _analyze_queue_performance(
        self, queue_name: str, queue_metrics: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        # Low consumer count
        if (
            consumer_count < self.optimization_thresholds["low_consumer_count"]
            and message_count > 0
        ):
...
class HealthCheckService:
    ...
    def _check_component_health(self, component: str) -> SystemHealthCheck:
        ...
        try:
            if component == "rabbitmq_connection":
                ...
                result = {
                    "status": "unknown",
                    "details": {"error": f"Unknown component: {component}"},
                }
    ...
    def _check_message_publishing(self) -> dict[str, Any]:
        ...
        try:
            ...
            if success:
                return {"status": "healthy", "details": {"publish_test": "success"}}
            else:
                return {"status": "degraded", "details": {"publish_test": "failed"}}
    ...
    def _check_message_consuming(self) -> dict[str, Any]:
        ...
        return {"status": "healthy", "details": {"consume_test": "simulated_success"}}
    ...
    def _check_queue_operations(self) -> dict[str, Any]:
        ...
        try:
            ...
            if "error" not in stats:
                ...
                return {"status": "degraded", "details": {"error": stats["error"]}}
