...
from monitoring.security_metrics import get_security_metrics_collector
...
class ComplianceDashboard:
    ...
    async def _recalculate_framework_score(self, framework: ComplianceFramework):
        ...
        self.compliance_scores.append(
            {"timestamp": datetime.now(), "framework": framework.value, "score": final_score}
        )
        ...
        with self._lock:
            ...
            if framework in self.framework_status:
                ...
                if violation.detected_at:
                    remediation_time = (
                        datetime.now() - violation.detected_at
                    ).total_seconds() / 3600
    ...
    def update_governance_metrics(
        self,
        cataloged_assets: int | None = None,
        classified_assets: int | None = None,
        lineage_tracked: int | None = None,
        quality_monitored: int | None = None,
        access_controlled: int | None = None,
        retention_applied: int | None = None,
    ):
    ...
    async def get_dashboard_data(self, user_id: str = "system") -> dict[str, Any]:
        ...
        with self._lock:
            ...
            total_violations = len(
                [v for v in self.violations if v.status == ComplianceStatus.VIOLATION]
            )
            overdue_violations = len(
                [
                    ...
                    and v.remediation_deadline
                    and v.remediation_deadline < datetime.now()
                ]
            )
            ...
            pia_summary = {
                ...
                "pending_assessments": len(
                    [p for p in self.privacy_assessments if p.status == "pending"]
                ),
                "high_risk_assessments": len(
                    [p for p in self.privacy_assessments if p.privacy_risk_score > 7]
                ),
                "avg_risk_score": sum(p.privacy_risk_score for p in self.privacy_assessments)
        ...
        return {
            ...
            "overview": {
                "overall_compliance_score": sum(
                    s.overall_score for s in self.framework_status.values()
                )
                ...
                "compliant_frameworks": len(
                    [
                        s
                        for s in self.framework_status.values()
    ...
    async def handle_websocket_connection(self, websocket: WebSocket):
        ...
        try:
            ...
            await websocket.send_json({"type": "dashboard_data", "data": dashboard_data})
    ...
    def generate_compliance_report(
        ...
        report_data = {
            ...
            "summary": {
                ...
                "compliance_status": framework_status.status.value
                if framework_status
                else "unknown",
                "total_violations": total_violations,
            ...
            "detailed_violations": [
                {
                    ...
                    "days_to_resolve": (v.resolved_at - v.detected_at).days
                    if v.resolved_at
                    else None,
                }
                for v in framework_violations
                ...
                writer.writerow(
                    {
                        ...
                        if len(violation["description"]) > 50
        ...
        while True:
            try:
                ...
                if self.active_connections:
                    dashboard_data = await self.get_dashboard_data()
                    update_data = {"type": "periodic_update", "data": dashboard_data}
    ...
    async def _run_framework_assessment(self, framework: ComplianceFramework):
        ...
        with self._lock:
            if framework in self.framework_status:
                ...
                self.framework_status[framework].next_assessment = datetime.now() + timedelta(
                    days=1
                )
    ...
    async def _cleanup_old_data(self):
        ...
        with self._lock:
            ...
            self.privacy_assessments = [
                p for p in self.privacy_assessments if p.created_at >= cutoff_date
            ]
...
_compliance_dashboard: ComplianceDashboard | None = None
