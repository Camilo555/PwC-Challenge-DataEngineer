    ...
    with session_scope() as s:
        ...
        for r in prod_rows:
            ...
            if not obj:
                ...
                s.add(obj)
                s.flush()
            prod_map[code] = obj.product_key  # type: ignore[attr-defined]

        # Countries
        for r in country_rows:
            name = r["country"]
            obj = s.exec(select(DimCountry).where(DimCountry.name == name)).first()
            if not obj:
                obj = DimCountry(name=name)
                s.add(obj)
                s.flush()
            country_map[name] = obj.country_key  # type: ignore[attr-defined]
        ...
        for r in cust_rows:
            ...
            if cid is None:
                ...
                continue
            obj = s.exec(select(DimCustomer).where(DimCustomer.customer_id == cid)).first()
            if not obj:
                obj = DimCustomer(customer_id=cid)
                s.add(obj)
                s.flush()
            customer_map[cid] = obj.customer_key  # type: ignore[attr-defined]

        # Invoices
        for r in inv_rows:
            ino = r["invoice_no"]
            obj = s.exec(select(DimInvoice).where(DimInvoice.invoice_no == ino)).first()
            if not obj:
                obj = DimInvoice(invoice_no=ino, invoice_status="Completed", invoice_type="Sale")
                s.add(obj)
                s.flush()
            invoice_map[ino] = obj.invoice_key  # type: ignore[attr-defined]
