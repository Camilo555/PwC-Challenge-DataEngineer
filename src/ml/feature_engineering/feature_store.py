...
class FeatureGroup:
    ...
    refresh_interval: str | None = "daily"  # "hourly", "daily", "weekly"
    tags: list[str] = field(default_factory=list)
    owner: str = "system"
    created_at: datetime = field(default_factory=datetime.utcnow)
    updated_at: datetime = field(default_factory=datetime.utcnow)
...
class FeatureStore:
    ...
    async def _cache_feature_group_metadata(
        self, feature_group: FeatureGroup, feature_group_id: str
    ):
    ...
    async def _cache_features(
        ...
        try:
            ...
            await self.cache_manager.set(
                cache_key, cache_data, ttl=self.cache_ttl_seconds, namespace="feature_store"
            )
    ...
    async def _stream_feature_update(
        self,
        feature_group_name: str,
        version: str,
        entity_keys: dict[str, Any],
    ...
    async def register_feature_group(self, feature_group: FeatureGroup) -> str:
        ...
        try:
            ...
            self.supabase.table("feature_groups").insert(feature_group_data).execute()
            ...
            await self._publish_feature_event(
                "group_registered",
                {
            ...
            await self._stream_feature_event(
                "group_registered",
                {
                    "feature_group_id": feature_group_id,
                    "name": feature_group.name,
    ...
    async def ingest_features(
        self, feature_group_name: str, version: str, features_df: pd.DataFrame
    ) -> bool:
        ...
        try:
            ...
            for _, row in features_df.iterrows():
                entity_keys = {key: row.get(key) for key in feature_group["entity_keys"]}
                features = {
                    k: v for k, v in row.to_dict().items() if k not in feature_group["entity_keys"]
                }
                await self._stream_feature_update(
                    feature_group_name, version, entity_keys, features
                )
            ...
            self.metrics_collector.record_histogram(
                "feature_store_ingestion_duration_seconds", duration
            )
        ...
        try:
            ...
            if self.cache_manager and self.enable_caching:
                ...
                if cached_data:
                    if (
                        not timestamp
                        or datetime.fromisoformat(cached_data["timestamp"]) >= timestamp
                    ):
    ...
    async def get_features_batch(
        ...
        try:
            ...
            self.metrics_collector.record_histogram(
                "feature_store_batch_retrieval_duration_seconds", duration
            )
    ...
    async def get_feature_group(self, name: str, version: str) -> dict[str, Any] | None:
        """Get feature group metadata."""
        try:
            result = (
                self.supabase.table("feature_groups")
                .select("*")
                .eq("name", name)
                .eq("version", version)
                .execute()
    ...
    async def list_feature_groups(self, tags: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    async def get_feature_statistics(
        self, feature_group_name: str, version: str, start_date: datetime, end_date: datetime
    ) -> dict[str, Any]:
    ...
    async def _validate_features(self, features_df: pd.DataFrame, feature_group: dict[str, Any]):
        ...
        # Get feature schemas
        schemas_result = (
            self.supabase.table("feature_schemas")
            .select("*")
            .eq("feature_group_id", feature_group["id"])
            .execute()
        )
        ...
        for column in features_df.columns:
            if column in feature_schemas:
                ...
                if schema["data_type"] == "float" and not pd.api.types.is_numeric_dtype(
                    features_df[column]
                ):
                    logger.warning(
                        f"Feature {column} should be numeric but found {features_df[column].dtype}"
                    )
                ...
                if schema["min_value"] is not None:
                    if features_df[column].min() < schema["min_value"]:
                        logger.warning(
                            f"Feature {column} has values below minimum {schema['min_value']}"
                        )

                if schema["max_value"] is not None:
                    if features_df[column].max() > schema["max_value"]:
                        logger.warning(
                            f"Feature {column} has values above maximum {schema['max_value']}"
                        )

                # Check categories for categorical features
                if schema["categories"] and schema["feature_type"] == "categorical":
                    invalid_categories = set(features_df[column].dropna().unique()) - set(
                        schema["categories"]
                    )
                    if invalid_categories:
                        logger.warning(
                            f"Feature {column} has invalid categories: {invalid_categories}"
                        )

    async def _cache_latest_features(
        self, feature_group_name: str, version: str, features_df: pd.DataFrame
    ):
        """Cache latest features for real-time serving."""
        try:
            ...
            for _, row in features_df.iterrows():
                ...
                cache_data = {"features": row.to_dict(), "timestamp": datetime.utcnow().isoformat()}
    ...
    def _generate_cache_key(
        self, feature_group_name: str, version: str, entity_keys: dict[str, Any]
    ) -> str:
...
class FeatureStoreClient:
    ...
    async def get_online_features(
        self, feature_groups: list[dict[str, str]], entity_keys: dict[str, Any]
    ) -> dict[str, Any]:
...
class FeatureLineage:
    ...
    async def track_feature_lineage(
        self,
        feature_name: str,
    ...
    async def get_feature_lineage(self, feature_name: str, depth: int = 5) -> dict[str, Any]:
        ...
        try:
            ...
            def get_upstream_features(feature: str, current_depth: int) -> dict[str, Any]:
                ...
                result = (
                    self.supabase.table("feature_lineage")
                    .select("*")
            ...
            downstream_result = (
                self.supabase.table("feature_lineage")
                .select("*")
                .contains("source_features", [feature_name])
                .execute()
            )
            ...
            for record in downstream_result.data:
                downstream.append(
                    {"feature": record["feature_name"], "transformation": record["transformation"]}
                )
...
class FeatureValidation:
    ...
    async def validate_feature_quality(
        self, feature_group_name: str, version: str, quality_rules: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for feature_name, feature_stats in stats.items():
                ...
                if "null_count" in feature_stats and "count" in feature_stats:
                    null_rate = feature_stats["null_count"] / (
                        feature_stats["count"] + feature_stats["null_count"]
                    )
                ...
                if "unique_count" in feature_stats:
                    if quality_rules.get("min_unique_values"):
                        ...
                        if feature_stats["unique_count"] < min_unique:
                            feature_score -= 0.2
                            feature_issues.append(
                                f"Low uniqueness: {feature_stats['unique_count']} unique values"
                            )

                quality_results["feature_scores"][feature_name] = max(0.0, feature_score)
                if feature_issues:
                    quality_results["issues"].extend(
                        [f"{feature_name}: {issue}" for issue in feature_issues]
                    )
            ...
            quality_results["overall_score"] = (
                total_score / feature_count if feature_count > 0 else 0.0
            )
        ...
        try:
            ...
            for feature_name in reference_stats:
                ...
                if "mean" in ref_stats and "mean" in comp_stats:
                    mean_drift = abs(comp_stats["mean"] - ref_stats["mean"]) / (
                        ref_stats["std"] + 1e-8
                    )
                    ...
                    std_drift = abs(comp_stats["std"] - ref_stats["std"]) / (
                        ref_stats["std"] + 1e-8
                    )
                ...
                if "top_values" in ref_stats and "top_values" in comp_stats:
                    ...
                    if js_divergence > 0.1:
                        ...
                        drift_indicators.append(
                            f"Distribution drift: {js_divergence:.3f} JS divergence"
                        )
    ...
    def _calculate_js_divergence(
        self, dist1: dict[str, float], dist2: dict[str, float], all_values: set
    ) -> float:
