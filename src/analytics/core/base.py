...
class BaseMLComponent(ABC):
    ...
    @abstractmethod
    def fit(self, data: pd.DataFrame | np.ndarray, **kwargs) -> "BaseMLComponent":
    ...
    def fit_transform(self, data: pd.DataFrame | np.ndarray, **kwargs) -> pd.DataFrame | np.ndarray:
    ...
    @classmethod
    def load(cls, path: str | Path) -> "BaseMLComponent":
    ...
    def validate_data(self, data: pd.DataFrame | np.ndarray) -> None:
        """Validate input data"""
        if data is None:
...
class MLExperiment:
    ...
    def get_best_run(self, metric: str, ascending: bool = False) -> dict[str, Any] | None:
        ...
        best_run = (
            min(valid_runs, key=lambda x: x["metrics"][metric])
            if ascending
            else max(valid_runs, key=lambda x: x["metrics"][metric])
        )
