...
class DataValidationUtils:
    ...
    def validate_phone(phone: str | None) -> str | None:
        ...
        if not 7 <= len(cleaned_phone) <= 15:
            raise ValidationError(
                f"Invalid phone number length: {phone}", field="phone", value=phone
            )
    ...
    def validate_positive_number(
        value: int | float | None, field_name: str = "value"
    ) -> int | float | None:
        ...
        if not isinstance(value, int | float) or value <= 0:
            raise ValidationError(
                f"{field_name} must be a positive number", field=field_name, value=value
            )
    ...
    def validate_string_length(
        value: str | None, min_length: int = 0, max_length: int = 255, field_name: str = "value"
    ) -> str | None:
        ...
        if len(value) > max_length:
            raise ValidationError(
                f"{field_name} cannot exceed {max_length} characters", field=field_name, value=value
            )
    ...
    def validate_country_code(country_code: str | None) -> str | None:
        ...
        if not re.match(r"^[A-Z]{2,3}$", country_code.upper()):
            raise ValidationError(
                f"Invalid country code format: {country_code}",
                field="country_code",
                value=country_code,
            )
...
class BusinessRuleValidators:
    ...
    def validate_invoice_number(invoice_no: str | None) -> str | None:
        ...
        if not re.match(r"^[A-Za-z0-9-_.]{3,}$", invoice_no):
            raise ValidationError(
                f"Invalid invoice number format: {invoice_no}", field="invoice_no", value=invoice_no
            )
    ...
    def validate_stock_code(stock_code: str | None) -> str | None:
        ...
        if not re.match(r"^[A-Za-z0-9-_.]{1,50}$", stock_code):
            raise ValidationError(
                f"Invalid stock code format: {stock_code}", field="stock_code", value=stock_code
            )
    ...
    def validate_customer_id(customer_id: str | None) -> str | None:
        ...
        raise ValidationError(
            f"Invalid customer ID format: {customer_id}", field="customer_id", value=customer_id
        )
    ...
    def validate_quantity(quantity: int | float | None) -> int | None:
        ...
        try:
            ...
            if qty > 100000:  # Reasonable upper limit
                raise ValidationError(
                    "Quantity exceeds maximum allowed", field="quantity", value=quantity
                )
            return qty
        except (ValueError, TypeError) as e:
            raise ValidationError(
                f"Invalid quantity format: {quantity}", field="quantity", value=quantity
            ) from e
    ...
    def validate_unit_price(price: int | float | None) -> float | None:
        ...
        try:
            ...
            if price_float <= 0:
                raise ValidationError(
                    "Unit price must be positive", field="unit_price", value=price
                )
            if price_float > 1000000:  # Reasonable upper limit
                raise ValidationError(
                    "Unit price exceeds maximum allowed", field="unit_price", value=price
                )
            return round(price_float, 2)
        except (ValueError, TypeError) as e:
            raise ValidationError(
                f"Invalid unit price format: {price}", field="unit_price", value=price
            ) from e
...
class DataQualityValidators:
    ...
    def check_data_types(data: dict[str, Any], expected_types: dict[str, type]) -> dict[str, Any]:
        ...
        return {
            ...
            "type_consistency_score": max(
                0, (len(expected_types) - len(type_errors)) / len(expected_types)
            ),
        }

    @staticmethod
    def check_value_ranges(
        data: dict[str, Any], ranges: dict[str, dict[str, int | float]]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "range_validity_score": max(0, (len(ranges) - len(range_errors)) / len(ranges))
            if ranges
            else 1.0,
        }
