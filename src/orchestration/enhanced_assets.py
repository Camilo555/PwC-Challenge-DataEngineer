...
class RetailDataConfig(Config):
    ...
    enable_external_enrichment: bool = Field(
        default=True, description="Enable external API enrichment"
    )
    enable_data_quality_checks: bool = Field(
        default=True, description="Enable comprehensive data quality checks"
    )
    enable_spark_processing: bool = Field(
        default=False, description="Use Spark for large-scale processing"
    )
...
class DataQualityResource(ConfigurableResource):
    ...
    def assess_dataframe(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        passes_check = (
            ...
            and null_percentage <= self.max_null_percentage
            and duplicate_percentage <= self.max_duplicate_percentage
        )
    ...
    def create_spark_session(self):
        ...
        try:
            ...
            return create_optimized_session(
                app_name=self.app_name, processing_type="batch", data_size="medium"
            )
        except ImportError:
...
@asset(
    ...
    metadata={"data_source": "CSV files", "update_frequency": "daily", "schema_version": "1.0"},
)
...
@asset(
    ...
    metadata={"layer": "bronze", "processing_type": "standardization"},
)
...
@asset(
    ...
    metadata={"layer": "silver", "processing_type": "business_rules"},
)
def silver_retail_data(
    ...
    df = df[(df["quantity"] > 0) & (df["unit_price"] >= 0) & (df["invoice_no"].notna())]
    ...
    context.add_output_metadata(
        {
            ...
            "quality_score": quality_report["quality_score"],
            "average_transaction_amount": df["total_amount"].mean()
...
def gold_analytics_tables(
    context: AssetExecutionContext, silver_retail_data: pd.DataFrame
...
def get_resources(config: BaseConfig, dagster_config: DagsterConfig):
    ...
    return {
        ...
        "spark": SparkResource(
            app_name="Dagster-Enhanced-Retail-ETL", spark_conf=dagster_config.default_spark_conf
        )
        if config.processing_engine == ProcessingEngine.SPARK
        else None,
    }
...
def create_enhanced_definitions():
    ...
    return Definitions(
        assets=[raw_retail_data, bronze_retail_data, silver_retail_data, gold_analytics_tables],
        jobs=[retail_etl_job, data_quality_job, analytics_job],
        sensors=[file_sensor],
