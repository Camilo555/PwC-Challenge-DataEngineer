...
class ExternalDataEnricher:
    ...
    def enrich_with_currency_rates(
        self, df: pd.DataFrame, base_currency: str = "GBP", target_currency: str = "USD"
    ) -> pd.DataFrame:
        """Enrich data with currency exchange rates."""
        logger.info(f"Enriching with currency rates: {base_currency} to {target_currency}")

        if (
            not settings.currency_api_key
            or settings.currency_api_key == "your_exchangerate_api_key_here"
        ):
    ...
    def _add_mock_currency_data(self, df: pd.DataFrame, base: str, target: str) -> pd.DataFrame:
        ...
        df[f"exchange_rate_{base}_to_{target}"] = np.random.normal(base_rate, 0.05, len(df)).clip(
            min=0.1
        )

        if "unit_price" in df.columns:
            df[f"unit_price_{target}"] = df["unit_price"] * df[f"exchange_rate_{base}_to_{target}"]
    ...
    def enrich_with_weather_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        weather_data = df.apply(
            lambda row: generate_weather(row["country"], row["invoice_timestamp"]), axis=1
        )
        ...
        for i in range(n_products):
            ...
            product = {
                ...
                "brand_tier": np.random.choice(
                    ["Budget", "Mid-range", "Premium"], p=[0.4, 0.5, 0.1]
                ),
                ...
                + pd.Timedelta(days=np.random.randint(0, 1000)),
            }
