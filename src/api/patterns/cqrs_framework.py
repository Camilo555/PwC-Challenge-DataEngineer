...
class Command(ABC):
    ...
    def __init__(self, command_id: str | None = None, correlation_id: str | None = None):
...
class EventStore:
    ...
    async def save_events(
        self, aggregate_id: str, events: list[DomainEvent], expected_version: int = -1
    ):
...
class CommandBus:
        ...
        try:
            ...
            if not command.validate():
                return CommandResult(
                    command_id=command.command_id, success=False, error="Command validation failed"
                )
    ...
    def _find_handler(self, command: Command) -> CommandHandler | None:
...
class QueryBus:
        ...
        try:
            ...
            if cache_key and result.data:
                await self.redis_cache.set(
                    cache_key, json.dumps(result.data), self.cache_ttl_seconds
                )
    ...
    def _find_handler(self, query: Query) -> QueryHandler | None:
...
class CQRSFramework:
    ...
    async def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "event_projectors": sum(
                len(projectors) for projectors in self.read_model_updater.projectors.values()
            ),
...
class SalesAnalyticsQuery(Query):
    ...
    def get_cache_key(self) -> str:
        return (
            f"sales_analytics:{self.date_range['start']}:{self.date_range['end']}:{self.group_by}"
        )
