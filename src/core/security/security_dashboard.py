...
from core.security.compliance_framework import (
...
class AlertManager:
    ...
    def _process_alert_correlations(self):
        ...
        for key in list(self.correlation_keys.keys()):
            self.correlation_keys[key] = [
                alert_id
                for alert_id in self.correlation_keys[key]
    ...
    def _cleanup_old_alerts(self):
        ...
        for alert_id, alert in self.alerts.items():
            if (
                alert.status in {AlertStatus.RESOLVED, AlertStatus.FALSE_POSITIVE}
    ...
    def collect_current_metrics(self) -> SecurityMetrics:
        ...
        try:
            ...
            metrics.critical_threats_24h = len(
                [
                    e
                    for e in threat_dashboard.get("recent_high_severity_events", [])
            ...
            if len(self.metrics_history) > 0:
                ...
                if previous_threats > 0:
                    metrics.threat_trend = (
                        (metrics.total_threats_24h - previous_threats) / previous_threats
                    ) * 100
            ...
            if metrics.dlp_violations_24h > 0:
                metrics.dlp_effectiveness = (
                    metrics.data_blocked / metrics.dlp_violations_24h
                ) * 100

            # Collect compliance metrics
            compliance_dashboard = self.compliance_engine.get_compliance_dashboard()
            metrics.compliance_violations = compliance_dashboard["violation_summary"][
                "total_violations"
            ]
            ...
            metrics.compliance_score = (
                compliance_dashboard["overall_compliance"]["compliance_rate"] * 100
            )
            ...
            metrics.governance_score = (
                governance_dashboard.get("overall_governance_score", 0.0) * 100
            )
            metrics.assets_cataloged = governance_dashboard["catalog_statistics"]["total_assets"]
            metrics.lineage_coverage = governance_dashboard["lineage_statistics"][
                "lineage_coverage"
            ]["coverage_percentage"]
...
class RealTimeNotificationManager:
    ...
    async def _broadcast_to_subscribers(self, data: dict[str, Any], subscription_type: str):
        ...
        for user_id, sub_info in self.user_subscriptions.items():
            if subscription_type in sub_info["subscriptions"] or "all" in sub_info["subscriptions"]:
                try:
...
class SecurityDashboard:
    ...
    def _initialize_default_widgets(self):
        ...
        default_widgets = [
            ...
            DashboardWidget(
                ...
                config={"chart_type": "gauge", "metric": "compliance_score", "target": 95},
            ),
            DashboardWidget(
                ...
                config={"max_alerts": 10, "severity_filter": ["high", "critical"]},
            ),
            DashboardWidget(
    ...
    async def get_dashboard_data(self, user_id: str = "system") -> dict[str, Any]:
        ...
        return {
            ...
            "user_id": user_id,
            "metrics": {"current": current_metrics.__dict__, "aggregated": aggregated_metrics},
            "alerts": {
    ...
    async def _get_widget_data(
        self, widget: DashboardWidget, current_metrics: SecurityMetrics
    ) -> dict[str, Any]:
            ...
            for metric_name in metrics:
                ...
                for m in trend_data:
                    if hasattr(m, metric_name):
                        series_data.append(
                            {"timestamp": m.timestamp.isoformat(), "value": getattr(m, metric_name)}
                        )

                chart_data.append({"name": metric_name, "data": series_data})

            widget_data["data"] = {"chart_type": chart_type, "series": chart_data}
            ...
            while True:
                ...
                if message.get("type") == "subscribe":
                    ...
                    if user_id in self.notification_manager.user_subscriptions:
                        self.notification_manager.user_subscriptions[user_id]["subscriptions"] = (
                            subscriptions
                        )
...
_security_dashboard: SecurityDashboard | None = None
