...
class ETLJobStatus:
    ...
    cost_estimate_dollars: float | None = None
...
class ETLPerformanceMonitor:
    ...
    def start_etl_job(
        self, job_name: str, pipeline_name: str, environment: str = "production", **tags
    ) -> str:
    ...
    def complete_stage(
            ...
            if job.job_name in self.performance_metrics:
                for metrics in reversed(self.performance_metrics[job.job_name]):
                    if (
                        ...
                        and metrics.stage == stage
                        and metrics.end_time is None
                    ):
        ...
        if stage_metrics.duration_seconds > 0:
            stage_metrics.throughput_records_per_second = (
                records_processed / stage_metrics.duration_seconds
            )
        ...
        self.logger.info(
            f"Completed ETL stage: {stage.value} for job {job_id} "
    ...
    def complete_job(self, job_id: str, success: bool = True):
        ...
        self.logger.info(
            f"Completed ETL job: {job_id} ({job.job_name}) - "
    ...
    def record_data_quality_metrics(
        ...
        self.prometheus.set_gauge(
            ...
            {"dataset": dataset_name, "table": table_name, "stage": stage.value},
        )
        ...
        self.logger.info(
            f"Recorded data quality metrics for {dataset_name}.{table_name}: "
    ...
    def get_pipeline_health_metrics(self, pipeline_name: str) -> PipelineHealthMetrics:
        ...
        with self.lock:
            for job_list in self.job_history.values():
                pipeline_jobs.extend(
                    [job for job in job_list if job.pipeline_name == pipeline_name]
                )
        ...
        last_success = (
            max(successful_jobs, key=lambda x: x.start_time).start_time if successful_jobs else None
        )
        last_failure = (
            max(failed_jobs, key=lambda x: x.start_time).start_time if failed_jobs else None
        )
    ...
    def get_etl_dashboard_data(self) -> dict[str, Any]:
        ...
        success_rate_24h = (
            (successful_jobs_24h / total_jobs_24h * 100) if total_jobs_24h > 0 else 100
        )
        ...
        # Get pipeline health
        pipelines = list({job.pipeline_name for job in recent_jobs})
        pipeline_health = {}
    ...
    def _calculate_performance_score(self, job: ETLJobStatus) -> float:
        ...
        if job.total_duration_seconds:
            target_duration = (
                self.sla_targets.get(f"{job.pipeline_name}_max_duration_minutes", 60) * 60
            )
            if job.total_duration_seconds > target_duration:
                duration_penalty = min(
                    50, (job.total_duration_seconds - target_duration) / target_duration * 25
                )
        ...
        if job.success_rate_percent < self.sla_targets["min_success_rate_percent"]:
            success_penalty = (
                self.sla_targets["min_success_rate_percent"] - job.success_rate_percent
            ) * 0.5
    ...
    def _calculate_performance_trend(
        self, jobs_24h: list[ETLJobStatus], jobs_7d: list[ETLJobStatus]
    ) -> str:
    ...
    def _generate_pipeline_recommendations(
        self, pipeline_name: str, success_rate: float, avg_duration: float, failure_count: int
    ) -> list[str]:
        ...
        if success_rate < 95:
            recommendations.append(
                f"Success rate ({success_rate:.1f}%) below target. Review error handling and data validation."
            )

        if avg_duration > 3600:  # 1 hour
            recommendations.append(
                "Long average duration detected. Consider performance optimization and parallelization."
            )

        if failure_count > 3:
            recommendations.append(
                "High failure rate detected. Review pipeline stability and monitoring."
            )
    ...
    def _get_data_quality_summary(self) -> dict[str, Any]:
        ...
        # Calculate averages
        avg_completeness = sum(m.completeness_score for m in recent_quality_metrics) / len(
            recent_quality_metrics
        )
        avg_accuracy = sum(m.accuracy_score for m in recent_quality_metrics) / len(
            recent_quality_metrics
        )
        avg_consistency = sum(m.consistency_score for m in recent_quality_metrics) / len(
            recent_quality_metrics
        )
        avg_overall = sum(m.overall_quality_score for m in recent_quality_metrics) / len(
            recent_quality_metrics
        )
        ...
        for status in DataQualityStatus:
            status_counts[status.value] = len(
                [m for m in recent_quality_metrics if m.quality_status == status]
            )
        ...
        return {
            ...
            "datasets_monitored": len({m.dataset_name for m in recent_quality_metrics}),
        }
            ...
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                """,
                    (
                        ...
                        metrics.quality_status.value,
                        json.dumps(metrics.quality_issues),
                        metrics.check_timestamp.isoformat(),
                    ),
                )
        except Exception as e:
            ...
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                """,
                    (
                        ...
                        metrics.average_duration_24h,
                        metrics.average_duration_7d,
                        metrics.data_quality_trend,
...
_etl_monitor: ETLPerformanceMonitor | None = None
