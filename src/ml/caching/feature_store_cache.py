...
from core.caching.redis_streams import publish_cache_warming
...
class FeatureUsageTracker:
    ...
    async def record_access(
        ...
        await cache_manager.async_redis_client.zadd(
            cache_manager._build_key(entity_key, self.namespace), {str(entity_hash): timestamp}
        )
    ...
    async def get_popular_features(
        self, feature_group: str, time_window: timedelta = timedelta(hours=24), limit: int = 100
        ...
        for _access_record, _score in popular_accesses:
        ...
        for key in all_keys:
            ...
            accesses = await cache_manager.async_redis_client.zcount(key, min_timestamp, "+inf")
    ...
    async def get_access_patterns(
        self, feature_group: str, time_window: timedelta = timedelta(days=7)
    ) -> dict[str, Any]:
        ...
        for _access_record, timestamp in timeline:
...
class FeatureStoreCache:
    ...
    def __init__(
        ...
        # Statistics
        self.stats = {"hits": 0, "misses": 0, "cache_size": 0, "warming_operations": 0}
    ...
    async def cache_features(
        ...
        # Store features
        success = await cache_manager.set(cache_key, cache_data, cache_ttl, self.namespace)
        ...
        if cached_data:
            ...
            if self.usage_tracker:
                await self.usage_tracker.record_access(
                    feature_group,
                    feature_names or list(cached_data.get("features", {}).keys()),
                    entity_keys,
                )
    ...
    async def get_features_batch(
        ...
        for i, (cache_key, cached_data) in enumerate(
            zip(cache_keys, cached_results.values(), strict=False)
        ):
            ...
            if isinstance(loaded_features, dict):
                for entity_id, features in loaded_features.items():
                    if features:
                        ...
                        entity_keys = next(ek for eid, ek in missing_entities if eid == entity_id)
                        await self.cache_features(feature_group, entity_keys, features, version)
    ...
    async def cache_feature_group_metadata(
        self, feature_group: str, version: str, metadata: dict[str, Any], ttl: int | None = None
    ) -> bool:
        ...
        return await cache_manager.set(cache_key, metadata, cache_ttl, self.namespace)

    async def get_feature_group_metadata(
        self, feature_group: str, version: str = "latest"
    ) -> dict[str, Any] | None:
    ...
    async def warm_cache(
        ...
        strategy: CacheWarmingStrategy = CacheWarmingStrategy.EAGER,
    ) -> int:
            ...
            if strategy == CacheWarmingStrategy.EAGER:
                ...
                if self.usage_tracker:
                    ...
                    popular_feature_names = [
                        name for name, _ in popular_features if name != feature_group
                    ]
                    ...
                    if popular_feature_names:
                        if asyncio.iscoroutinefunction(loader_func):
                            loaded_data = await loader_func(
                                feature_group,
                                entity_keys_list[:10],  # Limit entities
                                popular_feature_names[:5],
                                version,  # Top 5 features
                            )
                        else:
                            loaded_data = loader_func(
                                feature_group,
    ...
    async def invalidate_features(
        self, feature_group: str, entity_keys: dict[str, Any] | None = None, version: str = "latest"
    ) -> int:
    ...
    async def get_cache_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "feature_groups": {},
            "cache_health": {"hit_rate": 0.0, "avg_computation_cost": 0.0, "memory_usage": 0},
        }
            ...
            for key in all_keys[:100]:  # Limit for performance
                try:
                    ...
                    if metadata:
                        ...
                        stats["feature_groups"][feature_group]["total_hits"] += metadata.get(
                            "hit_count", 0
                        )
...
_feature_store_cache: FeatureStoreCache | None = None
...
def cache_features(feature_group: str, ttl: int | None = None):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            cached_features = await feature_cache.get_features(feature_group, entity_keys)
            ...
            if isinstance(result, dict):
                await feature_cache.cache_features(
                    feature_group, entity_keys, result, ttl=ttl, computation_cost=computation_time
                )
