...
class StreamingDataTransformer(DataTransformer):
    ...
    def transform(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        sorted_rules = sorted(self.transformation_rules.values(), key=lambda r: r.priority)
        ...
        for field in rule.source_fields:
            ...
            if "email" in validation_logic and value:
                if not re.match(r"^[^\s@]+@[^\s@]+\.[^\s@]+$", str(value)):
                    data.setdefault("_validation_errors", []).append(
                        f"Invalid email format: {field}"
                    )

            if "phone" in validation_logic and value:
                if not re.match(r"^\+?[\d\s\-\(\)]+$", str(value)):
                    data.setdefault("_validation_errors", []).append(
                        f"Invalid phone format: {field}"
                    )
...
class StreamingDataEnricher:
    ...
    async def _apply_enrichment_rule(
        self, rule: EnrichmentRule, data: dict[str, Any]
    ) -> dict[str, Any] | None:
    ...
    async def _enrich_from_customer_api(
        self, customer_id: str, rule: EnrichmentRule
    ) -> dict[str, Any] | None:
    ...
    async def _enrich_from_product_api(
        self, product_code: str, rule: EnrichmentRule
    ) -> dict[str, Any] | None:
    ...
    async def _enrich_from_currency_api(
        self, currency_code: str, rule: EnrichmentRule
    ) -> dict[str, Any] | None:
    ...
    async def _enrich_from_location_api(
        self, location: str, rule: EnrichmentRule
    ) -> dict[str, Any] | None:
    ...
    def _get_from_cache(self, cache_key: str, ttl_seconds: int) -> dict[str, Any] | None:
    ...
    def _store_in_cache(self, cache_key: str, data: dict[str, Any]):
        ...
        if len(self.enrichment_cache) > 10000:
            ...
            oldest_keys = sorted(self.cache_timestamps.items(), key=lambda x: x[1])[:1000]
    ...
    def _get_fallback_data(self, rule: EnrichmentRule) -> dict[str, Any] | None:
        """Get fallback data for failed enrichment"""
        if rule.fallback_value:
            return dict.fromkeys(rule.target_fields, rule.fallback_value)
        return None
...
class RealtimeTransformationPipeline:
    ...
    def _process_topic_pipeline(self, source_topic: str, output_topic: str, consumer_group: str):
        ...
        try:
            consumer = self.kafka_manager.create_consumer(
                [source_topic], f"{consumer_group}_{source_topic}"
            )
    ...
    async def _transform_message(self, message) -> TransformationResult | None:
        ...
        try:
            ...
            if self.metrics_collector:
                self.metrics_collector.increment_counter(
                    ...
                    {"success": str(result.success), "output_topic": output_topic},
                )

                self.metrics_collector.record_histogram(
                    "transformation_processing_time_ms", result.processing_time_ms
                )
...
# Factory function
def create_realtime_transformation_pipeline(
    config: dict[str, Any] | None = None,
) -> RealtimeTransformationPipeline:
...
if __name__ == "__main__":
    ...
    try:
        while pipeline.running:
            ...
            print(
                f"Pipeline Stats: {stats['processed_count']} processed, "
