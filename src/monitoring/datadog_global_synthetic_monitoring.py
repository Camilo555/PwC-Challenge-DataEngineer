...
class DataDogGlobalSyntheticMonitoring:
    ...
    def __init__(self, synthetic_monitoring: DataDogSyntheticMonitoring, base_url: str):
    ...
    def _get_regional_configs(self) -> list[RegionConfig]:
        ...
        return [
            RegionConfig(
                ...
                primary_locations=[TestLocation.AWS_US_EAST_1, TestLocation.AWS_US_WEST_2],
                secondary_locations=[TestLocation.GCP_US_CENTRAL1, TestLocation.AZURE_EAST_US],
            ...
            RegionConfig(
                ...
                primary_locations=[TestLocation.AWS_EU_WEST_1],
                secondary_locations=[TestLocation.AZURE_WEST_EUROPE],
                expected_latency_ms=200,
                cdn_endpoints=["https://cdn.eu-west.example.com"],
                disaster_recovery_endpoints=["https://dr-eu.example.com"],
            ),
            RegionConfig(
                region=GeographicRegion.ASIA_PACIFIC,
                primary_locations=[TestLocation.AWS_AP_SOUTHEAST_1],
                secondary_locations=[],
                expected_latency_ms=300,
                cdn_endpoints=["https://cdn.ap-southeast.example.com"],
                disaster_recovery_endpoints=["https://dr-ap.example.com"],
            ),
        ]
        ...
        return [
            DisasterRecoveryConfig(
                ...
                recovery_validation_steps=[
                    {"step": "health_check", "endpoint": "/health", "expected_status": 200},
                    {
    ...
    async def deploy_regional_tests(
        self,
        test_suite: str = "critical",
        frequency: TestFrequency = TestFrequency.EVERY_15_MINUTES,
                ...
                for test_spec in selected_tests:
                    ...
                    test_config = SyntheticTestConfig(
                        ...
                        locations=all_locations[
                            :2
                        ],  # Limit to 2 locations per region to control costs
                    ...
                    self.logger.info(
                        f"Deployed regional test for {region_name} - {test_spec['name']}: {test_id}"
                    )
                    await asyncio.sleep(1)

            self.logger.info(
                f"Successfully deployed regional tests across {len(deployed_regional)} regions"
            )
    ...
    async def deploy_cdn_monitoring_tests(
        self, frequency: TestFrequency = TestFrequency.EVERY_15_MINUTES
    ) -> dict[str, str]:
            ...
            for cdn_config in self.cdn_configs:
                ...
                for edge_location in cdn_config.edge_locations:
                            ...
                            APITestStep(
                                ...
                                assertions=[
                                    {"type": "statusCode", "operator": "is", "target": 200},
                                    {
                                        "type": "responseTime",
                                        "operator": "lessThan",
                                        "target": cdn_config.performance_thresholds[
                                            "total_download_time"
                                        ],
                                    },
                                    {
                                ...
                                + self._create_content_validation_assertions(
                                    endpoint, cdn_config.content_validation_rules
                                ),
                            )
                        )
                    ...
                    test_id = await self.synthetic_monitoring.create_multistep_api_test(
                        f"CDN - {edge_location}", cdn_steps, test_config
                    )
    ...
    async def deploy_network_latency_tests(
        self, frequency: TestFrequency = TestFrequency.EVERY_5_MINUTES
    ) -> dict[str, str]:
                ...
                for target_host in network_config.target_hosts:
                    ...
                    for test_type in network_config.test_types:
                        SyntheticTestConfig(
                            ...
                            type=TestType.TCP
                            if test_type == NetworkTestType.TCP_CONNECT
                            else TestType.ICMP,
                            url=target_host,
                            frequency=frequency,
                            locations=[
                        ...
                        self.logger.info(
                            f"Deployed network test for {target_host} ({test_type.value}): {test_id}"
                        )
    ...
    async def deploy_disaster_recovery_tests(
        self, frequency: TestFrequency = TestFrequency.HOURLY
    ) -> dict[str, str]:
            ...
            for dr_config in self.dr_configs:
                ...
                dr_steps = [
                    ...
                    APITestStep(
                        ...
                        extract_variables=[{"name": "PRIMARY_STATUS", "type": "statusCode"}],
                    )
                ]
                        ...
                        APITestStep(
                            ...
                            assertions=[
                                {"type": "statusCode", "operator": "is", "target": 200},
                                {
                                    "type": "responseTime",
                                    "operator": "lessThan",
                                    "target": dr_config.rto_threshold
                ...
                test_id = await self.synthetic_monitoring.create_multistep_api_test(
                    f"DR - {dr_config.name}", dr_steps, test_config
                )
    ...
    def _create_content_validation_assertions(
        self, endpoint: str, validation_rules: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    async def analyze_global_performance(self) -> dict[str, Any]:
        ...
        try:
            ...
            for region_name in self.regional_tests.keys():
                ...
                if region_results:
                    response_times = [
                        r["response_time"] for r in region_results if r["status"] == "passed"
                    ]
                    success_rate = (
                        len([r for r in region_results if r["status"] == "passed"])
                        / len(region_results)
                        * 100
                    )
                    ...
                    regional_analysis[region_name] = {
                        ...
                        "locations_tested": list({r["location"] for r in region_results}),
                    }

            # Global SLA analysis
            all_response_times = [
                r["response_time"] for r in all_regional_results if r["status"] == "passed"
            ]
            global_success_rate = (
                len([r for r in all_regional_results if r["status"] == "passed"])
                / len(all_regional_results)
                * 100
            ...
            best_region = (
                min(regional_analysis.items(), key=lambda x: x[1]["avg_response_time"])
                if regional_analysis
                else None
            )
            worst_region = (
                max(regional_analysis.items(), key=lambda x: x[1]["avg_response_time"])
                if regional_analysis
                else None
            )
            ...
            return {
                ...
                "global_performance": {
                    ...
                    "total_locations": len({r["location"] for r in all_regional_results}),
                    "regions_monitored": len(self.regional_tests),
                },
                "regional_analysis": regional_analysis,
                "performance_comparison": {
                    "best_performing_region": best_region[0] if best_region else None,
                    "best_avg_response_time": best_region[1]["avg_response_time"]
                    if best_region
                    else 0,
                    "worst_performing_region": worst_region[0] if worst_region else None,
                    "worst_avg_response_time": worst_region[1]["avg_response_time"]
                    if worst_region
                    else 0,
                    "performance_variance": statistics.stdev(
                        [data["avg_response_time"] for data in regional_analysis.values()]
                    )
                ...
                "sla_compliance": {
                    ...
                    "current_p95_response_time": statistics.quantiles(all_response_times, n=20)[18]
                    if len(all_response_times) >= 20
                    else 0,
                    "sla_met": global_success_rate >= 99.9
                    and (
        ...
        try:
            ...
            return {
                ...
                "global_coverage": {
                    ...
                    "total_global_tests": total_regional_tests
                    + len(self.cdn_tests)
                    + len(self.network_tests)
                    + len(self.dr_tests),
                },
                ...
                "cdn_monitoring": {
                    ...
                    "edge_locations_monitored": sum(
                        len(cdn.edge_locations) for cdn in self.cdn_configs
                    ),
                    "cache_endpoints_tested": sum(
                        len(cdn.cache_endpoints) for cdn in self.cdn_configs
                    ),
                    "performance_thresholds_configured": True,
                },
                "disaster_recovery": {
                    "dr_scenarios_tested": len(self.dr_configs),
                    "failover_endpoints_monitored": sum(
                        len(dr.failover_endpoints) for dr in self.dr_configs
                    ),
                    "rto_monitoring": True,
                    "rpo_monitoring": True,
                ...
                "global_sla_configuration": {
                    ...
                    "primary_regions": [loc.value for loc in global_config.primary_regions]
                    if global_config
                    else [],
                    "secondary_regions": [loc.value for loc in global_config.secondary_regions]
                    if global_config
                    else [],
                    "cdn_monitoring_enabled": global_config.enable_cdn_monitoring
                    if global_config
                    else False,
                    "network_latency_tests_enabled": global_config.enable_network_latency_tests
                    if global_config
                    else False,
                    "disaster_recovery_validation_enabled": global_config.enable_disaster_recovery_validation
                    if global_config
                    else False,
                },
            }
