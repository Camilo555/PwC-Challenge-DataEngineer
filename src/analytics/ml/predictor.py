...
from ..core.feature_engineering import FeatureConfig, FeatureEngineer
from ..core.model_manager import ModelManager, ModelStatus, ModelType
...
class MLPredictor(BaseMLComponent):
    ...
    def __init__(self, config: PredictionConfig | None = None, ml_config: MLConfig | None = None):
    ...
    def fit(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"Starting ML training with {len(data)} samples and {len(data.columns)} features"
        )
        ...
        self.model_type = (
            ModelType.CLASSIFICATION if problem_type == "classification" else ModelType.REGRESSION
        )
        ...
        if self.config.auto_feature_engineering:
            ...
            self.feature_engineer = FeatureEngineer(
                config=FeatureConfig(auto_feature_selection=self.config.enable_feature_selection)
            )
        ...
        if best_model is not None:
            ...
            self.model_manager.registry.update_model(
                self.best_model_id, validation_metrics=test_metrics, status=ModelStatus.VALIDATED
            )
        ...
        for model_name, model in models_to_try:
            try:
                ...
                cv_scores = cross_val_score(
                    model, X_train, y_train, cv=self.config.cross_validation_folds, scoring=scoring
                )
                ...
                self.logger.info(
                    f"{model_name} - CV Score: {mean_cv_score:.4f}, Test Score: {test_score:.4f}"
                )
    ...
    def _generate_prediction_explanation(
        self, model: Any, predictions: np.ndarray, data: pd.DataFrame
    ) -> str:
        ...
        try:
            ...
            if self.model_type == ModelType.CLASSIFICATION:
                ...
                explanation_parts.append(
                    f"Prediction range: {predictions.min():.3f} to {predictions.max():.3f}"
                )
    ...
    def get_model_comparison(self) -> dict[str, Any]:
            ...
            comparison.append(
                {
                    ...
                    "is_best": (
                        model_info["model"] == self.model_manager.load_model(self.best_model_id)
        ...
        # Sort by test score
        comparison.sort(
            key=lambda x: x["test_score"], reverse=self.model_type == ModelType.CLASSIFICATION
        )
...
class PredictionPipeline:
    ...
    def __init__(self, pipeline_name: str, config: PredictionConfig | None = None):
            ...
            if self.predictor.model_type == ModelType.CLASSIFICATION:
                ...
                validation_results = {
                    "mse": mean_squared_error(
                        validation_target, validation_predictions.predictions
                    ),
