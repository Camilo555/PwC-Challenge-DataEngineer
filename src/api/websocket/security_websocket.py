...
class SecurityWebSocketManager:
    ...
    async def connect_websocket(
        ...
        try:
            ...
            if not auth_result.success:
                ...
                self.logger.warning(
                    f"WebSocket authentication failed for {client_ip}: {auth_result.error_message}"
                )
                return None
            ...
            if not any(
                perm in permissions
                for perm in ["perm_dashboard_read", "perm_security_read", "perm_admin_system"]
            ):
                await websocket.close(
                    code=status.WS_1008_POLICY_VIOLATION, reason="Insufficient permissions"
                )
                self.logger.warning(
                    f"WebSocket access denied for user {user_id}: insufficient permissions"
                )
                return None
            ...
            if len(user_connections) >= self.max_connections_per_user:
                await websocket.close(
                    code=status.WS_1013_TRY_AGAIN_LATER, reason="Too many connections"
                )
            ...
            connection = WebSocketConnection(
                ...
                security_context={
                    ...
                    "authentication_method": getattr(
                        auth_result.token_claims, "authentication_method", "jwt"
                    ),
    ...
    async def handle_websocket_message(self, connection_id: str, message: str):
        ...
        try:
            ...
            await self._log_websocket_event(
                connection_id,
                "message_received",
                {"message_type": message_type, "user_id": connection.user_id},
            )
    ...
    async def broadcast_security_event(
        ...
        try:
            ...
            self.logger.info(
                f"Broadcast {event_type.value} to {len(authorized_connections)} connections"
            )
        ...
        for channel in channels:
            ...
            if channel not in self.security_channels:
                failed_subscriptions.append(
                    {"channel": channel, "reason": "Channel does not exist"}
                )
                continue
            ...
            if not any(perm in connection.permissions for perm in required_permissions):
                failed_subscriptions.append(
                    {"channel": channel, "reason": "Insufficient permissions"}
                )
                continue
    ...
    async def _is_authorized_for_message(
        self, connection: WebSocketConnection, message: SecurityWebSocketMessage
    ) -> bool:
    ...
    async def _send_to_connection(self, connection_id: str, message: SecurityWebSocketMessage):
        ...
        try:
            ...
            redacted_data = await self._apply_message_redaction(
                message.data, connection, message.redaction_level
            )
    ...
    async def _apply_message_redaction(
        self, data: dict[str, Any], connection: WebSocketConnection, redaction_level: str
    ) -> dict[str, Any]:
    ...
    def _redact_sensitive_fields(
        self, data: dict[str, Any], level: str = "standard"
    ) -> dict[str, Any]:
        ...
        if command_type == "run_security_scan":
            ...
            await self._send_to_connection(
                ...
                SecurityWebSocketMessage(
                    event_type=WebSocketEventType.DASHBOARD_UPDATE,
                    timestamp=datetime.now(),
                    data={"type": "security_scan_result", "result": result},
                ),
            )
    ...
    async def _log_websocket_event(
        self, connection_id: str, event_type: str, metadata: dict[str, Any]
    ):
        ...
        try:
            self.security_manager.audit_logger.log_audit_event(
                ...
                metadata={"event_type": event_type, "websocket_metadata": metadata},
            )
        except Exception as e:
            ...
            try:
                ...
                if metrics_subscribers:
                    metrics_data = {
                        ...
                        "active_connections": len(self.connections),
                        "active_threats": len(
                            getattr(self.security_manager, "security_events", [])
                        ),
...
_websocket_manager: SecurityWebSocketManager | None = None
