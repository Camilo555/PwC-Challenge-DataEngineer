...
def start_pipeline(**context: dict) -> dict:
...
def validate_environment(**context: dict) -> bool:
...
def monitor_data_quality(**context: dict) -> dict:
    ...
    completeness_score = 100 - sum(
        [v["null_percentage"] for v in quality_metrics["completeness"].values()]
    ) / len(quality_metrics["completeness"])
    validity_score = 100 - sum(
        [v["negative_percentage"] for v in quality_metrics["validity"].values()]
    ...
    if quality_metrics["overall_score"] < threshold:
        ...
        quality_metrics["issues"].append(
            f"Quality score {quality_metrics['overall_score']}% below threshold {threshold}%"
        )

    logger.info(
        f"Data quality assessment: {quality_metrics['overall_score']}% (threshold: {threshold}%)"
    )
...
def upload_to_supabase(**context):
    ...
    try:
        ...
        # Get processed data
        gold_file = context["task_instance"].xcom_pull(
            key="gold_file", task_ids="transform_group.silver_to_gold"
        )
        ...
        return {"status": "failed", "error": str(e), "timestamp": pd.Timestamp.now().isoformat()}
...
def generate_pipeline_report(**context):
    ...
    ingestion_metrics = context["task_instance"].xcom_pull(
        key="ingestion_metrics", task_ids="ingest_group.ingest_raw_data"
    )
    quality_metrics = context["task_instance"].xcom_pull(
        key="quality_metrics", task_ids="monitor_data_quality"
    )
