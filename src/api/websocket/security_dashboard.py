...
class SecureWebSocketConnection:
    ...
    async def send_error(self, error_code: str, error_message: str, details: dict[str, Any] = None):
        ...
        await self.send_message(
            "error",
            {"error_code": error_code, "error_message": error_message, "details": details or {}},
        )
    ...
    def is_rate_limited(self, max_messages: int = 10, window_seconds: int = 60) -> bool:
        ...
        self.rate_limit_window = [
            timestamp for timestamp in self.rate_limit_window if timestamp > window_start
        ]
...
class SecurityDashboardWebSocketManager:
    ...
    async def handle_client_message(
        self, connection: SecureWebSocketConnection, message: dict[str, Any]
    ):
        ...
        try:
            ...
            if connection.is_rate_limited():
                await connection.send_error("rate_limit_exceeded", "Too many messages sent")
                return
            ...
            await connection.send_error("message_processing_error", "Failed to process message")
    ...
    async def _handle_subscribe(self, connection: SecureWebSocketConnection, data: dict[str, Any]):
        ...
        if connection.add_subscription(topic):
            ...
            await connection.send_error(
                "subscription_denied", f"Permission denied for topic: {topic}"
            )

    async def _handle_unsubscribe(
        self, connection: SecureWebSocketConnection, data: dict[str, Any]
    ):
    ...
    async def _handle_get_dashboard_data(
        ...
        try:
            dashboard_data = await self.security_orchestrator.get_unified_dashboard(
                connection.user_id
            )

            await connection.send_message(
                "dashboard_data", dashboard_data, {"data_timestamp": datetime.now().isoformat()}
            )

        except Exception as e:
            await connection.send_error(
                "dashboard_data_error", f"Failed to retrieve dashboard data: {str(e)}"
            )
    ...
    async def _handle_security_scan(
        ...
        if "perm_admin_system" not in connection.permissions:
            await connection.send_error("permission_denied", "Administrative privileges required")
            return
        ...
        try:
            ...
            await connection.send_error("security_scan_error", f"Security scan failed: {str(e)}")

    async def broadcast_to_topic(
        self, topic: str, message_type: str, data: Any, metadata: dict[str, Any] = None
    ):
    ...
    async def _send_welcome_message(self, connection: SecureWebSocketConnection):
        ...
        await connection.send_message(
            ...
            {"connection_established": True, "heartbeat_interval": self.heartbeat_interval},
        )
    ...
    async def _send_topic_initial_data(self, connection: SecureWebSocketConnection, topic: str):
        ...
        try:
            ...
            if topic == "dashboard_metrics":
                initial_data = await self.security_orchestrator.get_unified_dashboard(
                    connection.user_id
                )
            ...
            if initial_data:
                await connection.send_message(
                    f"{topic}_initial_data", initial_data, {"topic": topic, "initial_data": True}
                )
            ...
            try:
                ...
                mock_events = [
                    {
                        ...
                        "details": {
                            "endpoint": "/api/v1/sales/customer-data",
                            "data_type": "email",
                        },
                    },
                    {
                ...
                for event in mock_events:
                    if event["type"] == "dlp_detection":
                        await self.broadcast_to_topic(
                            "dlp_incidents", "dlp_incident", event, {"real_time": True}
                        )
                    elif event["type"] == "access_denied":
                        await self.broadcast_to_topic(
                            "access_control_events", "access_event", event, {"real_time": True}
                        )
                ...
                if self.topic_subscriptions.get("dashboard_metrics"):
                    ...
                    await self.broadcast_to_topic(
                        ...
                        "metrics_update",
                        dashboard_summary,
                        {"update_type": "periodic", "timestamp": current_time.isoformat()},
                    )
    ...
    def get_connection_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_session_duration": sum(
                (datetime.now() - conn.connected_at).total_seconds() for conn in active_connections
            )
...
_websocket_manager: SecurityDashboardWebSocketManager | None = None
...
async def handle_websocket_connection(websocket: WebSocket, token: str | None = None):
    ...
    try:
        ...
        # Clean up connection
        if connection:
            await websocket_manager.disconnect_client(connection.connection_id, "Connection closed")
