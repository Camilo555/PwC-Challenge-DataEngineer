...
class StreamProcessor:
    ...
    def __init__(self, processor_name: str, supported_events: list[EventType] | None = None):
...
class CacheInvalidationProcessor(StreamProcessor):
    ...
    def __init__(self, cache_manager: RedisCacheManager | None = None):
        super().__init__("cache_invalidation", [EventType.CACHE_INVALIDATION])
...
class CacheWarmingProcessor(StreamProcessor):
    ...
    def __init__(self, cache_manager: RedisCacheManager | None = None):
        super().__init__("cache_warming", [EventType.CACHE_WARMING])
        self.cache_manager = cache_manager
...
class RedisStreamsManager:
    ...
    def __init__(
        self, cache_manager: RedisCacheManager | None = None, stream_prefix: str = "events"
    ):
        self.cache_manager = cache_manager
    ...
    async def publish_event(self, event: StreamEvent, stream_name: str | None = None) -> str:
        """
    ...
    async def start_consumers(self, consumer_name: str = "cache-consumer", batch_size: int = 10):
        ...
        for event_type in EventType:
            ...
            if group_name:
                task = asyncio.create_task(
                    self._consume_stream(stream_name, group_name, consumer_name, batch_size)
                )
    ...
    async def _consume_stream(
        self, stream_name: str, group_name: str, consumer_name: str, batch_size: int
    ):
    ...
    async def get_pending_messages(self, stream_name: str, group_name: str) -> list[dict[str, Any]]:
        ...
        try:
            pending = await cache_manager.async_redis_client.xpending(stream_name, group_name)
...
async def publish_cache_invalidation(
    key: str, namespace: str = "", pattern: bool = False, source: str = "system"
) -> str:
...
async def publish_cache_warming(
    key: str, value: Any, ttl: int = 3600, namespace: str = "", source: str = "system"
) -> str:
    ...
    event = StreamEvent(
        ...
        source=source,
        timestamp=datetime.utcnow(),
        data={"key": key, "value": value, "ttl": ttl, "namespace": namespace},
...
async def publish_model_update(
    model_id: str, version: str, metadata: dict[str, Any], source: str = "ml_system"
) -> str:
    ...
    event = StreamEvent(
        ...
        source=source,
        timestamp=datetime.utcnow(),
        data={"model_id": model_id, "version": version, "metadata": metadata},
        priority=2,  # High priority
    )
