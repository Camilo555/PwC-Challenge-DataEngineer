...
class MLCacheManager:
    ...
    def _build_cache_key(
        self, model_id: str, input_hash: str, cache_type: MLCacheType, suffix: str = ""
    ) -> str:
    ...
    async def cache_prediction(
        ...
        ttl_factors: dict[str, float] | None = None,
        ...
        # Build cache key
        cache_key = self._build_cache_key(model_id, input_hash, MLCacheType.PREDICTION)

        # Calculate TTL
        ttl = self.ttl_manager.calculate_ttl(
            MLCacheType.PREDICTION, model_performance=confidence, custom_factors=ttl_factors
        )
        ...
        await cache_manager.async_redis_client.setex(
            cache_manager._build_key(cache_key, self.namespace), ttl, serialized_data
        )
    ...
    async def get_cached_prediction(self, model_id: str, input_data: Any) -> dict[str, Any] | None:
    ...
    async def cache_batch_predictions(
        ...
        # Build cache key
        cache_key = self._build_cache_key(model_id, batch_hash, MLCacheType.BATCH_PREDICTION)

        # Calculate TTL
        ttl = self.ttl_manager.calculate_ttl(
            MLCacheType.BATCH_PREDICTION, custom_factors=ttl_factors
        )
        ...
        logger.info(
            f"Cached batch predictions for model {model_id}, size: {len(serialized_data)} bytes"
        )
        return cache_key

    async def cache_model_artifact(
        self,
        model_id: str,
        model_version: str,
        artifact_type: str,
        ...
        cache_key = self._build_cache_key(
            model_id, artifact_hash, MLCacheType.MODEL_ARTIFACT, artifact_type
        )
    ...
    async def cache_feature_computation(
        self,
        feature_name: str,
        input_data: Any,
        ...
        # Build cache key
        cache_key = self._build_cache_key(feature_name, input_hash, MLCacheType.FEATURE_COMPUTATION)

        # Calculate TTL
        ttl = self.ttl_manager.calculate_ttl(
            MLCacheType.FEATURE_COMPUTATION, custom_factors=ttl_factors
        )
    ...
    async def get_cached_features(
        self, feature_name: str, input_data: Any
    ) -> dict[str, Any] | None:
    ...
    async def _update_hit_count(self, cache_key: str):
        ...
        await cache_manager.async_redis_client.hincrby(
            cache_manager._build_key(metadata_key, self.metadata_namespace), "hit_count", 1
        )

    async def invalidate_model_cache(
        self, model_id: str, cache_types: list[MLCacheType] | None = None
    ) -> int:
        ...
        await publish_cache_invalidation(
            f"ml_model:{model_id}:*", self.namespace, pattern=True, source="ml_cache_manager"
        )
    ...
    async def invalidate_feature_cache(self, feature_name: str) -> int:
        ...
        # Publish invalidation event
        await publish_cache_invalidation(
            f"ml_feature:{feature_name}:*", self.namespace, pattern=True, source="ml_cache_manager"
        )
    ...
    async def get_cache_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "cache_types": {},
            "model_stats": {},
            "size_stats": {"total_entries": 0, "total_size_bytes": 0, "avg_size_bytes": 0},
        }
                ...
                for key in all_keys[:100]:  # Limit to first 100 for performance
                    try:
                        meta_data = await cache_manager.hgetall(
                            key.replace(
                                cache_manager.key_prefix + self.metadata_namespace + ":", ""
                            ),
                    ...
                    if meta_data:
                        ...
                        if cache_entry.is_expired():
                            ...
                            await cache_manager.delete(
                                f"meta:{cache_entry.cache_key}", self.metadata_namespace
                            )
                            cleaned_count += 1
...
_ml_cache_manager: MLCacheManager | None = None
...
def cache_ml_prediction(model_id: str, ttl_factors: dict[str, float] | None = None):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            await ml_cache.cache_prediction(
                model_id, "1.0", input_data, result, ttl_factors=ttl_factors
            )
