...
class ServiceMeshMonitor:
    def __init__(self, jaeger_endpoint: str = "http://jaeger:14268/api/traces"):
        ...
        self.active_connections = Gauge(
            "service_mesh_active_connections", "Active connections per service", ["service"]
        )
    ...
    async def start_monitoring(self):
        ...
        for service_name, health_check in self.services.items():
            task = asyncio.create_task(self._monitor_service_health(service_name, health_check))
                ...
                async with httpx.AsyncClient(timeout=health_check.timeout) as client:
                    with self.tracer.start_as_current_span(f"health_check_{service_name}") as span:
                        ...
                        self.request_duration.labels(
                            service=service_name, method="GET", endpoint="health"
                        ).observe(duration)
                        ...
                        if not is_healthy:
                            span.record_exception(
                                Exception(f"Health check failed: {response.status_code}")
                            )
                            logger.warning(
    ...
    async def _calculate_error_rates(self):
        ...
        for service_name in self.services.keys():
            ...
            recent_requests = [
                ...
                and datetime.fromisoformat(req["timestamp"]) > five_minutes_ago
            ]
            ...
            if recent_requests:
                ...
                self.error_rate.labels(service=service_name, endpoint="all").set(error_rate)
    ...
    async def _update_circuit_breaker_metrics(self):
        ...
        for cb_key, cb_metrics in self.circuit_breakers.items():
            state_value = {"CLOSED": 0, "OPEN": 1, "HALF_OPEN": 2}.get(cb_metrics.state, 0)

            self.circuit_breaker_state.labels(service=cb_metrics.service_name, endpoint=cb_key).set(
                state_value
            )
    ...
    def record_request(
        ...
        self.request_count.labels(
            service=service_name, method=method, status=status_code, endpoint=endpoint
        ).inc()
    ...
    def update_circuit_breaker(
        self, service_name: str, endpoint: str, metrics: CircuitBreakerMetrics
    ):
        """Update circuit breaker metrics"""
            ...
            for service in discovered_services:
                if service["name"] not in self.services:
                    health_check = ServiceHealthCheck(
                        service_name=service["name"], endpoint=service["endpoint"]
                    )
    ...
    async def get_performance_metrics(self) -> dict[str, Any]:
        ...
        p95_response_time = sorted([req["duration"] for req in recent_requests])[
            int(total_requests * 0.95)
        ]
        p99_response_time = sorted([req["duration"] for req in recent_requests])[
...
class ComprehensiveMonitoringIntegration:
    ...
    async def initialize_messaging_monitoring(
        self, rabbitmq_manager, kafka_manager, metric_collector
    ):
        ...
        try:
            ...
            return {
                "overall_status": self._determine_overall_health(
                    service_topology, messaging_health, alert_summary
                ),
...
async def create_comprehensive_monitoring(
    ...
    await monitoring.initialize_messaging_monitoring(
        rabbitmq_manager, kafka_manager, metric_collector
    )
    return monitoring
