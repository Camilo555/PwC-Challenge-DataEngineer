...
class DataDogSyntheticMonitoring:
    ...
    def __init__(
        self, api_key: str, app_key: str, service_name: str = "datadog-synthetic-monitoring"
    ):
        ...
        self.global_config: GlobalMonitoringConfig | None = None
    ...
    async def create_api_synthetic_test(self, config: SyntheticTestConfig) -> str:
        ...
        try:
            test_definition = {
                ...
                "tags": config.tags
                or [f"service:{self.service_name}", f"environment:{config.environment}"],
            ...
            response = requests.post(
                f"{self.base_url}/tests", headers=self.headers, json=test_definition
            )
    ...
    async def create_multistep_api_test(
        self, name: str, steps: list[APITestStep], config: SyntheticTestConfig
    ) -> str:
        ...
        try:
            ...
            for _i, step in enumerate(steps):
                test_step = {
                    ...
                    or [{"type": "statusCode", "operator": "is", "target": 200}],
                }
            ...
            test_definition = {
                ...
                "config": {
                    "configVariables": [
                        {"name": name, "type": "text", "pattern": None, "example": value}
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Failed to create multi-step test: {response.status_code} - {response.text}"
                )
    ...
    async def create_browser_synthetic_test(
        self, name: str, steps: list[BrowserTestStep], config: SyntheticTestConfig
    ) -> str:
        ...
        try:
            browser_steps = []
            for step in steps:
                browser_step = {"type": step.action, "name": step.name, "timeout": step.timeout}

                if step.selector:
                    browser_step["params"] = {"element": step.selector}
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Failed to create browser test: {response.status_code} - {response.text}"
                )
    ...
    async def get_test_results(
        self, test_id: str, from_ts: int | None = None, to_ts: int | None = None
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/tests/{test_id}/results", headers=self.headers, params=params
            )
            ...
            if response.status_code == 200:
                ...
                for result in results_data.get("results", []):
                    test_result = SyntheticTestResult(
                        test_id=test_id,
                        test_name=self.active_tests.get(test_id, {}).name
                ...
                raise Exception(
                    f"Failed to get test results: {response.status_code} - {response.text}"
                )
    ...
    async def update_test_configuration(self, test_id: str, config: SyntheticTestConfig) -> bool:
        ...
        try:
            ...
            response = requests.get(f"{self.base_url}/tests/{test_id}", headers=self.headers)
            ...
            response = requests.put(
                f"{self.base_url}/tests/{test_id}", headers=self.headers, json=updated_config
            )
    ...
    async def delete_synthetic_test(self, test_id: str) -> bool:
        ...
        try:
            response = requests.delete(f"{self.base_url}/tests/{test_id}", headers=self.headers)
    ...
    async def pause_test(self, test_id: str) -> bool:
        ...
        try:
            ...
            response = requests.put(
                f"{self.base_url}/tests/{test_id}", headers=self.headers, json=test_config
            )
    ...
    def get_monitoring_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            if recent_results:
                ...
                uptime_percentage = (
                    (successful_tests / total_tests) * 100 if total_tests > 0 else 100.0
                )

                avg_response_time = sum(r.response_time for r in recent_results) / len(
                    recent_results
                )
            ...
            return {
                ...
                "active_tests_summary": {
                    ...
                    "api_tests": len(
                        [t for t in self.active_tests.values() if t.type == TestType.API]
                    ),
                    "browser_tests": len(
                        [t for t in self.active_tests.values() if t.type == TestType.BROWSER]
                    ),
                    "multistep_tests": len(
                        [t for t in self.active_tests.values() if t.type == TestType.MULTISTEP_API]
                    ),
                },
                "global_monitoring": {
                    "enabled": self.global_config is not None,
                    "primary_regions": [r.value for r in self.global_config.primary_regions]
                    if self.global_config
                    else [],
                    "secondary_regions": [r.value for r in self.global_config.secondary_regions]
                    if self.global_config
                    else [],
                    "cdn_monitoring": self.global_config.enable_cdn_monitoring
                    if self.global_config
                    else False,
                    "disaster_recovery_validation": self.global_config.enable_disaster_recovery_validation
                    if self.global_config
                    else False,
                },
                "sla_compliance": {
                    "availability_target": self.global_config.sla_thresholds.get(
                        "availability", 99.5
                    )
                    if self.global_config
                    else 99.5,
                    "current_availability": uptime_percentage,
                    "response_time_target": self.global_config.sla_thresholds.get(
                        "api_response_time", 2000
                    )
                    if self.global_config
                    else 2000,
                    "current_avg_response_time": avg_response_time,
                    "sla_met": uptime_percentage
                    >= (
                        self.global_config.sla_thresholds.get("availability", 99.5)
                        if self.global_config
                        else 99.5
                    ),
                },
