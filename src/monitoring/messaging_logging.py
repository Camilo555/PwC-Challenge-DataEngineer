...
class MessageErrorTracker:
    ...
    def _update_error_stats(self, log_entry: MessageLogEntry):
        ...
        if log_entry.error_type:
            ...
            self.error_stats["by_error_type"][error_type] = (
                self.error_stats["by_error_type"].get(error_type, 0) + 1
            )

        # By queue/topic
        if log_entry.queue_or_topic:
            queue_topic = log_entry.queue_or_topic
            self.error_stats["by_queue_topic"][queue_topic] = (
                self.error_stats["by_queue_topic"].get(queue_topic, 0) + 1
            )
    ...
    def get_error_summary(self, hours: int = 24) -> dict[str, Any]:
        ...
        recent_errors = [
            error for error in self.errors if error.timestamp.timestamp() > cutoff_time
        ]
...
class MessagingLogger:
    ...
    def log_connection_event(
        self, event_type: MessageEventType, connection_info: str, error: Exception | None = None
    ):
...
_rabbitmq_logger: MessagingLogger | None = None
_kafka_logger: MessagingLogger | None = None
