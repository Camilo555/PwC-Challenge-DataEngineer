...
async def bronze_retail_data(
    context: AssetExecutionContext, config: RetailDataConfig, raw_retail_data: pd.DataFrame
) -> pd.DataFrame:
    """
...
def silver_retail_data(
    context: AssetExecutionContext, bronze_retail_data: pd.DataFrame
) -> pd.DataFrame:
    """
...
def gold_sales_summary(
    ...
    country_sales = (
        df.groupby("country")
        .agg({"revenue": ["sum", "mean", "count"], "quantity": "sum", "customer_id": "nunique"})
        .round(2)
    )
    ...
    monthly_sales = (
        ...
        .agg({"revenue": "sum", "invoice_no": "nunique", "customer_id": "nunique"})
        .round(2)
    )
    ...
    customer_summary = df.groupby("customer_id").agg(
        {"revenue": "sum", "invoice_no": "nunique", "quantity": "sum"}
    )

    # Define customer segments based on RFM-like analysis
    customer_summary["segment"] = "Regular"
    customer_summary.loc[
        customer_summary["revenue"] > customer_summary["revenue"].quantile(0.8), "segment"
    ] = "VIP"
    customer_summary.loc[
        customer_summary["revenue"] < customer_summary["revenue"].quantile(0.2), "segment"
    ] = "Low Value"
...
def data_quality_metrics(
    ...
    # Consistency metrics
    consistency = {
        "description_consistency": df.groupby("stock_code")["description"].nunique().eq(1).sum()
        / df["stock_code"].nunique()
        * 100,
        "price_consistency": df.groupby("stock_code")["unit_price"].std().fillna(0).lt(0.1).sum()
        / df["stock_code"].nunique()
        * 100,
    }
    ...
    context.log.info(
        f"Data quality assessment completed. Overall score: {overall_quality_score:.1f}%"
    )
