...
class ServiceRegistry:
        ...
        try:
            ...
            if len(service_list) < original_length:
                ...
                await self._publish_service_event(
                    "deregistered",
                    ServiceInstance(
                        service_name=service_name, instance_id=instance_id, host="unknown", port=0
                    ),
                )
    ...
    def _consistent_hash_select(
        self, instances: list[ServiceInstance], client_id: str
    ) -> ServiceInstance:
    ...
    async def _mark_service_unhealthy(self, service: ServiceInstance, reason: str):
        ...
        if old_status != ServiceStatus.UNHEALTHY:
            ...
            logger.warning(
                f"Service unhealthy: {service.service_name} ({service.instance_id}) - {reason}"
            )

    async def _publish_service_event(
        self, event_type: str, service: ServiceInstance, extra_data: dict = None
    ):
    ...
    async def get_service_metrics(self, service_name: str | None = None) -> dict[str, Any]:
