...
def run_bronze_etl(**context) -> dict[str, Any]:
    ...
    try:
        ...
        raise AirflowException(f"Bronze layer processing failed: {e}") from e
...
def run_silver_etl(**context) -> dict[str, Any]:
    ...
    try:
        ...
        raise AirflowException(f"Silver layer processing failed: {e}") from e
...
def run_gold_etl(**context) -> dict[str, Any]:
    ...
    try:
        ...
        raise AirflowException(f"Gold layer processing failed: {e}") from e
...
def cleanup_old_data(**context) -> dict[str, Any]:
    ...
    try:
        cleanup_results = {"files_deleted": 0, "space_freed_mb": 0}
...
def enterprise_retail_etl_dag():
    ...
    success_notification = (
        ...
        if airflow_config.slack_webhook_url
        else DummyOperator(task_id="success_notification")
    )
    ...
    failure_notification = (
        ...
        if airflow_config.slack_webhook_url
        else DummyOperator(task_id="failure_notification")
    )
