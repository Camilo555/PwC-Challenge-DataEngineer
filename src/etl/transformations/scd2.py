...
class SCD2Processor(DataFrameTransformationStrategy):
    ...
    def _create_initial_records(
        self, data: pd.DataFrame, effective_date: date
        ...
        result_data[self.config.end_date_column] = pd.to_datetime(
    ...
    def _process_changes(
        self, new_data: pd.DataFrame, existing_data: pd.DataFrame, effective_date: date
    ) -> TransformationResult:
        """Process changes using SCD2 logic."""

        # Get current records only
        current_data = existing_data[existing_data[self.config.is_current_column]].copy()
        ...
        historical_records = existing_data[not existing_data[self.config.is_current_column]]
    ...
    def _prepare_new_records(self, new_records: pd.DataFrame, effective_date: date) -> pd.DataFrame:
        ...
        # Rename _new columns
        rename_dict = {
            col: col.replace("_new", "") for col in result.columns if col.endswith("_new")
        }
    ...
    def _process_changed_records(
        self, changed_records: pd.DataFrame, effective_date: date
        ...
        new_versions[self.config.end_date_column] = pd.to_datetime(
            self.config.default_end_date
        ).date()
    ...
    def _process_deleted_records(
        self, deleted_records: pd.DataFrame, effective_date: date
    ) -> pd.DataFrame:
