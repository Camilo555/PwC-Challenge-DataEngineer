...
def ingest_bronze_chunked(
    chunk_size: int = 10000, max_memory_mb: int = 2048, parallel_chunks: int = 2
) -> bool:
    """
        ...
        for csv_file in csv_files:
            ...
            try:
                ...
                metrics = chunked_bronze_ingestion(
                    input_path=csv_file, output_path=output_file, chunk_size=chunk_size
                )

                logger.info(
                    f"Processed {csv_file.name}: {metrics.total_rows} rows in {metrics.processing_time_seconds:.2f}s"
                )
...
def ingest_bronze_adaptive(
    auto_detect_large_files: bool = True, large_file_threshold_mb: float = 50.0
) -> bool:
    """
        ...
        if large_files:
            ...
            for csv_file, file_size in large_files:
                try:
                    ...
                    logger.info(
                        f"Processed large file {csv_file.name}: {metrics.total_rows} rows in {metrics.processing_time_seconds:.2f}s"
                    )
...
def optimize_bronze_chunk_config(file_path: Path) -> ChunkConfig:
    ...
    try:
        ...
        int(
            ...
            * 1000
            * len(sample_df)
            / (sample_df.memory_usage(deep=True).sum() / (1024 * 1024))
        )
        ...
        max_memory_mb = min(
            int(available_memory_mb * 0.7), 4096
        )  # Use 70% of available memory, max 4GB
        ...
        logger.info(
            f"Optimized config for {file_path.name}: chunk_size={chunk_size}, "
